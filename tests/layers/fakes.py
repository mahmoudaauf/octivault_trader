...
class FakeExchange:
    ...
    def reconcile(self, exchange_orders: list) -> Dict[str, Any]:
        ...
        return {"missing_local": ex_ids - local_ids,
                "stale_local":  local_ids - ex_ids}
...
class FakePortfolio:
    ...
    def buckets(self) -> Dict[str, float]:
        ...
        ext = sum(p["qty"] * p["price"] for p in self._external.values())
        reserved = sum(r.qty for r in self._reservations.values() if not r.released)
        return {"CASH": self._cash, "RESERVED": reserved,
                "TRADING": traded, "EXTERNAL": ext}

    def positions(self) -> Dict[str, Dict[str, Any]]:
        return {**self._trading, **{k: {**v, "external": True}
                                    for k, v in self._external.items()}}

    def classify(self, asset: str) -> str:
        if asset in self._external: return "EXTERNAL"
        if asset in self._trading:  return "TRADING"
    ...
    def reserve(self, sym: str, qty: float, reason: str) -> Optional[str]:
        ...
        self.journal.append({"event": "RESERVE", "token": token,
                             "sym": sym, "qty": qty, "reason": reason})
        return token
    ...
    def apply_fill(self, token: str, symbol: str, qty: float, price: float) -> bool:
        ...
        self.journal.append({
            "event": "FILL", "token": token, "sym": symbol,
            "qty": qty, "price": price,
        })
        return True
    ...
    def force_mutate_external(self, asset: str) -> None:
        ...
        raise PermissionError(
            "EXTERNAL positions are read-only — invariant violated"
        )
...
class FakePolicyGate:
    ...
    def approve(self, intent: Dict[str, Any]):
        ...
        if ok:
            ...
            veto = {"intent_id": intent.get("id"), "reason": reason,
...
class FakeMetrics:
    ...
    def emit(self, severity: str, source_layer: str, msg: str, ctx: dict):
        self.alerts.append({"severity": severity, "layer": source_layer,
                            "msg": msg, "ctx": ctx})
