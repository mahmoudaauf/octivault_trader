...
from utils.indicators import compute_atr, compute_bollinger_bands, compute_ema
...
class DipSniper:
    ...
    def __init__(
        ...
        **kwargs,
        ...
        logger.info(
            f"🚀 {self.name} initialized (P9-compliant, signal-only, max_concurrency={max_concurrency})"
        )
    ...
    async def run_once(self):
        ...
        # Log results
        logger.info(
            f"[{self.name}] run_once END -> Generated {len(self._collected_signals)} signals across {len(self.symbols)} symbols"
        )
        ...
        try:
            ...
            now = time.time()
            if (len(self._collected_signals) == 0) and (
                now - float(self._last_diag_log_ts or 0.0) >= diag_interval
            ):
        ...
        try:
            ...
            try:
                ...
                if md_ready_event and hasattr(md_ready_event, "is_set"):
                    if not md_ready_event.is_set():
                        logger.debug(
                            f"[{self.name}] Market data ready event not set for {symbol}. Returning insufficient_data."
                        )
            ...
            if not isinstance(candles, list) or len(candles) < 50:
                ...
                return {
                    ...
                    "bars": int(len(candles or []) if isinstance(candles, list) else 0),
                }
            ...
            def compute_indicators_sync(dataframe):
                ...
                (
                    ...
                    df_copy["bb_middle"],
                    df_copy["bb_lower"],
                ) = compute_bollinger_bands(df_copy["close"])
            ...
            if df.empty or len(df) < 2:
                return {
                    "symbol": sym,
                    "status": "insufficient_after_indicators",
                    "rows": int(len(df)),
                }
            ...
            dip_percent = (
                (previous["close"] - latest["close"]) / previous["close"] * 100
                if previous["close"] != 0
                else 0.0
            )
            ...
            require_ema = bool(
                self._cfg("DIPSNIPER_REQUIRE_BELOW_EMA", False)
            )  # Changed default to False
            min_score = int(
                self._cfg("DIPSNIPER_MIN_SCORE", 1) or 1
            )  # Lowered from 2 to 1 for higher sensitivity
            ...
            if self.enabled and condition:
                ...
                if uptrend_momentum:
                    ...
                    uptrend_ema_factor = (
                        0.0
                        if ema20 <= 0
                        else float(np.clip((close - ema20) / ema20 * 100, 0.0, 1.0))
                    )
                    # BB upper distance as bonus
                    uptrend_bb_factor = (
                        0.0 if atr <= 0 else float(np.clip((close - bb_upper) / atr, 0.0, 1.0))
                    )
                    # Higher weight on EMA distance, BB is bonus
                    base_confidence = float(
                        np.clip(0.7 * uptrend_ema_factor + 0.3 * uptrend_bb_factor, 0.0, 1.0)
                    )
                else:
                    # DIP-based confidence (original logic)
                    base_confidence = float(
                        np.clip((0.65 * dip_factor) + (0.35 * bb_factor), 0.0, 1.0) * ema_factor
                    )

                # Volume spike adds +10% confidence bonus
                confidence = float(
                    np.clip(base_confidence * (1.1 if volume_spike else 1.0), 0.0, 1.0)
                )
                ...
                if hasattr(self.shared_state, "get_dynamic_param"):
                    agg_factor = float(
                        self.shared_state.get_dynamic_param("aggression_factor", 1.0)
                    )
                ...
                if confidence < min_conf:
                    logger.debug(
                        f"[{self.name}] {symbol} confidence {confidence:.2f} below dynamic threshold {min_conf:.2f}"
                    )
                    return {
                        "symbol": sym,
                        "status": "conf_below_min",
                        "bars": int(len(df)),
                        "conds_met": int(price_below_bb)
                        + int(price_below_ema)
                        + int(dip_deep_enough),
                ...
                min_entry = float(
                    getattr(
                        self.config, "MIN_ENTRY_USDT", getattr(self.config, "SAFE_ENTRY_USDT", 12.0)
                    )
                )
                ...
                logger.info(
                    f"[{self.name}] 📤 BUY signal: {symbol} conf={confidence:.2f} dip={dip_percent:.2f}%"
                )
