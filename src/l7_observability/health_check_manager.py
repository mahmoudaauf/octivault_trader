...
class HealthCheckManager:
        ...
        for component_name, check_func in checks:
            try:
                ...
                if result.status == HealthStatus.UNHEALTHY:
                    self.logger.error(
                        f"❌ CRITICAL: {result.component} is unhealthy. " f"Blocking startup."
                    )
                    return False

            except Exception as e:
                self.logger.error(f"❌ Health check for {component_name} raised exception: {e}")
                return False

        self.last_check_time = datetime.now(timezone.utc)

        # All critical checks passed
        self.logger.info("✅ All critical health checks passed. System ready to start.")
    ...
    async def check_all_optional(self) -> dict[str, HealthCheckResult]:
        ...
        for component_name, check_func in checks.items():
            try:
                ...
                self.logger.debug(f"{status_icon} {component_name}: {result.message}")

            except Exception as e:
                self.logger.warning(f"Optional health check {component_name} failed: {e}")
    ...
    async def _check_logging(self) -> HealthCheckResult:
        ...
        try:
            ...
            return HealthCheckResult(
                component="Logger",
                status=HealthStatus.UNHEALTHY,
                message=f"Logging check failed: {e!s}",
                timestamp=datetime.now(timezone.utc),
            )
    ...
    async def _check_disk_space(self) -> HealthCheckResult:
        ...
        try:
            ...
            return HealthCheckResult(
                component="DiskSpace",
                status=HealthStatus.UNHEALTHY,
                message=f"Disk space check failed: {e!s}",
                timestamp=datetime.now(timezone.utc),
            )
    ...
    def get_health_report(self) -> dict[str, Any]:
        ...
        critical_results = [
            ...
            if r.component in ["DatabaseManager", "ExchangeClient", "SharedState"]
        ]
        ...
        return {
            "overall_status": (
                ...
                if unhealthy
                else (HealthStatus.DEGRADED.value if degraded else HealthStatus.HEALTHY.value)
            ),
