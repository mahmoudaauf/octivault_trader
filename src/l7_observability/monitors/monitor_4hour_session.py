...
class FourHourSessionMonitor:
    ...
    def display_session_info(self, session_state):
        ...
        print(
            f"│ Progress:            {progress_percent:.1f}% [{'█' * int(progress_percent/5)}{' ' * (20 - int(progress_percent/5))}] │"
        )
        print("└────────────────────────────────────────────────────────────────────────────┘")
        print()
    ...
    def display_checkpoint_info(self, checkpoint_data):
        ...
        if balance:
            ...
            print(
                f"│ Total Change:        ${change:,.2f} ({return_pct:+.2f}%) {' ' * (24 - len(f'{change:,.2f}'))}│"
            )
    ...
    def display_checkpoints_timeline(self):
        ...
        for i, cp_file in enumerate(checkpoints, 1):
            try:
                ...
                print(
                    f"│ {status} CP#{cp_num:02d} @ {self.format_duration(elapsed_min):7} | {cycles:6} cycles | ${current:>10,.2f} │"
                )
    ...
    def display_final_summary(self, final_state):
        ...
        print(
            f"│ Total Duration:      {self.format_duration(final_state.get('total_duration_minutes', 0)):50} │"
        )
        ...
        if balance:
            ...
            print(
                f"│ Total Change:        ${total_change:+,.2f} ({return_pct:+.2f}%) {' ' * (20 - len(f'{total_change:+,.2f}'))}│"
            )
