...
def main():
    ...
    checks = [
        ...
        (
            ...
            lambda: grep_file(
                f"{base}/core/config.py", r"self\.dust_liquidation_enabled\s*=", True
            ),
        ),
        (
        ...
        (
            ...
            lambda: grep_file(
                f"{base}/core/config.py", r"self\.DUST_LIQUIDATION_ENABLED\s*=", False
            ),
        ),
        (
        ...
        (
            ...
            lambda: grep_file(
                f"{base}/core/shared_state.py", r"dust_liquidation_enabled:\s*bool\s*=\s*True", True
            ),
        ),
        (
            "✅ shared_state.py: dust_reentry_override field",
            lambda: grep_file(
                f"{base}/core/shared_state.py", r"dust_reentry_override:\s*bool\s*=\s*True", True
            ),
        ),
        (
            "✅ shared_state.py: allow_entry_below_significant_floor field",
            lambda: grep_file(
                f"{base}/core/shared_state.py",
                r"allow_entry_below_significant_floor:\s*bool\s*=\s*False",
                True,
            ),
        ),
        # ===== ExecutionManager Guard Method =====
        (
            "✅ execution_manager.py exists",
            lambda: check_file_exists(f"{base}/core/execution_manager.py"),
        ),
        (
            "✅ execution_manager.py: _check_entry_floor_guard method exists",
            lambda: grep_file(
                f"{base}/core/execution_manager.py", r"async def _check_entry_floor_guard\(", True
            ),
        ),
        (
            r"✅ execution_manager.py: Guard returns Tuple\[bool, str\]",
            lambda: grep_file(
                f"{base}/core/execution_manager.py", r"\)\s*->\s*Tuple\[bool,\s*str\]:", True
            ),
        ),
        (
            "✅ execution_manager.py: Guard checks is_dust_healing_buy bypass",
            lambda: grep_file(
                f"{base}/core/execution_manager.py", r"if is_dust_healing_buy:", True
            ),
        ),
        (
            "✅ execution_manager.py: Guard checks allow_entry_below_significant_floor",
            lambda: grep_file(
                f"{base}/core/execution_manager.py", r"allow_entry_below_significant_floor", True
            ),
        ),
        ...
        (
            ...
            lambda: grep_file(
                f"{base}/core/execution_manager.py",
                r"guard_allowed, guard_reason = await self\._check_entry_floor_guard",
                True,
            ),
        ),
        (
            "✅ execution_manager.py: Quote-based guard blocks execution",
            lambda: grep_file(f"{base}/core/execution_manager.py", r"ENTRY_FLOOR_GUARD", True),
        ),
        # ===== Qty-Based BUY Path Integration =====
        (
            "✅ execution_manager.py: Qty-based BUY has guard check",
            lambda: grep_file(
                f"{base}/core/execution_manager.py", r"estimated_quote = float", True
            ),
        ),
        (
            "✅ execution_manager.py: Qty-based guard calls _check_entry_floor_guard",
            lambda: grep_file(f"{base}/core/execution_manager.py", r"get_mark_price", True),
        ),
        # ===== Documentation =====
        (
            "✅ Implementation doc exists",
            lambda: check_file_exists(f"{base}/DUST_LIQUIDATION_FIX_IMPLEMENTATION.md"),
        ),
        (
            "✅ Implementation doc has testing plan",
            lambda: grep_file(
                f"{base}/DUST_LIQUIDATION_FIX_IMPLEMENTATION.md", r"## Testing Plan", True
            ),
        ),
        (
            "✅ Implementation doc has deployment checklist",
            lambda: grep_file(
                f"{base}/DUST_LIQUIDATION_FIX_IMPLEMENTATION.md", r"## Deployment Checklist", True
            ),
        ),
    ]
