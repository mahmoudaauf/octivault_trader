...
class OrderCacheManager:
    ...
    def __init__(
        ...
        self.stale_order_timeout_sec = float(self.config.get("stale_order_timeout_sec", 300.0))
    ...
    async def get_open_orders(self, symbol: Optional[str] = None) -> dict[str, list[CachedOrder]]:
        ...
        async with self._cache_lock:
            ...
            for sym in symbols_to_check:
                ...
                open_orders = [o for o in self._cache[sym].values() if o.is_open]
                if open_orders:
    ...
    async def get_cache_stats(self) -> dict[str, Any]:
        ...
        async with self._cache_lock:
            ...
            open_cached = sum(
                sum(1 for o in orders.values() if o.is_open) for orders in self._cache.values()
            )
        ...
        try:
            while self._running:
                try:
                    ...
                    self.logger.error(
                        "[OrderCacheManager] Reconciliation error: %s", e, exc_info=True
                    )
        ...
        try:
            async with self._cache_lock:
                ...
                symbols_to_reconcile = [
                    sym
                    for sym, orders in self._cache.items()
                    ...
                    async with self._cache_lock:
                        ...
                        for oid, cached_order in cached_open.items():
                            if oid not in exchange_order_ids:
                                ...
                                self._handle_cache_miss(oid, symbol, cached_order, reason)

                    # Detect new fills from exchange data
                    await self._detect_fills_from_exchange(symbol, exchange_orders)

                except Exception as e:
                    self.logger.error("[OrderCacheManager] Reconcile failed for %s: %s", symbol, e)
        ...
        async with self._cache_lock:
            for symbol in list(self._cache.keys()):
                ...
                stale_ids = [
                    ...
                    and (now - order.last_update) > self.stale_order_timeout_sec
                ]
