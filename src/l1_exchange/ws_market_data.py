...
class WebSocketMarketData:
    ...
    def __init__(
        ...
        *,
        ...
        try:
            from src.l0_core.shared_state import Component as ComponentEnum
        ...
        try:
            from src.l0_core.shared_state import HealthCode as HealthEnum
    ...
    async def subscribe(self, symbols: list[str]) -> None:
        ...
        if new_symbols:
            ...
            self._logger.info(
                f"[WS:Subscribe] Added {new_symbols}, total={len(self._symbols_subscribed)}"
            )
    ...
    async def _get_fallback_symbols(self) -> list[str]:
        ...
        try:
            ...
            if DEFAULT_SYMBOLS:
                ...
                self._logger.info(
                    f"[WS:Fallback] Using {len(symbols)} bootstrap symbols: {symbols[:5]}..."
                )
                return symbols
        except Exception as e:
    ...
    async def _ws_main_loop(self) -> None:
        ...
        while self._running and reconnect_attempts < self.max_reconnect_attempts:
            try:
                self._logger.info(
                    f"[WS:Connect] Connecting (attempt {reconnect_attempts + 1}/{self.max_reconnect_attempts})"
                )
                    ...
                    try:
                        ...
                        if accepted and isinstance(accepted, dict):
                            ...
                            if syms:
                                ...
                                self._logger.info(
                                    f"[WS:AutoSubscribe] Subscribed to {len(syms)} symbols from accepted_symbols"
                                )
                    except Exception as e:
                ...
                if not self._symbols_subscribed:
                    ...
                    if fallback_syms:
                        ...
                        self._logger.warning(
                            f"[WS:Fallback] Subscribed to {len(fallback_syms)} fallback symbols"
                        )
                    ...
                    while self._running:
                        try:
                            ...
                            if "read loop has been closed" in error_msg or "closed" in error_msg:
                                self._logger.info("[WS:Closed] Connection closed, reconnecting...")
                                break
                            elif "disconnected" in error_msg:
                                self._logger.info("[WS:Disconnect] WebSocket disconnected")
                                break
                            else:
                ...
                if reconnect_attempts < self.max_reconnect_attempts:
                    self._logger.info(
                        f"[WS:Backoff] Reconnecting in {self._current_backoff:.1f}s..."
                    )
                    await asyncio.sleep(self._current_backoff)
                    self._current_backoff = min(self._current_backoff * 1.5, self.max_backoff_sec)

        if reconnect_attempts >= self.max_reconnect_attempts:
            self._logger.error(
                f"[WS:Failed] Max reconnect attempts ({self.max_reconnect_attempts}) exceeded"
            )
    ...
    async def _get_binance_client(self) -> Optional[Any]:
        ...
        try:
            ...
            api_key = getattr(self.config, "BINANCE_API_KEY", None) or getattr(
                self.config, "api_key", None
            )
            api_secret = getattr(self.config, "BINANCE_API_SECRET", None) or getattr(
                self.config, "api_secret", None
            )

            if isinstance(self.config, dict):
                api_key = (
                    api_key or self.config.get("BINANCE_API_KEY") or self.config.get("api_key")
                )
                api_secret = (
                    api_secret
                    or self.config.get("BINANCE_API_SECRET")
        ...
        while self._running:
            try:
                ...
                if time_since_msg > 60:
                    ...
                    market_data_count = (
                        len(self.shared_state.market_data)
                        if hasattr(self.shared_state, "market_data")
                        else 0
                    )
        ...
        try:
            if hasattr(self.shared_state, "update_component_health"):
                self.shared_state.update_component_health(
                    component=self._component_key, code=code, reason=reason or f"WebSocket {code}"
                )
        except Exception as e:
...
async def create_ws_market_data(
    shared_state: Any, exchange_client: Any, config: Optional[dict[str, Any]] = None, **kwargs
) -> WebSocketMarketData:
    """
    ...
    ws = WebSocketMarketData(
        shared_state=shared_state, exchange_client=exchange_client, config=config, **kwargs
    )
    return ws
