...
class RebalancePlan:
    ...
    def to_dict(self) -> dict[str, Any]:
        ...
        d["target_allocation"] = {k: v.to_dict() for k, v in self.target_allocation.items()}
...
class RebalancingEngine:
    ...
    def set_target_allocation(self, allocation: dict[str, AllocationTarget]) -> bool:
        ...
        if not (0.95 <= total_weight <= 1.05):  # Allow 5% variance
            self.logger.error(
                f"[RebalancingEngine] Total allocation weight {total_weight:.2%} not ~100%"
            )
            return False
    ...
    async def plan_equal_weight_rebalance(
        self, portfolio_state: PortfolioState, trigger: RebalanceTrigger
    ...
    async def plan_risk_parity_rebalance(
        self,
        portfolio_state: PortfolioState,
    ...
    async def plan_drift_threshold_rebalance(
        self, portfolio_state: PortfolioState, trigger: RebalanceTrigger
    ) -> Optional[RebalancePlan]:
        """
    ...
    async def _generate_rebalance_orders(
        self, plan: RebalancePlan, target_values: dict[str, float], portfolio_state: PortfolioState
    ) -> None:
        ...
        try:
            ...
            if not hasattr(self.execution_manager, "submit_order"):
                self.logger.warning(
                    ...
                    type(self.execution_manager).__name__,
                    plan.plan_id,
                )
                return False
            ...
            for order in plan.rebalance_orders:
                try:
                    ...
                    if result:
                        ...
                        self.logger.debug(
                            f"[RebalancingEngine] Submitted order {order.order_id}: {order.symbol} {order.side}"
                        )
                    else:
                        self.logger.warning(
                            f"[RebalancingEngine] Failed to submit order {order.order_id}"
                        )
                        return False

                except Exception as e:
                    self.logger.error(
                        f"[RebalancingEngine] Error submitting order {order.order_id}: {e}"
                    )
                    return False

            self.logger.info(
                f"[RebalancingEngine] Successfully submitted {len(submitted_orders)} orders for plan {plan.plan_id}"
            )
            return True

        except Exception as e:
            self.logger.error(
                f"[RebalancingEngine] Error executing rebalancing orders: {e}", exc_info=True
            )
            return False
        ...
        try:
            async with self._lock:
                ...
                if execution_success:
                    ...
                    self.logger.error(
                        f"[RebalancingEngine] Rebalance execution failed: {plan.plan_id}"
                    )
                    return False
                ...
                self.logger.info(
                    f"[RebalancingEngine] Executed {plan.strategy.value} rebalance "
    ...
    async def auto_rebalance(
        ...
        strategy: RebalanceStrategy = RebalanceStrategy.DRIFT_THRESHOLD,
    ) -> Optional[RebalancePlan]:
        """
    ...
    def get_summary(self) -> dict[str, Any]:
        ...
        return {
            ...
            "success_rate": (
                ...
                if self.metrics.total_rebalances > 0
                else 0.0
            ),
            "total_fee_cost": self.metrics.total_fee_cost,
            "last_rebalance": self.metrics.last_rebalance_timestamp,
            "target_allocation": {s: t.to_dict() for s, t in self.target_allocation.items()},
