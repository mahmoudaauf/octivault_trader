...
class AnomalyDetector:
    ...
    def __init__(self, min_history_samples: int = 10, review_threshold_sigma: float = 2.5):
        ...
        self.lock = threading.Lock()
        logger.info(
            f"AnomalyDetector initialized (min_samples={min_history_samples}, review_sigma={review_threshold_sigma})"
        )
    ...
    def update_baseline(self, signal_id: str) -> bool:
        ...
        logger.debug(
            f"Updated baseline for {signal_id}: mean={baseline.mean:.4f}, std_dev={baseline.std_dev:.4f}"
        )
        return True
    ...
    def check_signal(self, signal_id: str, value: float) -> AnomalyDetectionResult:
        ...
        if is_moderate:
            ...
            self._log_result(result)
            logger.info(
                f"ANOMALY MODERATE: {signal_id} value={value:.4f} z_score={z_score:.2f} - recommend review"
            )
            return result
        ...
        if not baseline.is_in_valid_range(value):
            ...
            self._log_result(result)
            logger.warning(
                f"ANOMALY RANGE: {signal_id} value={value:.4f} outside range [{baseline.min_val:.4f}, {baseline.max_val:.4f}]"
            )
            return result
        ...
        with _detector_lock:
            if _anomaly_detector is None:
                _anomaly_detector = AnomalyDetector(
                    min_history_samples=10, review_threshold_sigma=2.5
                )
