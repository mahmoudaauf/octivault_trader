...
class MarketRegimeDetector:
    ...
    def _calculate_adx(self, highs: list[float], lows: list[float], closes: list[float]) -> float:
        ...
        try:
            ...
            plus_di = (
                (sum(plus_dm[-self.atr_period :]) / self.atr_period / atr * 100) if atr > 0 else 0.0
            )
            minus_di = (
                (sum(minus_dm[-self.atr_period :]) / self.atr_period / atr * 100)
                if atr > 0
                else 0.0
            )
    ...
    def _calculate_momentum(self, closes: list[float]) -> float:
        ...
        try:
            ...
            change = (
                (closes[-1] - closes[-period]) / closes[-period] if closes[-period] > 0 else 0.0
            )
