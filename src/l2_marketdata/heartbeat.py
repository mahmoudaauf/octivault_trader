...
try:
    ...
    from src.l0_core.stubs import maybe_await
except Exception:
...
class Heartbeat:
        ...
        **_,
        ...
        self.interval = float(
            ...
            if interval_seconds is not None
                ...
                config, "HEARTBEAT_INTERVAL", getattr(config, "HEARTBEAT_INTERVAL_SECONDS", 15.0)
            )
        )
        self.components = set(components or [])
        self._last_overall: Optional[
            tuple[str, str]
        ] = None  # ensure first consolidated line always prints
        self._stop_event = asyncio.Event()
        self._task = None

        # Seed initial status so Watchdog sees us immediately
        self._safe_set_component_status(
            "Heartbeat", "Initialized", f"Ready. Interval={self.interval:.1f}s"
        )
        # Mirror to CSL (don’t await in __init__; schedule)
        try:
            loop = asyncio.get_running_loop()
            loop.create_task(
                _csl_call("log_status", "Heartbeat", "Initialized", "Ready to send periodic beats.")
            )
        except RuntimeError:
    ...
    def _safe_set_component_status(self, comp: str, status: str, detail: str = ""):
        ...
        fn2 = getattr(self.shared_state, "update_component_status", None) or getattr(
    ...
    @staticmethod
    def _summarize(
        statuses: dict[str, dict[str, Any]], restrict: Optional[Iterable[str]] = None
    ) -> tuple[str, str]:
        """
    ...
    async def run_loop(self):
        ...
        await self._safe_update_component_status(
            "Heartbeat", "Running", "Periodic beats activated."
        )
        ...
        try:
            ...
            await self._safe_update_system_health(
                "SystemOverall", "Degraded", f"Heartbeat failure: {e}"
            )
            raise
            ...
            try:
                ...
                try:
                    await asyncio.wait_for(
                        t,
                        timeout=float(
                            getattr(getattr(self, "config", None), "STOP_JOIN_TIMEOUT_S", 5.0)
                        ),
                    )
                except asyncio.CancelledError:
                    pass
...
def spawn_heartbeat(
    ...
    """
    hb = Heartbeat(
        shared_state, config=config, interval_seconds=interval_seconds, components=components
    )
