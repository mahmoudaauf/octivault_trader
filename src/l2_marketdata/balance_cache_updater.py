...
class BalanceCacheUpdater:
    ...
    def __init__(self, shared_state=None, exchange_client=None, config=None):
        ...
        if self.config:
            self.poll_interval_seconds = float(getattr(
                config, 'BALANCE_CACHE_POLL_INTERVAL_SEC', 5.0
            ))
            self.min_balance_increase_threshold = float(getattr(
                config, 'BALANCE_CACHE_MIN_INCREASE_THRESHOLD', 0.01
            ))
            self.enable_logging = bool(getattr(
                config, 'BALANCE_CACHE_LOGGING_ENABLED', True
            ))
        ...
        try:
            while True:
                try:
                    ...
                    self.logger.error(
                        f"[BalanceCacheUpdater] Error in monitoring loop: {e}",
                        exc_info=True
                    )
    ...
    async def _fetch_fresh_balance(self) -> Optional[float]:
        ...
        try:
            ...
            self.logger.warning(
                f"[BalanceCacheUpdater] Failed to fetch fresh balance: {e}"
            )
            return None
    ...
    async def _update_cache(self, new_balance: float, balance_change: float):
        ...
        try:
            ...
            self.logger.error(
                f"[BalanceCacheUpdater] Failed to update cache: {e}",
                exc_info=True
            )
            return False
    ...
    async def force_balance_update(self, new_balance: float):
        ...
        """
        self.logger.info(
            f"[BalanceCacheUpdater:FORCE_UPDATE] "
            f"Forcing balance to ${new_balance:.2f}"
        )
