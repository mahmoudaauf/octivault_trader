...
class TradingCoordinator:
    ...
    def prepare_positions(
        ...
        if len(positions) == 1:
            self.logger.debug(
                f"[TradingCoordinator] Single position, no consolidation needed for {symbol}"
            )
        ...
        self.logger.info(
            f"[TradingCoordinator] Consolidating {len(positions)} positions for {symbol}"
        )
    ...
    def track_trade_execution(
        ...
        self.logger.debug(
            f"[TradingCoordinator] Recorded trade: {symbol} {quantity} @ {entry_price}"
        )
    ...
    def execute_trade(
        self, symbol: str, positions: list[dict[str, Any]], order_params: dict[str, Any]
    ) -> Optional[str]:
        """
    ...
    def _place_order(
        self, symbol: str, quantity: float, entry_price: float, order_params: dict[str, Any]
    ) -> Optional[str]:
        """
    ...
    def get_system_status(self) -> dict[str, Any]:
        ...
        merge_summary = (
            self.shared_state.position_merger.get_merge_summary()
            if self.shared_state.position_merger
            else {}
        )
            ...
            if len(positions) > 1:
                ...
                if not should_merge:
                    diagnosis["issues"].append(
                        "Consolidation not feasible (poor feasibility score)"
                    )

        # Overall readiness
        diagnosis["overall_ready"] = (
            diagnosis["system_ready"]
            and diagnosis["positions_valid"]
            and (not diagnosis["consolidation_needed"] or diagnosis["consolidation_feasible"])
        )
