...
from src.l4_execution.maker_execution import MakerExecutionConfig, MakerExecutor

# Optional: Import EventStore for Phase 5 event sourcing
try:
    from src.l3_portfolio.event_store import EventStore, EventType
except Exception:
    EventStore = None
    EventType = None
...
# =============================
try:
    from src.l0_core.stubs import ExecutionBlocked
except Exception:

    class ExecutionBlocked(Exception):
        def __init__(
            self, code: str, planned_quote: float, available_quote: float, min_required: float
        ):
            self.code = code
            ...
            super().__init__(
                f"{code}: planned={self.planned_quote:.2f} available={self.available_quote:.2f} min_required={self.min_required:.2f}"
            )
    ...
    *,
    ...
    if use_quote_amount is not None:
        ...
        if step > 0:
            ...
            q = (Decimal(str(estimated_qty)) / Decimal(str(step))).to_integral_value(
                rounding=ROUND_UP
            )
...
class ExecutionManager:
    ...
    def _normalize_quantity(self, quantity: float, step_size: float) -> float:
        ...
        try:
            ...
            self.logger.warning(
                "[EM:NormalizeQty] Error normalizing qty=%.8f step=%.8f: %s", qty, step_size, str(e)
            )
            return 0.0
    ...
    def _canonical_exec_result(
        ...
        *,
        ...
        cumm_quote = self._safe_float(
            raw.get("cummulativeQuoteQty") or raw.get("cummulative_quote"), 0.0
        )
        ...
        client_order_id = (
            raw.get("clientOrderId") or raw.get("client_order_id") or raw.get("origClientOrderId")
        )
    ...
    async def _handle_post_fill(
        ...
        try:
            self.logger.debug(
                f"[DEBUG] Entering _handle_post_fill: symbol={symbol} side={side} exec_qty={order.get('executedQty')}"
            )
        except Exception:
            pass
        emitted = False
        ...
        try:
            ...
            self.logger.debug(
                f"[DEBUG] Emitting trade executed event: symbol={sym} side={side_u} tag={tag}"
            )
            ...
            self.logger.debug(
                f"[DEBUG] Trade executed event emitted: symbol={sym} side={side_u} tag={tag} emitted={trade_event_emitted}"
            )
                ...
                if isinstance(fills, list):
                    fee_base = (
                        sum(
                            ...
                            for f in fills
                            if str(
                                f.get("commissionAsset") or f.get("commission_asset") or ""
                            ).upper()
                            == base_asset
                        )
                        or fee_base
            ...
            if hasattr(ss, "record_trade"):
                try:
                    ...
                    _rt_result = await ss.record_trade(
                        sym,
                        side_u,
                        exec_qty,
                        price,
                    ...
                    trade_recorded = True
                    if (
                        isinstance(_rt_result, dict)
                        and _rt_result.get("realized_pnl_delta") is not None
                    ):
                    ...
                    if (
                        ...
                        if notional >= min_notional and hasattr(
                            self.tp_sl_engine, "set_initial_tp_sl"
                        ):
                            try:
            ...
            if delta is None:
                delta = await maybe_call(
                    ss, "compute_realized_pnl_delta", sym, side_u, exec_qty, price
                )
            ...
            if delta_f is None and side_u == "SELL" and not realized_committed:
                try:
                    pos = (
                        getattr(ss, "positions", {}).get(sym, {})
                        if hasattr(ss, "positions")
                        else {}
                    )
                    entry = float(pos.get("avg_price", 0.0) or 0.0)
                    if entry <= 0:
                        ot = (
                            getattr(ss, "open_trades", {}).get(sym, {})
                            if hasattr(ss, "open_trades")
                            else {}
                        )
            ...
            if not realized_committed and delta_f is not None and delta_f != 0.0:
                now = time.time()
                try:
                    ss.metrics["realized_pnl"] = (
                        float(getattr(ss, "metrics", {}).get("realized_pnl", 0.0) or 0.0) + delta_f
                    )
    ...
    async def _update_position_from_fill(
        self, symbol: str, side: str, order: dict[str, Any], tag: str = ""
    ) -> bool:
        """
        ...
        try:
            ...
            pos["last_filled_time"] = (
                order.get("updateTime") or order.get("timestamp") or int(time.time() * 1000)
            )
    ...
    async def _reconcile_delayed_fill(
        ...
        *,
        ...
        if status in ("FILLED", "PARTIALLY_FILLED") and exec_qty > 0:
            has_exchange_oid = bool(merged.get("orderId") or merged.get("exchange_order_id"))
            cid_probe = (
        ...
        if oid_raw in (None, "") and not cid_raw:
            if str(side or "").upper() == "SELL":
                ...
                if not is_non_submission:
                    self._journal(
                        "SELL_RECOVERY_UNAVAILABLE_NO_IDS",
                        {
        ...
        for attempt in range(1, attempts + 1):
            ...
            merged.update(fresh)
            merged.setdefault(
                "exchange_order_id", fresh.get("orderId") or fresh.get("order_id") or oid_raw
            )
            merged.setdefault(
                "client_order_id",
            ...
            fresh_qty = self._safe_float(
                merged.get("executedQty") or merged.get("executed_qty"), 0.0
            )
    ...
    async def _position_sync_qty_tol(self, symbol: str) -> float:
        ...
        try:
            filters = await self.exchange_client.ensure_symbol_filters_ready(
                self._norm_symbol(symbol)
            )
    ...
    def _schedule_sell_fill_recovery(
        ...
        *,
        ...
        sym = self._norm_symbol(symbol)
        key = str(order.get("_sell_finalize_key") or "").strip() or self._sell_finalize_key(
            sym, order
        )
    ...
    def _calc_close_payload(
        self, sym: str, raw: dict[str, Any]
            ...
            if isinstance(fills, list):
                fee_quote = (
                    sum(
                        float(f.get("commission", 0.0) or 0.0)
                        for f in fills
                        if str(f.get("commissionAsset") or f.get("commission_asset") or "").upper()
                        == quote_asset
                    )
                    or fee_quote
                )
        ...
        realized_pnl = 0.0
        pos = (
            getattr(self.shared_state, "positions", {}).get(sym, {})
    ...
    async def _emit_close_events(
        self, sym: str, raw: dict[str, Any], post_fill: Optional[dict[str, Any]] = None
    ) -> None:
        ...
        actual_executed_qty = self._safe_float(
            raw.get("executedQty") or raw.get("executed_qty"), 0.0
        )
            ...
            if close_delta is not None:
                ...
                with contextlib.suppress(Exception):
                    if hasattr(self.shared_state, "increment_realized_pnl"):
                        await maybe_call(
                            self.shared_state, "increment_realized_pnl", float(close_delta)
                        )
                        applied = True
                if not applied:
                    with contextlib.suppress(Exception):
                        cur = float(
                            getattr(self.shared_state, "metrics", {}).get("realized_pnl", 0.0)
                            or 0.0
                        )
                        self.shared_state.metrics["realized_pnl"] = cur + float(close_delta)
                        self.shared_state.realized_pnl = float(
                            self.shared_state.metrics["realized_pnl"]
                        )
    ...
    async def _record_sell_exit_bookkeeping(
        ...
        *,
        ...
        ctx = policy_ctx or {}
        raw_reason = str(
            ctx.get("exit_reason") or ctx.get("reason") or ctx.get("liquidation_reason") or ""
        ).strip()
    ...
    def _sell_finalize_key(self, symbol: str, order: dict[str, Any]) -> str:
        sym = self._norm_symbol(symbol)
        oid = str(
            order.get("orderId") or order.get("exchange_order_id") or order.get("order_id") or ""
        ).strip()
        if oid:
    ...
    def _track_sell_fill_observed(
        self, *, symbol: str, order: dict[str, Any], tag: str = ""
    ) -> None:
        if not isinstance(order, dict):
            return
        status = str(order.get("status", "")).upper()
        ...
        if int(row.get("fill_seen", 0) or 0) <= 0:
            row["fill_seen"] = 1
            row["fill_ts"] = now
            self._sell_finalize_stats["fills_seen"] = (
                int(self._sell_finalize_stats.get("fills_seen", 0) or 0) + 1
            )
        else:
            self._sell_finalize_stats["fills_seen_duplicate"] = (
                int(self._sell_finalize_stats.get("fills_seen_duplicate", 0) or 0) + 1
            )
        ...
        *,
        ...
        if duplicate_attempt:
            if already_finalized:
                self._sell_finalize_stats["duplicate_finalize"] = (
                    int(self._sell_finalize_stats.get("duplicate_finalize", 0) or 0) + 1
                )
        ...
        if int(row.get("fill_seen", 0) or 0) <= 0:
            self._sell_finalize_stats["finalize_without_fill"] = (
                int(self._sell_finalize_stats.get("finalize_without_fill", 0) or 0) + 1
            )
        ...
        if already_finalized:
            ...
            self._sell_finalize_stats["finalized"] = (
                int(self._sell_finalize_stats.get("finalized", 0) or 0) + 1
            )
    ...
    def _audit_sell_finalize_invariant(
        self, *, now: Optional[float] = None, force_log: bool = False
    ) -> None:
        ...
        for key, row in list(self._sell_finalize_state.items()):
            ...
            if fill_seen > 0 and finalized <= 0:
                ...
                if age_s >= assert_window and not bool(row.get("timeout_reported")):
                    ...
                    self._sell_finalize_stats["pending_timeout"] = (
                        int(self._sell_finalize_stats.get("pending_timeout", 0) or 0) + 1
                    )
                    self.logger.error(
        ...
        if self._sell_finalize_log_every > 0 and finalized_total > 0:
            if finalized_total % self._sell_finalize_log_every == 0 and finalized_total != int(
                self._sell_finalize_last_report_finalized
            ):
                should_log = True
    ...
    async def _finalize_sell_post_fill(
        ...
        *,
        ...
        finalize_key = str(
            order.get("_sell_finalize_key") or ""
        ).strip() or self._sell_finalize_key(sym, order)
        ...
        try:
            ...
            self.logger.debug(
                "[SELL_FINALIZE:PostVerify] Failed to queue verification: %s", e, exc_info=True
            )
                ...
                try:
                    ...
                    if hasattr(self.shared_state, "get_position_qty"):
                        ...
                        if isinstance(pos_entry, dict):
                            current_qty = float(
                                pos_entry.get("qty", pos_entry.get("quantity", 0.0)) or 0.0
                            )
                except Exception:
                    current_qty = 0.0
    ...
    def __init__(
        ...
        if not hasattr(self.exchange_client, "place_market_order") or not callable(
            getattr(self.exchange_client, "place_market_order", None)
        ):
        ...
        self.allow_taker_if_within_bps = float(
            self._cfg("execution.allow_taker_if_within_bps", 0.0)
        )
        ...
        self.no_remainder_below_quote = float(
            self._cfg(
                "execution.no_remainder_below_quote",
                getattr(config, "NO_REMAINDER_BELOW_QUOTE", 0.0),
            )
            or 0.0
        )
        ...
        self.bootstrap_phase_1_capital_max = float(
            self._cfg("BOOTSTRAP_PHASE_1_CAPITAL_MAX", 200.0)
        )
        self.bootstrap_phase_3_capital_threshold = float(
            self._cfg("BOOTSTRAP_PHASE_3_CAPITAL_THRESHOLD", 400.0)
        )
        ...
        self._sell_finalize_cache_ttl_s = float(
            self._cfg("SELL_FINALIZE_CACHE_TTL_SEC", 300.0) or 300.0
        )
        ...
        self._close_verification_check_interval_s = float(
            self._cfg("CLOSE_VERIFICATION_INTERVAL_SEC", 2.0) or 2.0
        )
        self._sell_finalize_pending = 0
        self._sell_finalize_assert_window_s = float(
            self._cfg("SELL_FINALIZE_ASSERT_WINDOW_SEC", 30.0) or 30.0
        )
        self._sell_finalize_track_ttl_s = float(
            self._cfg("SELL_FINALIZE_TRACK_TTL_SEC", 3600.0) or 3600.0
        )
        ...
        self._dust_position_tracker: dict[
            str, dict[str, Any]
        ] = {}  # symbol -> {last_remainder, stuck_count, timestamp}
        ...
        self._phantom_detection_enabled = bool(
            self._cfg("PHANTOM_POSITION_DETECTION_ENABLED", True)
        )
        ...
        self._symbol_filters_cache_max_age_s = float(
            getattr(config, "SYMBOL_FILTERS_CACHE_MAX_AGE_S", 900.0) or 900.0
        )
    ...
    async def startup_scan_for_phantoms(self) -> dict[str, str]:
        ...
        if not self._phantom_detection_enabled:
            self.logger.info(
                "[PHANTOM_STARTUP_SCAN] Phantom detection disabled via config. Skipping scan."
            )
            return repairs
        ...
        try:
            ...
            self.logger.info(
                "[PHANTOM_STARTUP_SCAN] Starting scan of %d positions for phantoms", len(positions)
            )
            ...
            for sym, pos_data in list(positions.items()):
                ...
                if qty <= 0:
                    phantoms_found += 1
                    self.logger.warning(
                        "[PHANTOM_STARTUP_DETECTED] %s: qty=%.8f (phantom)", sym, qty
                    )
                    ...
                    if repair_ok:
                        ...
                        phantoms_repaired += 1
                        self.logger.info(
                            "[PHANTOM_STARTUP_REPAIRED] %s: Successfully repaired", sym
                        )
                    else:
                        repairs[sym] = "UNRESOLVED"
                        self.logger.warning("[PHANTOM_STARTUP_UNRESOLVED] %s: Repair failed", sym)
            ...
            self.logger.error(
                "[PHANTOM_STARTUP_SCAN_ERROR] Error during startup scan: %s", str(e), exc_info=True
            )
            return repairs
                ...
                for symbol, pos_data in list(all_positions.items()):
                    try:
                        ...
                        if hasattr(self.shared_state, "safe_price"):
                            current_price = float(
                                await asyncio.create_task(self.shared_state.safe_price(symbol))
                                or 0.0
                            )
                        ...
                        if current_price >= tp_price:
                            self.logger.info(
                                f"[ExitMonitor:TP] {symbol} triggered TP: ${current_price:.4f} >= ${tp_price:.4f}"
                            )
                            await self._execute_tp_exit(symbol, qty, current_price)

                        elif current_price <= sl_price:
                            self.logger.info(
                                f"[ExitMonitor:SL] {symbol} triggered SL: ${current_price:.4f} <= ${sl_price:.4f}"
                            )
                            await self._execute_sl_exit(symbol, qty, current_price)

                        elif now_ts > time_deadline:
                            self.logger.info(
                                f"[ExitMonitor:TIME] {symbol} triggered TIME exit: 4h expired"
                            )
    ...
    async def _execute_time_exit(self, symbol: str, qty: float, price: float):
        ...
        try:
            ...
            self.logger.error(
                f"[ExitMonitor:TIME_ERROR] Failed to execute TIME exit for {symbol}: {e}"
            )
    ...
    async def _get_total_equity(self) -> float:
        ...
        try:
            if hasattr(self.shared_state, "get_nav_quote") and callable(
                self.shared_state.get_nav_quote
            ):
        ...
        try:
            ...
            if free is not None:
                ...
                if num > 0:
                    self.logger.info(
                        f"[NAV BOOTSTRAP] Using free {self.base_ccy} balance: {num:.2f} USDT"
                    )
                    return num
        except Exception as e:
        ...
        self.logger.warning(
            "[NAV] All equity sources returned 0 or None. Account may be in cold-start state."
        )
        return 0.0
    ...
    async def get_tradable_nav(self) -> float:
        ...
        try:
            ...
            if base_capital is None or base_capital <= 0:
                ...
                if nav == 0.0:
                    self.logger.warning(
                        "[NAV] get_tradable_nav() returning 0.0 - account may need bootstrap liquidity"
                    )
                return nav
    ...
    async def _calculate_adaptive_execution_threshold(
        ...
        slippage_estimate = float(
            self._cfg("SLIPPAGE_ESTIMATE_PCT", 0.0005) or 0.0005
        )  # 0.05% slippage
        ...
        edge_sufficiency_ratio = (
            current_market_edge / final_required_edge if final_required_edge > 0 else 0
        )
    ...
    async def _get_min_entry_quote(
        ...
        if price_f <= 0:
            with contextlib.suppress(Exception):
                price_f = float(
                    await maybe_call(self.exchange_client, "get_current_price", sym) or 0.0
                )
        if price_f <= 0:
            with contextlib.suppress(Exception):
    ...
    async def _heartbeat_loop(self):
        ...
        while True:
            ...
            with contextlib.suppress(Exception):
                ...
                _expired = [
                    k
                    for k, ts in list(self._sell_finalize_result_cache_ts.items())
                    if _now - ts > _ttl
                ]
                for k in _expired:
    ...
    async def _emit_status(self, status: str, detail: str = ""):
        try:
            ...
            if callable(upd):
                await (
                    upd("ExecutionManager", status, detail)
                    if asyncio.iscoroutinefunction(upd)
    ...
    def _ensure_semaphores_ready(self):
        ...
        try:
            if self._concurrent_orders_sem is None:
                ...
                cfg_max_conc = int(
                    self._cfg("execution.max_concurrency", self.max_concurrent_orders)
                )
    ...
    def health(self) -> dict[str, Any]:
        ...
        return {
            ...
            "sell_finalize_fills_seen": int(
                getattr(self, "_sell_finalize_stats", {}).get("fills_seen", 0) or 0
            ),
            "sell_finalize_finalized": int(
                getattr(self, "_sell_finalize_stats", {}).get("finalized", 0) or 0
            ),
            "sell_finalize_pending": int(getattr(self, "_sell_finalize_pending", 0) or 0),
            "sell_finalize_duplicate": int(
                getattr(self, "_sell_finalize_stats", {}).get("duplicate_finalize", 0) or 0
            ),
            "sell_finalize_pending_timeout": int(
                getattr(self, "_sell_finalize_stats", {}).get("pending_timeout", 0) or 0
            ),
        }
    ...
    async def wait_for_inflight_sells(self, timeout: float = 3.0, poll_sec: float = 0.1) -> bool:
        ...
        while True:
            ...
            if (
                ...
                and finalize_pending <= 0
                and exit_lock_pending <= 0
            ):
                return True
    ...
    async def get_symbol_filters_cached(
        self, symbol: str, *, max_age_s: Optional[float] = None
    ) -> dict[str, Any]:
    ...
    async def close_trade(
        self, symbol: str, reason: str = "", *, tag: str = "tp_sl", force_finalize: bool = False
    ) -> dict[str, Any]:
        """Compatibility shim expected by RiskManager."""
        return await self.close_position(
            symbol=symbol, reason=reason, tag=tag, force_finalize=force_finalize
        )
            ...
            if exchange_checked and exchange_query_ok:
                ...
                with contextlib.suppress(Exception):
                    if hasattr(self.shared_state, "get_position_quantity"):
                        ...
                        local_qty = float(
                            (self.shared_state.positions.get(sym, {}) or {}).get("quantity", 0.0)
                            or 0.0
                        )
                ...
                if local_qty > 0:
                    ...
                    self._exchange_zero_mismatch_counts.pop(sym, None)
                    self.logger.debug(
                        f"[GetSellable] {sym}: qty=0 from Exchange (authoritative, no local position)"
                    )
                    return 0.0
                ...
                if callable(getp):
                    with contextlib.suppress(Exception):
                        ...
                        if p:
                            ...
                            if qty > 0:
                                self.logger.debug(
                                    f"[GetSellable] {sym}: qty={qty:.6f} from PositionManager"
                                )
                                return qty
            ...
            if use_step_buffer and step_size > 0:
                ...
                if avoid_dust_trap and rounded_no_buffer > 0 and qty_out < rounded_no_buffer:
                    ...
                    if residual_qty > 0:
                        ...
                        try:
                            mark_price = float(
                                (getattr(self.shared_state, "latest_prices", {}) or {}).get(
                                    sym, 0.0
                                )
                                or 0.0
                            )
                        ...
                        if mark_price <= 0:
                            with contextlib.suppress(Exception):
                                get_px = getattr(
                                    self.exchange_client, "get_current_price", None
    ...
    async def _detect_stuck_dust_position(
        self, symbol: str, current_price: float, remainder_qty: float
    ) -> bool:
        """
        ...
        try:
            ...
            if abs(remainder_qty - last_remainder) < 1e-10:  # Effectively equal
                ...
                tracker["last_check_ts"] = now
                tracker["economic_threshold"] = (
                    current_price * remainder_qty if current_price > 0 else 0
                )
    ...
    async def _handle_phantom_position(self, symbol: str) -> bool:
        ...
        try:
            ...
            if has_exchange_position and exchange_qty > 0:
                ...
                try:
                    if hasattr(self.shared_state, "positions") and isinstance(
                        self.shared_state.positions, dict
                    ):
    ...
    def _entry_profitability_feasible(
        ...
        r_min_net = float(
            self._cfg("MIN_NET_PROFIT_AFTER_FEES", 0.0001) or 0.0001
        )  # 0.01% min profit
    ...
    async def _resolve_dynamic_sell_threshold(
        ...
        *,
        ...
        atr_tf = str(
            self._cfg("SELL_DYNAMIC_ATR_TIMEFRAME", self._cfg("VOLATILITY_REGIME_TIMEFRAME", "5m"))
            or "5m"
        )
        atr_period = int(
            self._cfg("SELL_DYNAMIC_ATR_PERIOD", self._cfg("VOLATILITY_REGIME_ATR_PERIOD", 14))
            or 14
        )
        atr_pct = 0.0
        try:
            if hasattr(self.shared_state, "calc_atr"):
                atr_val = float(
                    await maybe_call(self.shared_state, "calc_atr", sym, atr_tf, atr_period) or 0.0
                )
                if atr_val <= 0:
                    atr_val = float(
                        await maybe_call(self.shared_state, "calc_atr", sym, "1m", atr_period)
                        or 0.0
                    )
        ...
        if (
            pos_notional_quote > 0.0
            and micro_notional_cap > 0.0
            and pos_notional_quote < micro_notional_cap
        ):
        ...
        usdt_floor_pct = (
            (min_usdt_floor_effective / pos_notional_quote) if pos_notional_quote > 0 else 0.0
        )

        required_profit_pct = (
            fee_component + slippage_component + volatility_buffer + strategic_buffer
        )
    ...
    async def _check_sell_net_pnl_gate(
        ...
        *,
        ...
        bootstrap_override = (
            bool(policy_ctx.get("bootstrap_sell_override"))
            or rotation_override
            or ("bootstrap_exit" in tag_lower)
        )

        bootstrap_override = bootstrap_override and bool(
            self._cfg("BOOTSTRAP_ALLOW_SELL_BELOW_FEE", True)
        )
        bootstrap_min_net = float(
            policy_ctx.get("bootstrap_sell_min_net", self._cfg("BOOTSTRAP_MAX_NEGATIVE_PNL", 0.0))
            or 0.0
        )
        ...
        get_px = (
            getattr(self.exchange_client, "get_current_price", None)
            or self.exchange_client.get_price
        )
        price = 0.0
        try:
        ...
        deadlock_bypass_max_fraction = float(
            self._cfg("DEADLOCK_SELL_EDGE_BYPASS_MAX_FRACTION", 0.5) or 0.5
        )
        ...
        deadlock_bypass = bool(
            ...
            and (buy_lock_rej >= max(1, deadlock_bypass_trigger_buy) or is_capacity_recovery_sell)
        ...
        if dynamic_gate_enabled and expected_move_pct < required_move_pct and not deadlock_bypass:
            ...
            try:
                await self.shared_state.record_rejection(
                    sym, "SELL", "SELL_DYNAMIC_EDGE_MIN", source="ExecutionManager"
                )
            except Exception:
                pass
            return {
                ...
                "min_usdt_floor_effective": float(
                    dynamic.get("min_usdt_floor_effective", 0.0) or 0.0
                ),
        ...
        if legacy_net_pct_guard and min_net_pct > 0 and net_after_fees_pct < min_net_pct:
            ...
            try:
                await self.shared_state.record_rejection(
                    sym, "SELL", "SELL_NET_PCT_MIN", source="ExecutionManager"
                )
            except Exception:
                pass
            return {
        ...
        if net_pnl < min_net and not self.allow_sell_below_fee:
            ...
            try:
                await self.shared_state.record_rejection(
                    sym, "SELL", "SELL_NET_PNL_MIN", source="ExecutionManager"
                )
            except Exception:
                pass
            return {
        ...
        try:
            ...
            if min_required > 0 and projected < (realized + min_required):
                self.logger.info(
                    ...
                    sym,
                    projected,
                    realized + min_required,
                    realized,
                    min_required,
                )
                try:
                    await self.shared_state.record_rejection(
                        sym, "SELL", "PORTFOLIO_PNL_IMPROVEMENT", source="ExecutionManager"
                    )
                except Exception:
                    pass
                return {
    ...
    async def _check_dust_retirement_before_rejection(self, symbol: str, side: str) -> bool:
        ...
        if self.shared_state and hasattr(self.shared_state, "is_permanent_dust"):
            if self.shared_state.is_permanent_dust(sym):
                self.logger.debug(
                    f"[DUST_RETIREMENT] {sym} is PERMANENT_DUST, skipping rejection recording"
                )
                return True
    ...
    async def _is_position_terminal_dust(self, symbol: str) -> bool:
        ...
        try:
            if hasattr(
                self.shared_state, "is_permanent_dust"
            ) and self.shared_state.is_permanent_dust(sym):
                return True
        except Exception:
            pass
        ...
        try:
            ...
            if not price or price <= 0:
                ...
                price = (
                    float(pos.get("mark_price", 0.0))
                    or float(pos.get("avg_price", 0.0))
                    or float(pos.get("entry_price", 0.0))
                )
                if not price or price <= 0:
    ...
    async def _record_buy_block(self, symbol: str, available_quote: float) -> None:
        state = self._buy_block_state.setdefault(
            symbol, {"count": 0, "blocked_until": 0.0, "last_available": 0.0}
        )
        ...
        if state["count"] >= self.exec_block_max_retries:
            ...
            effective_cooldown_sec = max(
                30, int(self.exec_block_cooldown_sec / 20)
            )  # ~30s cooldown minimum
    ...
    def _is_live_trading_mode(self) -> bool:
        return (
            bool(self._cfg("LIVE_MODE", False))
            and not bool(self._cfg("SIMULATION_MODE", False))
            and not bool(self._cfg("PAPER_MODE", False))
            and not bool(self._cfg("TESTNET_MODE", False))
    ...
    def _ensure_trade_journal_ready(self, *, reason: str = "runtime") -> bool:
        ...
        if not bool(self._journal_bootstrap_attempted):
            ...
            try:
                from src.l3_portfolio.trade_journal import TradeJournal
        ...
        if bool(self._require_trade_journal_live and self._is_live_trading_mode()) and not bool(
            self._journal_fallback_warned
        ):
            self.logger.critical(
    ...
    async def _verify_position_invariants(
        self, symbol: str, event_type: str, before_qty: float = 0.0
    ) -> bool:
        """
        ...
        try:
            ...
            try:
                if hasattr(self.shared_state, "get_position_quantity"):
                    ...
                    internal_qty = float(
                        (self.shared_state.positions.get(sym, {}) or {}).get("quantity", 0.0) or 0.0
                    )
            except Exception:
                pass
            ...
            if event_type == "PERIODIC_SYNC_CHECK":
                ...
                if drift > eps:
                    ...
                    if internal_qty > 0 and (drift / internal_qty) > 0.01:
                        ...
                        self._journal(
                            "LARGE_POSITION_DRIFT",
                            {
                                "symbol": sym,
                                "exchange_qty": float(exchange_qty),
                                "internal_qty": float(internal_qty),
                                "drift_abs": float(drift),
                                "drift_pct": float((drift / internal_qty) * 100)
                                if internal_qty > 0
                                else 0.0,
                            },
                        )
    ...
    async def _passes_profit_gate(
        ...
        try:
            pos = (
                await self.shared_state.get_position(symbol)
            ...
            if not pos or not isinstance(pos, dict):
                ...
                self.logger.warning(
                    f"[EM:ProfitGate] Position not found for {symbol}, allowing SELL"
                )
                return True
            ...
            self.logger.warning(
                f"[EM:ProfitGate] Error getting position {symbol}: {e}, allowing SELL"
            )
            return True
            ...
            payload.update(
                {
                    ...
                    or order.get("cummulative_quote"),
    ...
    async def _emit_trade_audit(
        ...
        *,
        ...
        try:
            ...
            cumm_quote = self._safe_float(
                order.get("cummulativeQuoteQty") or order.get("cummulative_quote"), 0.0
            )
            status = str(order.get("status", "")).upper()
            ...
            if side_u == "SELL":
                opened_at = float(
                    (ot or {}).get("opened_at") or (ot or {}).get("created_at") or 0.0
                )
                if opened_at > 0:
            ...
            try:
                if avg_price > 0 and hasattr(ss, "calc_atr"):
                    ...
                    atr_tf_primary = str(
                        self._cfg("SELL_DYNAMIC_ATR_TIMEFRAME", "5m") or "5m"
                    ).strip()
                    atr_val = 0.0
                    checked = set()
                    for tf in (atr_tf_primary, "5m", "1h", "1m"):
                        ...
                        with contextlib.suppress(Exception):
                            atr_candidate = float(
                                await maybe_call(ss, "calc_atr", sym, tf_norm, atr_period) or 0.0
                            )
                            if atr_candidate > 0:
    ...
    async def _audit_post_fill_accounting(
        ...
        *,
        ...
        if side_l == "buy":
            cumm_quote = self._safe_float(
                (raw or {}).get("cummulativeQuoteQty") or (raw or {}).get("cummulative_quote"), 0.0
            )
            if cumm_quote > 0:
        ...
        if issues:
            ...
            try:
                await maybe_call(
                    self.shared_state, "emit_event", "ACCOUNTING_INVARIANT_BREACH", snapshot
                )
            except Exception:
    ...
    async def _on_order_failed(
        self, symbol: str, side: str, reason: str, quote: Optional[float] = None
    ):
        """
                ...
                try:
                    ...
                    if float(spendable) < (quote * 0.5):
                        ...
                        if hasattr(self.shared_state, "prune_reservations"):
                            ...
                            self.logger.info(
                                f"[OrderFailed:Prune] ✅ Pruned reservations for {symbol}"
                            )
                except Exception as e:
    ...
    def _classify_execution_error(
        self, exception: Exception, symbol: str = "", operation: str = ""
    ) -> ExecutionError:
        if isinstance(exception, BinanceAPIException):
            ...
            if code == -2010:
                ...
                return ExecutionError(
                    "ExternalAPIError", str(exception), symbol, {"api_code": code}
                )
    ...
    def _sanitize_tag(self, tag: Optional[str]) -> str:
        ...
        if not (
            s.startswith("meta/")
            or s in ("balancer", "liquidation", "tp_sl", "rebalance", "meta_exit")
        ):
            s = "meta/" + s
        out = []
        for ch in s:
                ...
                if last_rejection_ts and (now - last_rejection_ts) > self._rejection_reset_window_s:
                    ...
                    if hasattr(self.shared_state, "clear_rejections"):
                        ...
                        self.logger.info(
                            ...
                            sym,
                            side_upper,
                            self._rejection_reset_window_s,
                        )
            except Exception as e:
        ...
        try:
            if hasattr(self.shared_state, "get_nav_quote"):
                ...
                nav_q = await asyncio.wait_for(
                    maybe_call(self.shared_state, "get_nav_quote", self.base_ccy), timeout=2.0
                )
        except Exception:
            pass
    ...
    async def _get_free_quote_and_remainder_ok(
        self, quote_asset: str, spend: float
    ...
    async def can_afford_market_buy(
        ...
        ok, gap, reason = await self._explain_afford_market_buy_tuple(
            symbol,
    ...
    async def explain_afford_market_buy(
        self, symbol: str, quote_amount: Union[float, Decimal]
    ) -> tuple[bool, str]:
        ok, _, reason = await self._explain_afford_market_buy_tuple(
            symbol, Decimal(str(quote_amount))
        )
        return ok, reason
    ...
    async def _explain_afford_market_buy_tuple(
        ...
        affordability_probe = bool(
            policy_ctx.get("affordability_probe") or policy_ctx.get("probe_only")
        )
            ...
            try:
                if hasattr(self.shared_state, "is_bootstrap_mode"):
                    bootstrap_mode_active = bool(
                        await maybe_call(self.shared_state, "is_bootstrap_mode")
                    )
            except Exception:
        ...
        policy_authority = (
            str(policy_ctx.get("authority") or policy_ctx.get("policy_authority") or "")
            .strip()
            .lower()
        )
        meta_validated = bool(
        ...
        try:
            ...
            if price <= 0:
                get_px = (
                    getattr(self.exchange_client, "get_ticker_price", None)
            ...
            try:
                if hasattr(self.shared_state, "calc_atr"):
                    ...
                    for tf in atr_timeframes:
                        atr_candidate = float(
                            await maybe_call(self.shared_state, "calc_atr", sym, tf, atr_period)
                            or 0.0
                        )
                        if atr_candidate > 0:
                            atr = atr_candidate
            ...
            trade_regime = (
                str(policy_ctx.get("tradeability_regime") or policy_ctx.get("_regime") or "")
                .strip()
                .lower()
            )
            ...
            if not vol_regime:
                ...
                try:
                    if hasattr(self.shared_state, "get_volatility_regime"):
                        reginfo = await maybe_call(
                            self.shared_state, "get_volatility_regime", sym, tf, 600
                        )
                        if not reginfo:
                            reginfo = await maybe_call(
                                self.shared_state, "get_volatility_regime", "GLOBAL", tf, 600
                            )
            ...
            if not vol_regime:
                try:
                    vol_regime = (
                        str(
                            (getattr(self.shared_state, "metrics", {}) or {}).get(
                                "volatility_regime"
                            )
                            or ""
                        )
            ...
            if not skip_micro_trade_kill_switch and bool(
                self._cfg("DISABLE_SIDEWAYS_REGIME_TRADING", True)
            ):
            ...
            if not skip_micro_trade_kill_switch:
                ...
                if expected_move_pct is None:
                    econ_guard = (
                        policy_ctx.get("economic_guard") if isinstance(policy_ctx, dict) else None
                    )
                ...
                if not trade_regime or regime == "unknown":
                    try:
                        if hasattr(self.shared_state, "get_current_regime"):
                            mc_regime = await maybe_call(
                                self.shared_state, "get_current_regime", sym
                            )
                            if mc_regime:
                                regime = str(mc_regime).strip().lower()
                                self.logger.debug(
                                    "[EM:EV_HARD_GATE] Fetched regime from MetaController: %s",
                                    regime,
                                )
                    except Exception as e:
                        self.logger.debug(
                            "[EM:EV_HARD_GATE] Failed to fetch regime from MetaController: %s", e
                        )
                ...
                regime_mult = float(
                    regime_multipliers.get(regime, 0.95)
                )  # Default to conservative 0.95
                ...
                mult = max(
                    float(min_safe_mult), regime_mult
                )  # ← Adaptive intelligence wins, safety floor wins vs 1.0x
                ...
                if bool(self._cfg("EV_EXPECTED_MOVE_CALIBRATION_ENABLED", True)):
                    try:
                        ...
                        if hasattr(self.shared_state, "calc_atr") and em_mult > 0:
                            em_atr = float(
                                await maybe_call(
                                    self.shared_state, "calc_atr", sym, em_tf, em_period
                                )
                                or 0.0
                            )
                            if em_atr > 0 and price > 0:
                                expected_move_floor_pct = (
                                    em_atr / max(float(price), 1e-9)
                                ) * em_mult
                    except Exception:
                        expected_move_floor_pct = 0.0
                    if expected_move_floor_pct <= 0.0:
                        fallback_scale = float(
                            self._cfg("EV_EXPECTED_MOVE_FALLBACK_SCALE", 3.0) or 3.0
                        )
                        expected_move_floor_pct = float(atr_pct or 0.0) * max(1.0, fallback_scale)
                    if expected_move_floor_pct > 0:
                        expected_move_used_pct = max(
                            float(expected_move_used_pct), float(expected_move_floor_pct)
                        )
                ...
                # CRITICAL: If MetaController has sent an explicit override, bypass this gate
                if (
                    float(expected_move_used_pct) <= float(required_move_pct)
                    and not bootstrap_override
                ):
                    self.logger.warning(
            ...
            if not feasible and not is_dust_operation and not bootstrap_override:
                ...
                with contextlib.suppress(Exception):
                    await maybe_call(
                        self.shared_state, "emit_event", "EntryProfitabilityBlocked", payload
                    )
            ...
            if not skip_micro_trade_kill_switch:
                if self._cfg("ADAPTIVE_EXECUTION_THRESHOLD_ENABLED", True):  # Default enabled
                    ...
                    if policy_context:
                        signal_confidence = policy_context.get("confidence") or policy_context.get(
                            "signal_confidence"
                        )
                    ...
                    if not threshold_analysis["can_execute"]:
                        kill_reason = (
                            threshold_analysis["analysis"]["kill_reason"]
                            or "ADAPTIVE_THRESHOLD_KILL"
                        )
                        self.logger.warning(
            ...
            if is_dust_operation:
                ...
                min_required_val = await self._get_min_entry_quote(
                    sym, price=price, min_notional=float(min_notional), policy_context=policy_ctx
                )
            ...
            acc_val = (
                Decimal(str(intent.accumulated_quote))
            ...
            bypass_internal_economic_floor = (
                bypass_min_notional or is_dust_operation or skip_micro_trade_kill_switch
            )
            ...
            if not bypass_internal_economic_floor:
                ...
                if effective_qa < min_required - eps:
                    ...
                    if (
                        spendable_dec > 0
                        and (qa <= spendable_dec + eps)
                        and (spendable_dec + acc_val < min_required - eps)
                    ):
            ...
            if planned_qty is not None and not is_dust_operation:
                ...
                if abs(implied_quote - float(effective_qa)) > max(0.01, float(effective_qa) * 0.05):
                    ...
                    step = step_size
                    est_units = (
                        self._round_step_down(est_units_raw, step) if step > 0 else est_units_raw
                    )
            ...
            if spendable_dec < gross_needed_est - eps:
                ...
                if max_qa >= Decimal(str(exchange_floor)) or bypass_min_notional:
                    self.logger.info(
                        f"[EM] Dynamic Resizing: Downscaling {qa} -> {max_qa:.2f} to fit spendable {spendable_dec:.2f} (est_notional was {est_notional:.2f})"
                    )
            ...
            if price > 0:
                filters_obj = SymbolFilters(
                    ...
                    min_notional=float(min_notional),
                    min_entry_quote=(
                        0.0 if bypass_internal_economic_floor else float(min_required)
                    ),
                )
                ...
                v_ok, v_qty, _, v_reason = await self._validate_order_request_contract(
                    ...
                    price=price,
                    filters=filters_obj,
                    use_quote_amount=float(effective_qa),
                )
                ...
                if not v_ok:
                    ...
                    if spendable_dec >= min_req_dec:
                        ...
                        forced_qty = max(
                            Decimal(str(filters_obj.min_qty)),
                    ...
                    if v_reason == "QTY_LT_MIN" or v_reason == "NOTIONAL_LT_MIN_OR_ZERO_QTY":
                        ...
                        target_notional = max(
                            Decimal(str(filters_obj.min_notional)), target_qty * Decimal(str(price))
                        )
                        gap = (target_notional - qa).max(Decimal("0"))

                    self.logger.warning(
                        "[EM] ExecutionProbe = FAIL (Reason: %s, Gap: %.2f). Readiness = FALSE.",
                        v_reason,
                        gap,
                    )
            ...
            _free_q, _ok_rem, _why_rem = await self._get_free_quote_and_remainder_ok(
                quote_asset, gross_needed_f
            )
            if not _ok_rem:
    ...
    async def _attempt_liquidity_healing(
        self, symbol: str, needed_quote: float, context: dict[str, Any]
    ) -> bool:
        ...
        for attempt in range(self.max_liquidity_retries):
            try:
                self.logger.info(
                    f"[Heal] Liquidity attempt {attempt + 1}/{self.max_liquidity_retries} for {symbol}"
                )
                plan = await self.meta_controller.request_liquidity(symbol, needed_quote, context)
                if not plan:
                    continue
                if (
                    hasattr(self.meta_controller, "liquidation_agent")
                    and self.meta_controller.liquidation_agent
                ):
    ...
    async def _maybe_retry_close_with_forced_exit(
        ...
        *,
        ...
        allow_tokens = {tok.strip().lower() for tok in allow_csv.split(",") if str(tok).strip()}
        ...
        cooldown_sec = max(
            0.0, float(self._cfg("CLOSE_ESCAPE_HATCH_RETRY_COOLDOWN_SEC", 180.0) or 180.0)
        )
        now_ts = time.time()
                ...
                if hasattr(self.shared_state, "get_rejection_count"):
                    buy_lock_count = int(
                        self.shared_state.get_rejection_count(
                            symbol, "BUY", "POSITION_ALREADY_OPEN"
                        )
                        or 0
                    )
            except Exception:
        ...
        try:
            if hasattr(self.shared_state, "record_rejection"):
                await self.shared_state.record_rejection(
                    sym, "SELL", "CLOSE_ESCAPE_ATTEMPT", source="ExecutionManager"
                )
        except Exception:
            pass
        ...
        *,
        ...
        if self._detect_phantom_position(sym, pos_qty):
            ...
            if repair_ok:
                ...
                self.logger.warning(
                    "[PHANTOM_REPAIR_FAILED] %s: Repair failed. Position may remain stuck.", sym
                )
        ...
        if isinstance(res, dict):
            try:
                ...
                if (not has_submission_ref) and initial_status in {
                    "BLOCKED",
                    "SKIPPED",
                    "REJECTED",
                    "CANCELED",
                    ...
                    if is_terminal_dust:
                        ...
                        try:
                            ...
                            self.logger.debug(
                                "[EM] terminal dust local retirement failed for %s",
                                sym,
                                exc_info=True,
                            )
                    try:
                ...
                if is_fill:
                    ...
                    if force_finalize:
                        ...
                        self.logger.info(
                            "[EM:CLOSE_FINALIZE] symbol=%s applied=True reason=%s", sym, reason_text
                        )
                    ...
                    try:
                        if self.shared_state:
                            ...
                            await self.shared_state.hydrate_positions_from_balances()
                            self.logger.info(
                                "[EM:HYDRATE] symbol=%s post-fill balance refresh completed", sym
                            )
                    except Exception as e:
                        self.logger.warning(
                ...
                self.logger.error(
                    "[EM:CLOSE_RECONCILE_FAILED] symbol=%s — post-fill may be lost",
                    symbol,
                    exc_info=True,
                )
        return res
    ...
    async def _persist_trade_intent(self, intent: TradeIntent) -> Optional[int]:
        ...
        try:
            from src.l3_portfolio.event_store import Event
    ...
    async def _execute_trade_impl(
        ...
        if (now_ts - self._last_watchdog_report_ts) >= self._watchdog_report_interval_s:
            await self._report_watchdog_status(
                "Operational", f"Processed {self._execution_counter} executions"
            )
        ...
        self.logger.critical(
            f"[EXEC_TRACE_1_ENTRY] sym={sym} side={side.upper()} qty={quantity} planned_quote={planned_quote} conf={confidence} trace_id={trace_id} tag={tag}"
        )
        ...
        if not trace_id and not is_liquidation:
            self.logger.critical(
                f"[EXEC_BLOCK_TRACE_ID] {side.upper()} {sym}: BLOCKED - missing trace_id"
            )
            self.logger.warning(
        ...
        if side == "buy":
            has_tradeability_payload = (
                ...
                or bool(str(policy_ctx.get("tradeability_hint", "") or "").strip())
                or bool(str(policy_ctx.get("tradeability_regime", "") or "").strip())
            )
            if has_tradeability_payload and not bool(policy_ctx.get("tradeability_gate_checked")):
                self.logger.critical(
                    f"[EXEC_BLOCK_TRADEABILITY] {sym}: BLOCKED - missing tradeability_gate_checked flag"
                )
                self.logger.warning(
            ...
            self.logger.critical(
                f"[EXEC_TRACE_3_TRADEABILITY_OK] {sym} has_payload={has_tradeability_payload}"
            )
        ...
        if is_dust_healing_buy:
            policy_ctx["_is_dust_healing_buy"] = True
        is_dust_operation = self._is_dust_operation_context(
            policy_ctx, tier=tier, tag=tag, symbol=sym
        )
        ...
        meta_exit_forced_liq = bool(
            ...
            and any(
                k in reason_hint_u
                for k in ("LIQUIDATION", "EMERGENCY", "CAPITAL_RECOVERY", "DUST_CLEANUP")
            )
        )
            ...
            try:
                ...
                if nav > 0 and position_value > 0:
                    ...
                    if concentration >= 0.85:
                        ...
                        bypass_checks = True
                        is_liq_full = (
                            True  # Force liquidation priority for high concentration exits
                        )
            except Exception as e:
        ...
        if side == "sell":
            ...
            allow_authorized_meta_exit = bool(
                ...
                and any(
                    k in meta_reason_u
                    for k in ("STRATEGY_SELL", "STRATEGY_EXIT", "META_EXIT", "ROTATION")
                )
            )
        ...
        if (
            ...
            and is_real_mode
            and not is_liq_full
            and not bypass_checks
            and not allow_authorized_meta_exit
        ):
            reason_text = " ".join(
                [
                    str(policy_ctx.get("reason") or ""),
            ...
            if not is_emergency:
                self.logger.warning(
                    ...
                    sym,
                    policy_ctx.get("reason") or policy_ctx.get("exit_reason") or "",
                    tag or "",
                )
                return {
                    "ok": False,
        ...
        if not is_liq_full and not bypass_checks:
            ...
            allowed_actions = {str(a).upper() for a in (gov_decision.get("allowed_actions") or [])}
            if mode == "PAUSED":
                self.logger.warning(
                    f"[EM:GovBlock] Blocked {side.upper()} {sym}: System is PAUSED."
                )
                return {
                    "ok": False,
                    "status": "blocked",
                    "reason": "System PAUSED",
                    "error_code": "PAUSED_MODE",
                }
            ...
            if side == "buy" and mode == "PROTECTIVE":
                ...
                if "BUY" not in allowed_actions:
                    self.logger.warning(
                        f"[EM:GovBlock] Blocked BUY {sym}: System is in PROTECTIVE mode."
                    )
                    return {
                        "ok": False,
                        "status": "blocked",
                        "reason": "BUY Disabled in PROTECTIVE Mode",
                        "error_code": "PROTECTIVE_MODE",
                    }
                self.logger.info(
        ...
        self.logger.info(
            f"[EXEC] Request: {sym} {side.upper()} q={quantity} p_quote={planned_quote} tag={clean_tag}{tier_label}{liq_marker}"
        )
        ...
        if tier == "DUST_RECOVERY":
            ...
            self.logger.info(
                f"[EM:DUST_RECOVERY] Set risk_based_quote for {sym} to {planned_quote:.2f}"
            )
        ...
        if is_dust_operation and side == "buy":
            ...
            if new_position_value >= economic_floor:
                ...
                return {
                    "ok": False,
                    "status": "blocked",
                    "reason": "dust_already_healed",
                    "error_code": "DUST_ALREADY_HEALED",
                }
            ...
            if not hasattr(self.shared_state, "dust_operation_symbols") or not isinstance(
                getattr(self.shared_state, "dust_operation_symbols", None), dict
            ):
                self.shared_state.dust_operation_symbols = {}
            self.shared_state.dust_operation_symbols[sym] = time.time()
            self.logger.info(
                f"[EM:DUST_HEALING] Hard-separated sizing for {sym}: exact_deficit={planned_quote:.2f}, new_pos_value={new_position_value:.2f}, bypassing risk engine"
            )
        ...
        if policy_validated and side == "buy" and planned_quote and planned_quote > 0:
            ...
            is_bootstrap_override = (
                policy_ctx.get("_bootstrap_override", False)
                or policy_ctx.get("_bypass_reason") == "BOOTSTRAP_FIRST_TRADE"
            )

            # DEBUG: Log the policy context
            self.logger.info(
                f"[EM:Bootstrap:DEBUG] sym={sym} _bootstrap_override={policy_ctx.get('_bootstrap_override')} _bypass_reason={policy_ctx.get('_bypass_reason')} is_bootstrap={is_bootstrap_override}"
            )
            ...
            if not is_bootstrap_override:
                ...
                self.logger.info(
                    f"[EM:Bootstrap] Allowing downscale for {sym}: bootstrap_override=True"
                )
        ...
        if side == "sell":
            if self.shared_state.exit_in_progress.get(sym, False):
                self.logger.warning(
                    f"[EXEC:EXIT_LOCK] {sym} SELL blocked: exit already in progress"
                )
                return {
                    "ok": False,
                    "status": "blocked",
        ...
        if is_liq_full and side == "sell":
            self.logger.info(
                f"[EXEC:LIQ] LIQUIDATION SELL: {sym} - bypassing all guards (capital, min-notional, throughput)"
            )
            ...
            if not quantity or quantity <= 0:
                ...
                if qty <= 0:
                    ...
                    self._journal(
                        "LIQUIDATION_SKIPPED_NO_POSITION_QTY",
                        {
                            "symbol": sym,
                            "side": "SELL",
                            "status": "SKIPPED",
                    ...
                    await self.shared_state.record_rejection(
                        sym, "SELL", "NO_POSITION_QUANTITY", source="ExecutionManager"
                    )
                    self.shared_state.exit_in_progress[sym] = False
                    return {
                        "ok": False,
            ...
            liq_client_hint = (
                self._build_client_order_id(sym, "SELL", decision_id) if decision_id else None
            )
            ...
            if is_filled:
                ...
                is_bootstrap_sig = (
                    bool(policy_ctx.get("_bootstrap", False)) if policy_ctx else False
                )
                ...
                try:
                    if hasattr(self.shared_state, "clear_rejections"):
                        ...
                        self.logger.info(
                            f"[MemoryOfFailure] ✅ Cleared rejections for {sym} {side} (liquidation success)"
                        )
                except Exception as e:
                ...
                try:
                    if not merged.get("_post_fill_done"):
                        ...
                        merged["_post_fill_result"] = (
                            pf_result if isinstance(pf_result, dict) else {}
                        )
                except Exception:
                ...
                if not self._sell_finalize_already_done(symbol=sym, order=merged):
                    try:
                        ...
                        self.logger.error(
                            "[EM:LIQ_FINALIZE_CRASH] %s: canonical SELL finalization failed",
                            sym,
                            exc_info=True,
                        )
                else:
                ...
                try:
                    ...
                    self.logger.error(
                        f"[ACCOUNTING_AUDIT_CRASH] {sym} sell liquidation audit failed",
                        exc_info=True,
                    )
                ...
                self.logger.info(
                    f"[EXEC:LIQ] ✅ Liquidation SELL executed: {sym} qty={result['executedQty']:.6f}"
                )
                ...
                fail_reason = str(
                    merged.get("reason") or raw.get("reason") or "liquidation_not_filled"
                )
                fail_code = str(
                    merged.get("error_code") or raw.get("error_code") or "LiquidationFailed"
                )
                self.logger.warning(
                ...
                if float(post_fail_qty) <= 0.0:
                    ...
                    with contextlib.suppress(Exception):
                        await self.shared_state.close_position(
                            sym, reason="no_position_after_failed_liquidation"
                        )
                    self._liq_sell_fail_counts.pop(sym, None)
                    self._liq_sell_cooldown_until.pop(sym, None)
                    self.shared_state.exit_in_progress[sym] = False
                ...
                return {
                    ...
                    or raw.get("orderId")
                    or raw.get("order_id"),
                    "client_order_id": merged.get("clientOrderId")
            ...
            try:
                    ...
                    if side == "sell" and unified_sell_authority:
                        ...
                        if await self._check_dust_retirement_before_rejection(sym, side.upper()):
                            await self._log_execution_event(
                                "risk_block",
                                sym,
                                {"side": side, "reason": "permanent_dust_retired"},
                            )
                            ...
                            return {
                                "ok": False,
                                "status": "skipped",
                                "reason": "permanent_dust_retired",
                                "error_code": "DustRetired",
                            }

                        await self._log_execution_event(
                            "risk_block", sym, {"side": side, "reason": risk_reason}
                        )
                        await self.shared_state.record_rejection(
                            sym, side.upper(), "RISK_CAP_EXCEEDED", source="ExecutionManager"
                        )
                        self.logger.info(
                            f"[EXEC_REJECT] symbol={sym} side={side.upper()} reason=RISK_CAP_EXCEEDED count={self.shared_state.get_rejection_count(sym, side.upper())} action=SKIP"
                        )
                        if side == "sell" and self.shared_state:
                            self.shared_state.exit_in_progress[sym] = False
                        return {
                            "ok": False,
                            "status": "skipped" if not strict_risk else "error",
                            "reason": "RiskCapExceeded",
                            "error_code": "RiskCapExceeded",
                        }
                if adjusted:
                    if "quote_amount" in adjusted and planned_quote:
                        adjusted_quote = (
                            float(adjusted["quote_amount"])
                            if adjusted["quote_amount"] is not None
                            else None
                        )
                        if (
                            policy_ctx.get("_no_downscale_planned_quote")
                            and adjusted_quote is not None
                        ):
                            ...
                            planned_quote = min(
                                float(planned_quote), float(adjusted["quote_amount"])
                            )
                ...
                if strict_risk:
                    ...
                    return {
                        "ok": False,
                        "status": "error",
                        "reason": "RiskCheckFailed",
                        "error_code": "RiskCheckFailed",
                    }
        ...
        try:
            ...
            if callable(guard_fn):
                ...
                if (
                    side == "buy"
                    and not any(x in (tag or "") for x in ("liquidation", "tp_sl", "balancer"))
                    and not is_dust_operation
                ):
                    ...
                    if not guard_ok:
                        ...
                        await self._log_execution_event(
                            "profit_target_block",
                            sym,
                            {"side": side, "min_usdt_per_hour": min_target},
                        )
                        await self.shared_state.record_rejection(
                            sym, side.upper(), "PROFIT_TARGET_GUARD", source="ExecutionManager"
                        )
                        self.logger.info(
                            f"[EXEC_REJECT] symbol={sym} side={side.upper()} reason=PROFIT_TARGET_GUARD count={self.shared_state.get_rejection_count(sym, side.upper())} action=SKIP"
                        )
                        return {
                            "ok": False,
                            "status": "skipped",
                            "reason": "ProfitTargetGuard",
                            "error_code": "ProfitTargetGuard",
                        }
        except Exception:
            pass  # non-fatal
        ...
        try:
            ...
            if (
                side != "sell"
                and hasattr(self.shared_state, "is_circuit_breaker_open")
                and await self.shared_state.is_circuit_breaker_open()
            ):
                self.logger.warning(
                    f"[EXEC] 🛑 Circuit Breaker OPEN. Blocking BUY for {sym} (SELL exits always pass)."
                )
                return {
                    "ok": False,
                    "status": "blocked",
            ...
            async with self._small_nav_guard():
                if side == "buy":
                    ...
                    now_buy = time.time()
                    pos_open_block_until = float(
                        self._position_open_buy_block_until.get(sym, 0.0) or 0.0
                    )
                    ...
                    if planned_quote and planned_quote > 0:
                        ...
                        try:
                            ...
                            if classification == "BOT_POSITION":
                                ...
                                try:
                                    ...
                                    _pos_price = float(
                                        pos.get("current_price", pos.get("entry_price", 0.0)) or 0.0
                                    )
                                    _pos_value = (
                                        _pos_qty * _pos_price
                                        if _pos_price > 0.0
                                        else float(pos.get("value_usdt", 0.0) or 0.0)
                                    )
                                    _dust_floor = float(
                                        self._cfg("MIN_POSITION_VALUE_USDT", 15.0) or 15.0
                                    )
                                    if 0.0 < _pos_value < _dust_floor:
                                        ...
                                        cooldown_sec = float(
                                            self._cfg("POSITION_OPEN_BUY_COOLDOWN_SEC", 120.0)
                                            or 120.0
                                        )
                                        if rej_count >= max(1, cooldown_trigger):
                                            self._position_open_buy_block_until[sym] = (
                                    ...
                                    self.logger.debug(
                                        "[EM:DustReentry] Dust check error for %s: %s (falling through)",
                                        sym,
                                        _dust_err,
                                    )
                            ...
                            self.logger.debug(
                                "[EM:Ownership] Classification check error: %s (proceeding with caution)",
                                e,
                            )
                        ...
                        is_bootstrap_now = (
                            bool(policy_ctx.get("bootstrap_mode", False)) if policy_ctx else False
                        )
                        ...
                        if intent and intent.state == "ACCUMULATING":
                            ...
                            if projected_total < intent.min_notional:
                                await self.shared_state.add_to_accumulation(
                                    sym, "BUY", float(planned_quote)
                                )
                                self.logger.info(
                                    f"[EM] P9-Safe Accumulate: {sym} jar {intent.accumulated_quote:.2f} + {planned_quote:.2f} < {intent.min_notional:.2f}"
                                )
                                return {
                                    "ok": True,
                                    "status": "accumulating",
                                    "reason": "P9_SAFE_UNDER_HURDLE",
                                    "executedQty": 0.0,
                                }
                        ...
                        bootstrap_bypass = (
                            policy_ctx.get("bootstrap_bypass", False) if policy_ctx else False
                        )

                        if (
                            clean_tag.startswith("meta/")
                            and not bootstrap_bypass
                            and not is_dust_operation
                        ):
                            agent_id = clean_tag.split("/")[-1]
                            reservations = getattr(
                                self.shared_state, "_authoritative_reservations", {}
                            )
                            ...
                            if agent_budget < float(planned_quote or 0.0) - 0.01:
                                ...
                                if agent_id in reservations:
                                    # 🔒 DUST RETIREMENT CHECK: Don't record rejection for permanent dust
                                    if await self._check_dust_retirement_before_rejection(
                                        sym, "BUY"
                                    ):
                                        self.logger.warning(
                                            f"[EM] {sym} is PERMANENT_DUST, skipping rejection recording"
                                        )
                                        return {
                                            "ok": False,
                                            "status": "skipped",
                                    ...
                                    self.logger.warning(
                                        f"[EM] Reservation Veto: {agent_id} budget {agent_budget:.2f} < planned {planned_quote:.2f}"
                                    )
                                    # P9 MANDATORY: Record rejection (I1 Invariant - Failure Memory)
                                    await self.shared_state.record_rejection(
                                        sym,
                                        "BUY",
                                        "AGENT_RESERVATION_EXCEEDED",
                                        source="ExecutionManager",
                                    )
                                    ...
                                    return {
                                        "ok": False,
                                        "status": "skipped",
                                        "reason": "AGENT_RESERVATION_EXCEEDED",
                                        "error_code": "RESERVATION_LT_QUOTE",
                                    }
                                else:
                                    ...
                                    self.logger.debug(
                                        f"[EM] No reservation record for {agent_id}, proceeding to balance check."
                                    )
                        ...
                        can, gap, reason = await self.can_afford_market_buy(
                            sym, planned_quote, intent_override=intent, policy_context=policy_ctx
                        )
                        ...
                        if not can:
                            if is_dust_operation:
                                await self._log_execution_event(
                                    "order_skip",
                                    sym,
                                    {"side": "buy", "reason": reason, "dust_operation": True},
                                )
                                return {
                                    "ok": False,
                                    "status": "skipped",
                            ...
                            min_required_quote = await self._get_min_entry_quote(
                                sym, policy_context=policy_context
                            )
                            ...
                            if reason in (
                                "QUOTE_LT_MIN_NOTIONAL",
                                "ZERO_QTY_AFTER_ROUNDING",
                                "INSUFFICIENT_QUOTE_FOR_ACCUMULATION",
                            ):
                                # Intent creation/update logic
                                if not intent:
                                    filters = (
                                        await self.exchange_client.ensure_symbol_filters_ready(sym)
                                    )
                                    min_n = self._extract_min_notional(filters)
                                    intent = PendingPositionIntent(
                                        symbol=sym,
                                        side="BUY",
                                        target_quote=float(planned_quote),
                                ...
                                self.logger.info(
                                    f"[EM] Accumulating {planned_quote} for {sym} BUY (Reason: {reason}). Total: {total_acc}"
                                )
                                return {
                                    "ok": True,
                                    "status": "accumulating",
                            ...
                            if reason == "INSUFFICIENT_QUOTE":
                                healed = await self._attempt_liquidity_healing(
                                    sym,
                                    max(float(gap), float(planned_quote)),
                                    {
                                        "reason": reason,
                                        "needed_quote": float(max(gap, 0.0)),
                                        "planned_quote": planned_quote,
                                    },
                                )
                                if not healed:
                                    ...
                                    if self.shared_state and hasattr(
                                        self.shared_state, "report_agent_capital_failure"
                                    ):
                                        self.shared_state.report_agent_capital_failure(
                                            f"exec_fail_{sym}"
                                        )
                                    # P9 MANDATORY: Record rejection (I1 Invariant - Failure Memory)
                                    await self.shared_state.record_rejection(
                                        sym, "BUY", reason, source="ExecutionManager"
                                    )
                                    rej_count = self.shared_state.get_rejection_count(sym, "BUY")
                                    self.logger.info(
                                        f"[EXEC_REJECT] symbol={sym} side=BUY reason=AFFORDABILITY_CHECK_FAILED count={rej_count} action=RETRY"
                                    ...
                                    await self._log_execution_event(
                                        "order_skip", sym, {"side": "buy", "reason": reason}
                                    )
                                    return {
                                        "ok": False,
                                        "status": "skipped",
                        ...
                        execute_quote = (
                            float(gap) if reason == "OK_DOWNSCALED" else float(planned_quote)
                        )
                        ...
                        if (
                            intent
                            and intent.accumulated_quote > 0
                            and intent.state == "ACCUMULATING"
                        ):
                            # Condition B: Market Validity
                            if not self.shared_state.is_intent_valid(sym, "BUY"):
                                self.logger.info(
                                    f"[EM] Intent for {sym} BUY no longer valid. Clearing bucket."
                                )
                            ...
                            # Atomic claim
                                ...
                                self.logger.info(
                                    f"[EM] Pending intent READY -> executing {sym} BUY with total {execute_quote}"
                                )
                                await self.shared_state.clear_pending_intent(sym, "BUY")
                            else:
                                ...
                                min_n = self._extract_min_notional(filters)
                                await self.shared_state.add_to_accumulation(
                                    sym, "BUY", execute_quote
                                )
                                return {
                                    "ok": True,
                                    "status": "accumulating",
                                    "reason": "intent_claimed_restarting",
                                }
                        ...
                        if not guard_allowed:
                            ...
                            await self.shared_state.record_rejection(
                                sym, "BUY", guard_reason, source="ExecutionManager"
                            )
                            return {
                                "ok": False,
                                "status": "skipped",
                                "reason": guard_reason,
                                "error_code": "ENTRY_FLOOR_GUARD",
                            }
                        ...
                        if filled_qty > 0:
                            avg_price = (
                                float(raw.get("cummulativeQuoteQty", 0.0) or 0.0) / filled_qty
                            )
                        ...
                        if not quantity or quantity <= 0:
                            await self._log_execution_event(
                                "order_skip", sym, {"side": "buy", "reason": "InvalidQuantity"}
                            )
                            return {
                                "ok": False,
                                "status": "skipped",
                                "reason": "zero_or_negative_quantity",
                                "error_code": "InvalidQuantity",
                            }
                        ...
                        if raw and isinstance(raw, dict):
                            ...
                            if filled_qty > 0:
                                avg_price = float(
                                    raw.get("avgPrice", 0.0) or raw.get("price", 0.0) or 0.0
                                )
                                if avg_price <= 0:
                                    # Calculate avg price from cummulativeQuoteQty if not provided
                                    cummulative_quote = float(
                                        raw.get("cummulativeQuoteQty", 0.0) or 0.0
                                    )
                    ...
                    if not policy_validated:
                        is_cold = (
                            self.shared_state
                            and hasattr(self.shared_state, "is_cold_bootstrap")
                            and self.shared_state.is_cold_bootstrap()
                        )
                        is_liquidation = any(
                            x in tag_lower for x in ("liquidation", "tp_sl", "balancer")
                        )
                        ...
                        if is_cold and not is_liquidation:
                            ...
                            await self.shared_state.record_rejection(
                                sym, "SELL", "COLD_BOOTSTRAP_BLOCK", source="ExecutionManager"
                            )
                            rej_count = self.shared_state.get_rejection_count(sym, "SELL")
                            self.logger.info(
                                f"[EXEC_REJECT] symbol={sym} side=SELL reason=COLD_BOOTSTRAP_BLOCK count={rej_count} action=RETRY"
                            )
                            await self._log_execution_event(
                                "sell_block_cold_bootstrap", sym, {"reason": "cold_bootstrap"}
                            )
                            return {
                                "ok": False,
                                "status": "blocked",
                                "reason": "cold_bootstrap_no_sell",
                                "error_code": "ColdBootstrap",
                            }
                    ...
                    if planned_quote and planned_quote > 0 and (not quantity or quantity <= 0):
                        ...
                        if qty <= 0:
                            ...
                            if qty <= 0:
                                ...
                                rej_count = self.shared_state.get_rejection_count(sym, "SELL")
                                self.logger.info(
                                    f"[EXEC_REJECT] symbol={sym} side=SELL reason=NO_POSITION_QUANTITY count={rej_count} action=SKIP"
                                )
                                await self._log_execution_event(
                                    "sell_block_no_qty", sym, {"reason": "no_position_quantity"}
                                )
                                return {
                                    "ok": False,
                                    "status": "skipped",
                    ...
                    if await self._check_dust_retirement_before_rejection(sym, side.upper()):
                        await self._log_execution_event(
                            "order_skip", sym, {"side": side, "reason": "permanent_dust_retired"}
                        )
                        return {
                            "ok": False,
                            "status": "skipped",
                    ...
                    # P9 MANDATORY: Record rejection (I1 Invariant - Failure Memory)
                    await self.shared_state.record_rejection(
                        sym, side.upper(), "INVALID_SIDE", source="ExecutionManager"
                    )
                    rej_count = self.shared_state.get_rejection_count(sym, side.upper())
                    self.logger.info(
                        f"[EXEC_REJECT] symbol={sym} side={side.upper()} reason=INVALID_SIDE count={rej_count} action=SKIP"
                    )
                    await self._log_execution_event(
                        "order_skip", sym, {"side": side, "reason": "invalid_side"}
                    )
                    return {
                        "ok": False,
                        "status": "skipped",
                        "reason": "invalid_side",
                        "error_code": "InvalidSide",
                    }
            ...
            if isinstance(raw, dict):
                ...
                if str(raw.get("status") or "").upper() == "SKIPPED" and skip_reason in (
                    "IDEMPOTENT",
                    "ACTIVE_ORDER",
                ):
                    return {
            ...
            if is_filled:
                ...
                try:
                    await self.shared_state.sync_authoritative_balance(force=True)
                    self.logger.info(
                        f"[StateSync:PostFill] ✅ Refreshed balances after {sym} {side} fill"
                    )
                except Exception as e:
                ...
                try:
                    post_fill = await self._ensure_post_fill_handled(
                        sym, side, raw, tier=tier, tag=tag_raw
                    )
                except Exception as e:
                ...
                if side == "buy":
                    try:
                        ...
                        try:
                            if hasattr(self.shared_state, "get_significant_position_floor"):
                                significant_floor_usdt = float(
                                    await maybe_call(
                                        self.shared_state, "get_significant_position_floor", sym
                                    )
                                    or 0.0
                                )
                        except Exception:
                        ...
                        self.logger.debug(
                            "[EM] open_position finalize failed for %s", sym, exc_info=True
                        )
                ...
                try:
                    ...
                    self.logger.error(
                        "[ACCOUNTING_AUDIT_CRASH] %s %s audit failed", sym, side, exc_info=True
                    )
                    raise
                ...
                if tier:
                    ...
                    self.logger.info(
                        f"[EXEC] ✅ Tier-{tier} filled: {sym} {side.upper()} | "
                ...
                self.logger.warning(
                    f"[EXEC] ⚠️ Invariant Check Failed: status={status}, execQty={exec_qty}. Skipping State Mutation."
                )
                await self._emit_status("Warning", f"skipped_mutation {sym} {side} status={status}")
                # GAP #2 FIX: Trigger pruning on failure
                await self._on_order_failed(
                    sym, side, raw.get("reason") or "NOT_FILLED", planned_quote
                )
                return {
                    "ok": False,
                    "status": status.lower(),
            ...
            self.logger.error(
                f"[EM:SELL_EXCEPTION] symbol={sym} side={side} exception_type={type(e).__name__} error_type={error_type} message={error_msg}",
                exc_info=True,
            )
            await self._log_execution_event(
                "order_exception",
    ...
    async def execute_liquidation_plan(self, exits: list[dict]) -> bool:
        def _coalesce(exits_list: list[dict]) -> list[dict]:
            ...
            for ex in exits_list or []:
                ...
                entry = grouped.setdefault(
                    sym, {"symbol": sym, "quantity": 0.0, "tags": set(), "raw": []}
                )
                entry["quantity"] += qty
                entry["tags"].add(tag)
                entry["raw"].append(ex)
        ...
        for ex in exits:
            try:
                ...
                policy_ctx = (
                    ex.get("policy_ctx") if isinstance(ex, dict) and ex.get("policy_ctx") else {}
                )
                if qty <= 0:
                    continue
                await self._log_execution_event(
                ...
                try:
                    ...
                    oco_orders = [
                        ...
                        if o.get("type") in ("STOP_LOSS_LIMIT", "LIMIT_MAKER")
                    ]
                    if oco_orders:
                        self.logger.info(
                    ...
                    self.logger.debug(
                        "[ExecutionManager] OPTION_A: OCO pre-cancel check failed (benign): %s",
                        oco_check_err,
                    )
                ...
                status = (
                    str((merged or {}).get("status", "")).upper()
                    if isinstance(merged, dict)
                    else ""
                )
                exec_qty = self._safe_float(
                    (merged or {}).get("executedQty") if isinstance(merged, dict) else 0.0, 0.0
                )
                ok_field = bool((merged or {}).get("ok")) if isinstance(merged, dict) else False
                is_filled = bool(
                    ok_field or (status in ("FILLED", "PARTIALLY_FILLED") and exec_qty > 0.0)
                )
                await self._log_execution_event(
                    ...
                    sym,
                    {
                        ...
                        "executed_qty": float(exec_qty),
                        "order_id": (merged or {}).get("orderId")
                        if isinstance(merged, dict)
                        else None,
                        "exchange_order_id": (merged or {}).get("exchange_order_id")
                        if isinstance(merged, dict)
                        else None,
                        "client_order_id": (merged or {}).get("clientOrderId")
                        if isinstance(merged, dict)
                        else None,
                        "error_code": (merged or {}).get("error_code")
                        if isinstance(merged, dict)
                        else None,
                        "reason": (merged or {}).get("reason")
                        if isinstance(merged, dict)
                        else "no_raw_response",
                        "tag": tag,
                    },
                )
        ...
        try:
            ...
            while True:
                try:
                    ...
                    if violations:
                        self.logger.warning(
                            "[EM:PosSyncMonitor] Invariant violations on: %s", ", ".join(violations)
                        )
    ...
    async def _place_market_order_qty(
        ...
        if not current_price:
            await self._log_execution_event(
                "order_skip", symbol, {"side": side.upper(), "reason": "no_price"}
            )
        ...
        if qty <= 0:
            await self._log_execution_event(
                "order_skip", symbol, {"side": side.upper(), "reason": "qty_invalid"}
            )
            return self._canonical_exec_result(
                symbol=symbol,
                side=side,
            ...
            if not ok_aff:
                if "INSUFFICIENT_QUOTE" in why or "QUOTE_LT_MIN_NOTIONAL" in why:
                    need_quote = (
                        float(qty)
                        * float(current_price)
                        * float(1.0 + self.trade_fee_pct)
                        * float(self.safety_headroom)
                    )
                    heal_context = {
                        "reason": "affordability_gate",
                        "symbol": symbol,
                        "needed_quote": need_quote,
                        "current_price": current_price,
                    }
                    healed = await self._attempt_liquidity_healing(symbol, need_quote, heal_context)
                    if not healed:
                        await self._log_execution_event(
                            "order_skip", symbol, {"side": side.upper(), "reason": str(why)}
                        )
                        return self._canonical_exec_result(
                            symbol=symbol,
                            side=side,
        ...
        if side.upper() == "BUY":
            try:
                reservation_id = await self.shared_state.reserve_liquidity(
                    quote_asset, float(planned_quote), ttl_seconds=30
                )
                await self._log_execution_event(
                    "liquidity_reserved",
                    symbol,
        ...
        try:
            ...
            if not has_submission_ref and not is_filled:
                ...
                await self._log_execution_event(
                    "order_skip", symbol, {"side": side.upper(), "reason": "order_not_placed"}
                )
                return self._canonical_exec_result(
                    symbol=symbol,
                    side=side,
            ...
            if is_filled:
                position_updated = await self._update_position_from_fill(
                    symbol=symbol, side=side, order=order_res, tag=str(tag or "")
                )
                if not position_updated:
                    self.logger.warning("[PHASE4_SKIPPED] Position not updated for %s", symbol)
            else:
                self.logger.info(
    ...
    async def _place_market_order_quote(
        ...
        is_bootstrap = bool(
            bypass_min_notional
            or (tag and "bootstrap" in str(tag).lower())
            or (decision_id and "bootstrap" in str(decision_id).lower())
        )
        ...
        if is_bootstrap and side.upper() == "BUY" and quote > 0:
            try:
                ...
                try:
                    if hasattr(self.exchange_client, "get_symbol_info"):
                        ...
                        if info and isinstance(info, dict):
                            ...
                            for f in filters:
                                if (
                                    f.get("filterType") == "MIN_NOTIONAL"
                                    or f.get("filterType") == "NOTIONAL"
                                ):
                                    min_notional = float(
                                        f.get("minNotional", f.get("minValue", 10.0))
                                    )
                                    break
                except Exception:
                ...
                self.logger.warning(
                    "[EM:BOOTSTRAP_ALLOC] Error enforcing minimum allocation: %s", e
                )
        ...
        if side.upper() == "BUY" and not policy_validated:
            ...
            if not ok_aff:
                if "INSUFFICIENT_QUOTE" in why or "QUOTE_LT_MIN_NOTIONAL" in why:
                    need_quote = (
                        float(quote) * float(1.0 + self.trade_fee_pct) * float(self.safety_headroom)
                    )
                    heal_context = {
                        "reason": "affordability_gate",
                        "symbol": symbol,
        ...
        if side.upper() == "BUY":
            try:
                reservation_id = await self.shared_state.reserve_liquidity(
                    quote_asset, float(quote), ttl_seconds=30
                )
                await self._log_execution_event(
                    "liquidity_reserved",
                    symbol,
                    ...
                    self._journal(
                        "ORDER_FILLED",
                        {
                            ...
                            "avg_price": self._resolve_post_fill_price(
                                order_res, float(order_res.get("executedQty", 0.0) or 0.0)
                            ),
                            "cumm_quote": float(
                                order_res.get("cummulativeQuoteQty", quote) or quote
                            ),
    ...
    async def _decide_execution_method(
        ...
        try:
            (
                ...
                spread_pct,
                quality_reason,
            ) = await self.maker_executor.evaluate_spread_quality(
                symbol=symbol,
                side=side,
                current_price=current_price,
            )
            ...
            self.logger.debug(
                f"[MakerExec] evaluate_spread_quality failed: {e}; falling back to market"
            )
            return False, f"spread_eval_error({e!s})"
    ...
    async def _place_market_order_core(
        ...
        decision_id = decision_id or self._resolve_decision_id(
            getattr(self, "_current_policy_context", None)
        )
        ...
        if hasattr(self, "_current_policy_context") and self._current_policy_context:
            ...
            is_bootstrap_signal = is_bootstrap_signal or bool(
                self._current_policy_context.get("_bootstrap_override", False)
            )
        ...
        if not allow_bootstrap_bypass:
            if self._is_duplicate_client_order_id(client_id):
                self.logger.debug(
                    "[EM] Duplicate client_order_id for %s %s; skipping.", symbol, side.upper()
                )
        ...
        try:
            ...
            if side.upper() == "BUY" and planned_quote and planned_quote > 0:
                ...
                min_entry = await self._get_min_entry_quote(
                    symbol, price=current_price, min_notional=min_notional
                )
                ...
                if is_bootstrap or bypass_min_notional:
                    ...
                    _free_bootstrap, _ok_bootstrap, _ = await self._get_free_quote_and_remainder_ok(
                        q_asset_bootstrap, spend
                    )
                ...
                effective_min_entry = await self._get_min_entry_quote(
                    symbol, price=current_price, min_notional=min_notional
                )
                ...
                if was_downscaled_for_affordability:
                    ...
                    self.logger.info(
                        ...
                        float(spend),
                        float(min_entry_for_validation),
                        float(effective_min_entry),
                    )
                else:
                ...
                self.logger.info(
                    ...
                    was_downscaled_for_affordability,
                )
                ...
                is_bootstrap_override = bool(
                    (getattr(self, "_current_policy_context", None) or {}).get(
                        "_bootstrap_override", False
                    )
                )
                if (
                    spend < min_entry_after_rounding
                    and not (is_bootstrap or bypass_min_notional or is_bootstrap_override)
                    and not was_downscaled_for_affordability
                ):
                    self.logger.info(
                ...
                if hasattr(self, "_current_policy_context") and self._current_policy_context:
                    no_downscale = bool(
                        self._current_policy_context.get("_no_downscale_planned_quote", False)
                    )
                    is_bootstrap_escalation = bool(
                        self._current_policy_context.get("_bootstrap_override", False)
                    )
                ...
                if was_downscaled_for_affordability:
                    ...
                    _free_final, _ok_final, _ = await self._get_free_quote_and_remainder_ok(
                        q_asset_check, spend
                    )
                ...
                if spend < min_required_gross and not no_downscale and not is_bootstrap_escalation:
                    ...
                    (
                        ...
                        _ok_rem_esc,
                        _why_rem_esc,
                    ) = await self._get_free_quote_and_remainder_ok(
                ...
                _free, _ok_rem, _why_rem = await self._get_free_quote_and_remainder_ok(
                    q_asset, gross_needed
                )
                if not _ok_rem:
                    await self._log_execution_event(
                        "order_skip",
                ...
                if is_bootstrap:
                    ...
                    try:
                        ...
                        self.logger.warning(
                            f"[EM:BOOTSTRAP] Quote-based order failed: {e}, falling back to quantity-based"
                        )
                ...
                if self._is_order_fill_confirmed(order):
                    self._journal(
                        "ORDER_FILLED",
                        {
                            ...
                            "side": "BUY",
                            "executed_qty": self._safe_float(
                                order.get("executedQty") or order.get("executed_qty"), 0.0
                            ),
                            "avg_price": self._resolve_post_fill_price(
                                order,
                                self._safe_float(
            ...
            if side.upper() == "SELL" and step_size > 0 and not is_heal_c:
                ...
                qty_residual_is_dust = remainder > 0 and remainder < max(
                    float(min_qty), float(step_size)
                )
                ...
                notional_residual_is_dust = (
                    ...
                    < dust_floor_quote  # ✅ KEY FIX: Use dust_floor, not residual_floor
                )

                # 3. Position percentage: if selling 90%+ of position, clean exit by selling 100%
                position_pct_remaining = (
                    (remainder / _raw_quantity * 100) if _raw_quantity > 0 else 0
                )
                near_total_exit = (
                    position_pct_remaining > 0 and position_pct_remaining < 5.0
                )  # < 5% remaining
            ...
            if (
                not is_liquidation
                and not bypass_min_notional
                and (qty <= 0 or qty * current_price < min_required_notional)
            ):
                await self._log_execution_event(
                    "order_skip",
                    symbol,
                    {
                        "side": side.upper(),
                        "reason": "NOTIONAL_LT_MIN_PRE_VALIDATION",
                        "notional": qty * current_price,
                        "min_required": min_required_notional,
                    },
                )
                return None
            elif qty <= 0:
            ...
            filters_obj = SymbolFilters(
                ...
                tick_size=tick_size,
                min_notional=min_notional,
                min_entry_quote=float(exit_floor or 0.0),
            )
            ...
            if is_liquidation:
                ...
                ok, adj_qty, _, _ = await self._validate_order_request_contract(
                    ...
                    price=current_price,
                    filters=filters_obj,
                    taker_fee_bps=self._cfg("TAKER_FEE_BPS", 10),
                    use_quote_amount=None,
                )
                if not ok:
                    self.logger.critical(
                        f"[EXEC_BLOCK_CONTRACT_VALIDATION] {symbol}: BLOCKED - order contract validation failed qty={qty} adj_qty={adj_qty}"
                    )
                    await self._log_execution_event(
                        "order_skip",
                        symbol,
                        {"side": side.upper(), "reason": "order_filters_rejected"},
                    )
                    return None
                self.logger.critical(
                    f"[EXEC_TRACE_4a_CONTRACT_OK] {symbol} qty={qty} adj_qty={adj_qty}"
                )
            ...
            self.logger.critical(
                f"[EXEC_TRACE_4_AMOUNT] {symbol} {side.upper()} final_qty={final_qty:.8f} notional={notional:.8f} min_notional={min_notional:.8f}"
            )
            ...
            if final_qty <= 0 or notional <= 0:
                ...
                try:
                    _free, _ok_rem, _why_rem = await self._get_free_quote_and_remainder_ok(
                        q_asset, float(planned_quote or 0.0)
                    )
                except Exception:
                    _free, _ok_rem, _why_rem = 0.0, False, "error_fetching_free"

                self.logger.critical(
                    f"[EXEC_BLOCK_ZERO_AMOUNT] {symbol}: BLOCKED - final_qty={final_qty:.8f} notional={notional:.8f} planned_quote={planned_quote} free_quote={_free:.8f}"
                )
            ...
            if side.upper() == "BUY":
                ...
                if notional_now < min_req:
                    await self._log_execution_event(
                        ...
                        {
                            "side": "BUY",
                            "reason": "NOTIONAL_LT_MIN_AFTER_ROUND",
                            "rounded_qty": final_qty,
                ...
                q_asset = self._split_symbol_quote(symbol)
                gross_needed = (
                    float(final_qty * current_price)
                    * (1.0 + float(self.trade_fee_pct))
                    * float(self.safety_headroom)
                )
                _free, _ok_rem, _why_rem = await self._get_free_quote_and_remainder_ok(
                    q_asset, gross_needed
                ...
                if not is_liquidation and not await self._passes_profit_gate(
                    symbol, side, final_qty, current_price
                ):
                    self.logger.warning(
                        f"🚫 SELL blocked at Execution layer by profit gate for {symbol}"
                    )
                    return None
    ...
    async def _place_with_client_id(self, **kwargs) -> Any:
        ...
        if request_kwargs.get("quote_order_qty"):
            self.logger.info(
                ...
                symbol,
                side,
                float(request_kwargs.get("quote_order_qty", 0)),
            )
            ...
            self.logger.info(
                ...
                symbol,
                side,
                float(request_kwargs.get("quantity", 0)),
            )
        ...
        try:
            try:
                ...
                return self._classify_exchange_error(symbol, side, te, "place_market_order")
                ...
                return self._classify_exchange_error(symbol, side, bex, "place_market_order")
    ...
    def _classify_exchange_error(
        ...
        if isinstance(exc, BinanceAPIException):
            ...
            self.logger.error(f"[EM:Classify] {symbol} {side}: TYPE_ERROR in {context}: {exc}")
            return None
    ...
    def reset_idempotent_cache(self):
        ...
        try:
            ...
            self.logger.warning(
                "[EXEC:IDEMPOTENT_RESET] Failed to reset idempotent cache: %s", e, exc_info=True
            )
