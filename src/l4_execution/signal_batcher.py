...
class SignalBatcher:
    ...
    def __init__(
        ...
        self.max_batch_age_sec = (
            30.0  # 🔧 SAFETY: Force-flush after 30s even if micro-NAV threshold not met
        )
    ...
    async def _update_micro_nav_mode(self) -> None:
        ...
        if self._micro_nav_mode_active:
            self.logger.debug(
                "[Batcher:MicroNAV] Micro-NAV mode ACTIVE (NAV=%.2f) → accumulating signals", nav
            )
    ...
    async def _check_micro_nav_threshold(self) -> tuple[bool, float]:
        ...
        total_quote = sum(
            float(sig.extra.get("planned_quote", 10.0) or 10.0) for sig in self._pending_signals
        )
    ...
    def should_flush(self) -> bool:
        ...
        has_critical = any(
            sig.side in ("SELL", "LIQUIDATION")
            or sig.extra.get("_forced_exit")
            or sig.extra.get("_is_rotation")
        ...
        should_flush = (
            elapsed >= self.batch_window_sec
            or len(self._pending_signals) >= self.max_batch_size
            or elapsed >= self.max_batch_age_sec  # 🔧 SAFETY: Max age timeout
            or (has_critical and len(self._pending_signals) >= 1)
        )
    ...
    async def flush(self) -> list[BatchedSignal]:
        ...
        try:
            ...
            if (
                self._micro_nav_mode_active and not batch_too_old
            ...
            self.logger.debug(
                "[Batcher:MicroNAV] Micro-NAV check failed: %s, continuing with flush", e
            )
