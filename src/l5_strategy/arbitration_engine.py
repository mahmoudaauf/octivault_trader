    ...
    async def evaluate_signal(
        ...
        for gate_name in self.gate_order:
            ...
            if not result.passed:
                self.logger.warning(
                    f"Signal rejected at gate '{result.gate_name}': " f"{result.reason}"
                )
                return (False, results)

        # All gates passed
        self.logger.info(
            f"Signal approved: {signal.get('symbol')} " f"from {signal.get('source_agent')}"
        )
        return (True, results)
    ...
    async def _gate_confidence_threshold(self, signal: dict[str, Any]) -> GateResult:
        ...
        if confidence < min_confidence:
            return GateResult(
                ...
                passed=False,
                reason=(f"Confidence {confidence:.2f} below threshold " f"{min_confidence:.2f}"),
                severity="warning",
    ...
    async def _gate_position_limit_gate(self, signal: dict[str, Any]) -> GateResult:
        ...
        if current_positions >= max_positions:
            return GateResult(
                ...
                passed=False,
                reason=(f"Position limit reached: " f"{current_positions}/{max_positions}"),
                severity="warning",
                details={
