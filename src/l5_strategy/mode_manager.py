...
class ModeManager:
    def __init__(self, logger, config):
        ...
        if _startup_override in ("RECOVERY", "NORMAL", "BOOTSTRAP", "AGGRESSIVE"):
            ...
            logger.info(
                "[ModeManager:Init] STARTUP_MODE_OVERRIDE applied: starting in %s",
                _startup_override,
            )
        else:
    ...
    def _resolve_mode_universe_limit(self, mode: str) -> int:
        ...
        if str(mode).upper() == "BOOTSTRAP":
            bootstrap_limit = int(
                getattr(self.config, "BOOTSTRAP_UNIVERSE_SYMBOLS", self._bootstrap_symbol_limit)
                or self._bootstrap_symbol_limit
            )
    ...
    async def evaluate_state_machine(self, metrics: dict[str, Any]):
        ...
        if is_flat and (metrics.get("is_restart") or prolonged_idle):
            if current != "BOOTSTRAP":
                self.logger.warning(
                    "[ModeManager:SOP] 🟦 Triggering BOOTSTRAP: portfolio_flat=True, startup=%s, idle=%ds",
                    metrics.get("is_restart"),
                    int(idle_time),
                )
        ...
        if metrics.get("circuit_breaker_open", False):
            if current != "SAFE":
                self.logger.warning(
                    "[EmergencySOP] 🛑 Circuit Breaker OPEN -> Switching to SAFE immediately."
                )
        ...
        if metrics.get("manual_pause", False) or "paused" in status:
            if current != "PAUSED":
                self.logger.warning(
                    "[SOP:Authority] ⏸️ Manual Pause Detected -> Switching to PAUSED mode."
                )
        ...
        if "frozen" in status or metrics.get("manual_freeze", False):
            if current != "SAFE":
                self.logger.warning(
                    "[EmergencySOP] 🛑 Manual Freeze Detected -> Switching to SAFE immediately."
                )
        ...
        if metrics.get("integrity_error", False):
            if current != "SAFE":
                self.logger.warning(
                    "[EmergencySOP] 🛑 Integrity/Reservation Corruption -> Switching to SAFE immediately."
                )
        ...
        if metrics.get("repeated_failures", False):
            if current not in ("SAFE", "PROTECTIVE"):
                if current == "RECOVERY":
                    ...
                    persist_sec = float(
                        getattr(
                            self.config, "RECOVERY_TO_PROTECTIVE_FAILURE_PERSISTENCE_SEC", 120.0
                        )
                        or 120.0
                    )
                    if self._check_condition_persistence(
                        "recovery_to_protective_failures", now, persist_sec
                    ):
                        self.logger.warning(
                    ...
                    return
                self.logger.warning(
                    "[EmergencySOP] ⚠️ Repeated Outcomes/HYG Failures -> Switching to PROTECTIVE immediately."
                )
        ...
        if not health_ok or forced_liq:
            if current not in ("RECOVERY", "BOOTSTRAP", "SAFE", "PAUSED"):
                self.logger.warning(
                    "[ModeManager:SOP] 🟨 Triggering RECOVERY: health_ok=%s, forced_liq=%s, status=%s",
                    health_ok,
                    forced_liq,
                    status,
                )
        ...
        if current in ("NORMAL", "AGGRESSIVE"):
            ...
            if metrics.get("drawdown_pct", 0.0) >= protective_dd_limit:
                self.logger.warning(
                    "[ModeManager:SOP] 🛡️ Drawdown Breach -> Switching to PROTECTIVE (%.2f%%)",
        ...
        if current in ("PROTECTIVE", "NORMAL", "AGGRESSIVE"):
            ...
            if metrics.get("drawdown_pct", 0.0) >= hard_dd_limit:
                self.logger.warning(
                    "[ModeManager:SOP] 🛑 Hard Drawdown Breach -> Switching to SAFE (%.2f%%)",
                    metrics.get("drawdown_pct"),
                )
        ...
        if current == "NORMAL":
            ...
            if rr_low and dd_low:
                if self._check_condition_persistence("normal_to_aggressive", now, 1800):
                    self.logger.info(
                        "[ModeManager:SOP] NORMAL -> AGGRESSIVE: Low run-rate and drawdown persistence met."
                    )
        ...
        if current == "AGGRESSIVE":
            ...
            if rr_ok:
                if self._check_condition_persistence("aggressive_to_normal", now, 1200):
                    self.logger.info(
                        "[ModeManager:SOP] AGGRESSIVE -> NORMAL: Target run-rate recovery persistence met."
                    )
        ...
        if current == "NORMAL":
            ...
            if vol_high or risk_flags:
                ...
                if self._check_condition_persistence("normal_to_protective", now, 300):
                    self.logger.info(
                        "[ModeManager:SOP] NORMAL -> PROTECTIVE: Volatility/Risk flags persistence met."
                    )
        ...
        if current == "RECOVERY":
            ...
            if health_ok and first_trades:
                ...
                if self._check_condition_persistence("recovery_to_normal", now, stabilization_sec):
                    self.logger.info(
                        "[ModeManager:SOP] RECOVERY -> NORMAL: Health stabilization met."
                    )
        ...
        if current == "PROTECTIVE":
            ...
            if health_ok and dd_stable:
                ...
                if self._check_condition_persistence("protective_to_recovery", now, 60):
                    self.logger.info(
                        "[ModeManager:SOP] 🛡️ -> 🟨 PROTECTIVE -> RECOVERY: Health and PnL stabilized."
                    )
        ...
        if current == "BOOTSTRAP":
            if metrics.get("first_trade_executed", False) or metrics.get("has_positions", False):
                self.logger.info(
                    "[ModeManager:SOP] BOOTSTRAP -> NORMAL: First trade or position detected."
                )
    ...
    def set_mode(self, mode: str, force: bool = False, reason: str = "") -> bool:
        ...
        if mode != self._current_mode:
            now = time.time()
            if not force:
                min_mode_duration_sec = max(
                    0.0, float(self._mode_config.get("min_mode_duration_sec", 0.0) or 0.0)
                )
            ...
            self.logger.info(
                f"[ModeManager] Mode changed from {old_mode} to {mode} | Objective: {self._SOP_MATRIX[mode]['objective']}"
            )
    ...
    def transition_from_bootstrap(self):
        ...
        if self._current_mode == "BOOTSTRAP":
            ...
            self.logger.info(
                f"[ModeManager] Bootstrap transition complete. Active symbol limit: {self._active_symbol_limit}"
            )
