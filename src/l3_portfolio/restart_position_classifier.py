...
class RestartPositionClassifier:
    ...
    def __init__(self, shared_state: Any, config: Any, exchange_client: Optional[Any] = None):
        ...
        self.lookback_days = int(getattr(config, "RESTART_HISTORY_LOOKBACK_DAYS", 30))
        self.dust_threshold_usdt = float(getattr(config, "DUST_THRESHOLD_USDT", 5.0))
        self.quote_asset = str(getattr(config, "DEFAULT_QUOTE_CURRENCY", "USDT")).upper()
    ...
    async def classify_all_positions(self) -> dict[str, dict[str, Any]]:
        ...
        try:
            ...
            self.logger.info(
                f"Found {len(all_symbols)} symbols to classify "
            ...
            self.logger.debug(
                f"In-memory trade history: {len(trade_history)} entries (may be 0 after restart)"
            )
    ...
    async def _load_durable_signals(self) -> None:
        ...
        try:
            ...
            if db and hasattr(db, "load_open_positions"):
                ...
                self._db_bot_symbols = {
                    str(r.get("symbol", "")).upper() for r in rows if r.get("symbol")
                }
                self.logger.info(
                    f"[Classifier] DB bot symbols: {sorted(self._db_bot_symbols) or '(none)'}"
                )
        except Exception as e:
        ...
        try:
            ...
            if bm:
                self._ever_traded = (
                    bm.get_total_trades_executed() > 0 or bm.get_first_trade_at() is not None
                )
            else:
        ...
        try:
            self._universe = {str(s).upper() for s in (self.ss.accepted_symbols or {}).keys() if s}
        except Exception as e:
    ...
    async def _classify_symbol(
        self, symbol: str, trade_history: list[dict[str, Any]]
    ) -> dict[str, Any]:
        """
    ...
    def _apply_rules(self, symbol: str, signals: dict[str, Any]) -> tuple[str, float, str]:
        """
        ...
        if in_wallet and self._ever_traded:
            ...
            return (
                "EXTERNAL_POSITION",
                0.75,
                "In wallet, outside universe, system has traded before — " "likely a user deposit",
            )

        # Rule 9: In positions but no other signals
        if in_positions:
            return ("RECOVERY", 0.55, "Found in positions dict with no supporting signals")
            ...
            for symbol, result in self.classifications.items():
                try:
                    ...
                    await self.ss.register_position_classified(
                        ...
                        if classification_str in ("EXTERNAL_POSITION", "STABLE")
                        else "ACTIVE",
                    )
                    registered += 1
                    self.logger.debug(
                    ...
                    self.logger.error(f"Failed to register {symbol}: {e}", exc_info=True)

            self.logger.info(f"Registered {registered} positions in SharedState")

        except Exception as e:
            self.logger.error(f"Error registering classifications: {e}", exc_info=True)
    ...
    def _log_summary(self) -> None:
        ...
        if uncertain_count > 0:
            ...
            for symbol in sorted(self.uncertain_symbols):
                ...
                self.logger.warning(
                    f"    {symbol}: {result['reason']} " f"(confidence: {result['confidence']:.2%})"
                )
    ...
    def get_uncertain_symbols(self) -> list[tuple[str, float, str]]:
        ...
        for symbol in sorted(self.uncertain_symbols):
            ...
            result.append((symbol, class_result["confidence"], class_result["reason"]))
        return result
