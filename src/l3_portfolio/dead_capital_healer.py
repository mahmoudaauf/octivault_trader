...
class DeadCapitalHealer:
    ...
    def __init__(self, config: Optional[dict] = None):
        ...
        """
        from src.l3_portfolio.portfolio_buckets import (
            PortfolioBucketState,  # Import for adaptive thresholds
        )
        ...
        # Healing thresholds
        self.min_dead_to_heal = (
            self.config.get("min_dead_to_heal") or thresholds["min_dead_to_heal"]
        )
        ...
        self.max_liquidations_per_cycle = self.config.get(
            "max_liquidations", 10
        )  # Max 10 at a time
    ...
    def identify_liquidation_candidates(
        ...
        logger.debug("🔍 Searching for liquidation candidates...")
        ...
        for symbol in priority_order[: self.max_liquidations_per_cycle]:
            ...
            logger.debug(f"   ✅ Candidate: {symbol:10s} | ${value:>8.2f} | Reason: {reason}")
    ...
    def execute_liquidation_batch(
        ...
        for order in orders:
            ...
            try:
                ...
                logger.info(f"✅ Healed {symbol:10s} | Recovered: ${recovered:>8.2f}")
                ...
                report.healing_events.append(event)
                report.errors.append(f"{symbol}: {e!s}")

                logger.error(f"❌ Failed to heal {symbol}: {e!s}")
