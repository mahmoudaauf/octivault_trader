...
class ReserveManager:
    ...
    async def get_reserve_snapshot(self) -> ReserveSnapshot:
        ...
        if status == ReserveStatus.CRITICAL:
            ...
            actions.append(
                f"Buffer: {buffer:.2f} USDT (target: {required_minimum * (1 - policy.caution_threshold):.2f})"
            )
    ...
    async def adjust_reserve_for_volatility(self, new_regime: Optional[VolatilityRegime] = None):
        ...
        new_required = max(total_nav * new_policy.min_ratio, new_policy.min_absolute_usdt)
    ...
    async def get_reserve_statistics(self) -> dict[str, Any]:
        ...
        return {
            ...
            "critical_events": sum(
                1 for s in self.reserve_history if s.status == ReserveStatus.CRITICAL
            ),
            "warning_events": sum(
                1 for s in self.reserve_history if s.status == ReserveStatus.WARNING
            ),
        }
    ...
    async def print_reserve_status(self):
        ...
        print(
            f"Required Minimum:    {snapshot.required_minimum:>10.2f} USDT ({snapshot.policy.min_ratio*100:.0f}%)"
        )
