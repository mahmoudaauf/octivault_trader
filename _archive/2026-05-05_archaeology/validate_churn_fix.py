    ...
    try:
        ...
        if proven_match:
            ...
            if proven_entries != 7:
                ...
                return False
            print(f"  ✅ PROVEN_SYMBOLS has 7 entries")
        ...
        if excluded_match:
            ...
            if excluded_entries != 22:
                ...
                return False
            print(f"  ✅ EXCLUDED_SYMBOLS has 22 entries")
...
def validate_symbol_screener():
    ...
    try:
        ...
        if '_should_accept_symbol' not in source:
            print(f"  ❌ _propose() doesn't call _should_accept_symbol")
            return False
        print(f"  ✅ _propose() has convergence gating check")
...
def validate_swing_trade_hunter():
    ...
    try:
        ...
        if not hasattr(SwingTradeHunter, 'get_available_capital_for_symbol'):
            print(f"  ❌ get_available_capital_for_symbol method not found")
            return False
        
        print(f"  ✅ get_available_capital_for_symbol method exists")
...
def validate_liquidation_agent():
    ...
    try:
        ...
        if 'is_symbol_proven' not in source:
            print(f"  ❌ build_plan() doesn't check is_symbol_proven")
            return False
        print(f"  ✅ build_plan() checks is_symbol_proven")
        
        if 'sort_key' not in source and 'sort' not in source:
            print(f"  ❌ build_plan() doesn't have sorting logic")
            return False
        print(f"  ✅ build_plan() has sorting logic")
...
def validate_uure_gating():
    ...
    try:
        ...
        if 'can_add_new_symbol' not in source:
            print(f"  ❌ _collect_discovery_proposals() doesn't call can_add_new_symbol")
            return False
        print(f"  ✅ _collect_discovery_proposals() has convergence gate check")
        
        if 'convergence_gate' not in source:
            print(f"  ❌ _collect_discovery_proposals() doesn't filter on convergence_gate")
            return False
        print(f"  ✅ _collect_discovery_proposals() filters convergence_gate rejections")
...
def validate_checkpoint_files():
    ...
    try:
        ...
        if not convergence_file.exists():
            print(f"  ❌ fix_6_to_10_convergence_checkpoint.json not found")
            return False
        ...
        if checkpoint.get('status') != 'implemented':
            ...
            return False
        print(f"  ✅ Convergence checkpoint file valid")
