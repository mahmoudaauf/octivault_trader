...
def check_60_20_20_split():
    ...
    print("\n  Expected Split:")
    print_check("Trading Core (60%)", abs(core - 40.58) < 0.01, f"${core:.2f}")
    print_check("Trading Alts (20%)", abs(alts - 13.53) < 0.01, f"${alts:.2f}")
    print_check("Dust Healing (20%)", abs(dust - 13.53) < 0.01, f"${dust:.2f}")
    print_check("Total", abs(core + alts + dust - allocatable) < 0.01, f"${core + alts + dust:.2f}")
...
def check_orchestrator_flow():
    ...
    print_check(
        "Step 2: Calculate Allocatable",
        True,
        f"${allocatable:.2f} (${free_usdt:.2f} - ${reserve:.2f})",
    )
    ...
    print_check(
        "Step 5: Capital Floor Check",
        not capital_floor_met,
        f"${free_usdt:.2f} < $10.00 (needs recovery)",
    )
    ...
    print(f"    Mode: {mode}")
    print("    Status: LOCKED (capital floor not met)")
...
def check_code_deployment():
    ...
    target_file = Path(
        "/Users/mauf/Desktop/Octi AI Trading Bot/octivault_trader/src/l6_governance/capital_allocator.py"
    )
...
def check_integration():
    ...
    target_file = Path(
        "/Users/mauf/Desktop/Octi AI Trading Bot/octivault_trader/src/l5_meta/meta_controller.py"
    )
    ...
    with open(target_file) as f:
        ...
        print_check(
            "Calls allocate_with_nav_dynamics()",
            has_nav_dynamics_call,
            "If FALSE, integration may not be complete",
        )
        print_check(
            "Routes dust_healing budget",
            has_dust_budget,
