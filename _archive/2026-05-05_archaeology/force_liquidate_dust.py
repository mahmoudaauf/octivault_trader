...
async def liquidate_positions(
    ...
    for symbol, qty, price, value in positions:
        try:
            ...
            if dry_run:
                print("   [DRY RUN] Would submit SELL order")
                results["attempted"] += 1
                results["total_recovered"] += value
                results["successful"] += 1
            else:
                # Actually execute the liquidation
                print("   [EXECUTING] Submitting SELL order...")
                try:
...
async def main():
    ...
    if mode not in ("dry-run", "execute"):
        ...
        print("  dry-run:  Show what would be liquidated (DEFAULT)")
        print("  execute:  Actually liquidate dust (LIVE)")
        sys.exit(1)
    ...
    print("=" * 80)
    print(
        f"Mode: {'DRY RUN (no orders submitted)' if dry_run else '⚠️  LIVE EXECUTION (orders will be submitted)'}"
    )
    print("=" * 80)
