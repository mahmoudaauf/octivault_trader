...
class SignalFusion:
        ...
        try:
            ...
            if config:
                ...
                if custom_weights and isinstance(custom_weights, dict):
                    ...
                    self.logger.info(
                        f"[SignalFusion] Loaded custom agent weights: {custom_weights}"
                    )
        except Exception as e:
            ...
            try:
                ...
                for symbol in symbols_with_signals:
                    try:
                        ...
                        self.logger.debug(
                            f"[SignalFusion] Error fusing {symbol}: {e}", exc_info=True
                        )
                ...
                self.logger.warning(
                    f"[SignalFusion] Unexpected error in fusion loop: {e}", exc_info=True
                )
    ...
    async def _fuse_symbol_signals(self, symbol: str):
        ...
        try:
            async with self.shared_state.lock:
                agent_signals: dict[str, dict[str, str]] = self.shared_state.agent_signals.get(
                    symbol, {}
                )
        ...
        try:
            ...
            self.logger.debug(
                f"[SignalFusion] Composite edge failed for {symbol}: {e}. Falling back to voting."
            )
            decision = None
            confidence = 0.0
        ...
        if decision:
            self.logger.info(
                f"[SignalFusion:{symbol}] Consensus: {decision.upper()} (conf={confidence:.2f}, mode={self.fusion_mode})"
            )
            routed = await self._emit_fused_signal(
                symbol, decision, confidence, agent_signals, fusion_result
            )
    ...
    def _compute_composite_edge(
        self, agent_signals: dict[str, dict[str, str]]
