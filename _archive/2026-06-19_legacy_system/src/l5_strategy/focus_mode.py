...
class FocusModeManager:
    ...
    def _is_recovery_sellable(
        self, symbol: str, pos_data: dict, ignore_filters: bool = False, ignore_core: bool = True
    ) -> bool:
    ...
    async def _bootstrap_focus_symbols(self) -> set[str]:
        ...
        try:
            ...
            if not snap:
                self.logger.warning(
                    "[WALLET_FOCUS_BOOTSTRAP] No positions found in wallet snapshot. Using empty set."
                )
                self.FOCUS_SYMBOLS = set()
            ...
            for sym_raw, pos_data in snap.items():
                ...
                if not self._is_recovery_sellable(
                    sym, pos_data, ignore_filters=False, ignore_core=True
                ):
                    continue
            ...
            if not new_focus_symbols and snap:
                self.logger.warning(
                    ...
                    self.MIN_SIGNIFICANT_USDT,
                    self.MAX_FOCUS_SYMBOLS,
                )
                all_candidates = []
                for sym_raw, pos_data in snap.items():
                    if not isinstance(pos_data, dict):
            ...
            self.logger.warning(
                ...
                len(new_focus_symbols),
                sorted(new_focus_symbols),
                non_focus_count,
            )

            # Log capital allocation
            total_value = sum(
                float(pd.get("value_usdt", 0.0) or 0.0)
                for pd in snap.values()
    ...
    async def _update_focus_symbols(self) -> set[str]:
        ...
        try:
            ...
            self.logger.warning(
                "[WALLET_FOCUS_BOOTSTRAP] Fallback: Bootstrap not run yet, triggering now"
            )
            return await self._bootstrap_focus_symbols()

        except Exception as e:
            self.logger.error(
                "[WALLET_FOCUS_BOOTSTRAP] Failed to get focus symbols: %s", e, exc_info=True
            )
            return self.FOCUS_SYMBOLS
