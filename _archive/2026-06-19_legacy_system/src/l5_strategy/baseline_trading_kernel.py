...
# Back-compat shim
from .contracts import TradeIntent  # يمرر نفس الكلاس للاسم القديم
...
class KernelState:
    ...
    def set_nav_quote(self, v: float):
        self.nav_quote = max(0.0, float(v))

    def get_nav_quote(self) -> float:
        return float(self.nav_quote)
...
class ExecutionFacade:
    ...
    def __init__(
        self, state: KernelState, exchange_client, risk_manager=None, execution_manager=None
    ):
        self.s = state
    ...
    async def place(self, order: ExecOrder) -> OrderResult:
        ...
        if not self.s.is_symbol_tradable(order.symbol):
            return OrderResult(
                ok=False, symbol=order.symbol, side=order.side, reason="invalid_symbol"
            )
        ...
        if not (self.exec_mgr and hasattr(self.exec_mgr, "execute_trade")):
            ...
            return OrderResult(
                ok=False, symbol=order.symbol, side=order.side, reason="execution_manager_missing"
            )
        ...
        try:
            ...
            return OrderResult(
                ok=False, symbol=order.symbol, side=order.side, reason=f"exec_error:{e}"
            )
