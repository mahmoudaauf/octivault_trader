    ...
    def analyze_new_lines(self, lines: list):
        ...
        for line in lines:
            ...
            if "[Meta:SafeMinHold] Bypassing min-hold check" in line:
                ...
                if match:
                    ...
                    self.recovery_bypasses.append(
                        {"time": datetime.now().isoformat(), "symbol": symbol, "log": line.strip()}
                    )
            ...
            if "ERROR" in line or "CRITICAL" in line:
                self.errors.append({"time": datetime.now().isoformat(), "log": line.strip()})
                print(f"❌ ERROR: {line.strip()}")

            # Check for warnings
            if "WARNING" in line:
                self.warnings.append({"time": datetime.now().isoformat(), "log": line.strip()})
...
async def monitor_logs_realtime(
    log_file: str = "6hour_session_monitored.log", refresh_interval: int = 10
):
