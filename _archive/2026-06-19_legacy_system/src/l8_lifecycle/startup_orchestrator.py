...
class StartupOrchestrator:
    ...
    async def execute_startup_sequence(self) -> bool:
        ...
        try:
            ...
            self.logger.warning("[StartupOrchestrator] PHASE 8.5: STARTUP SEQUENCING ORCHESTRATOR")
            self.logger.warning(
            ...
            if not success:
                raise RuntimeError("Phase 8.5: Capital ledger construction failed - cannot proceed")
            ...
            self.logger.warning("[StartupOrchestrator] ✅ STARTUP ORCHESTRATION COMPLETE")
            self.logger.warning("[StartupOrchestrator] Portfolio is ready for MetaController")
            self.logger.warning(
                "[StartupOrchestrator] ═══════════════════════════════════════════════════"
            )
            ...
            self.logger.error(f"[StartupOrchestrator] 💥 FATAL ERROR: {e}", exc_info=True)
            raise
    ...
    async def _step_recovery_engine_rebuild(self) -> bool:
        ...
        try:
            ...
            if not self.recovery_engine:
                self.logger.warning(
                    f"[StartupOrchestrator] {step_name} - RecoveryEngine not available, skipping"
                )
            ...
            self.logger.info(f"[StartupOrchestrator] {step_name} complete: {elapsed:.2f}s")
            return True

        except Exception as e:
            self.logger.error(f"[StartupOrchestrator] {step_name} - Error: {e}", exc_info=True)
            return False
    ...
    async def _step_aggressive_exchange_liquidation(self) -> bool:
        ...
        try:
            ...
            if not symbols_to_sell:
                self.logger.info(
                    f"[StartupOrchestrator] {step_name} - " "No positions to sell (all clean)"
                )
                return True
            ...
            for pair_symbol in symbols_to_sell:
                try:
                    ...
                    self.logger.warning(
                        f"[StartupOrchestrator] {step_name} - " f"Error selling {pair_symbol}: {e}"
                    )
        ...
        try:
            ...
            if hasattr(self.shared_state, "authoritative_wallet_sync"):
                try:
                    ...
                    self.logger.debug(
                        f"[StartupOrchestrator] {step_name} - Called authoritative_wallet_sync()"
                    )
                except Exception as e:
                    self.logger.debug(
                    ...
                    if hasattr(self.shared_state, "hydrate_positions_from_balances"):
                        ...
                        if asyncio.iscoroutine(result):
                            await result
                        self.logger.debug(
                            f"[StartupOrchestrator] {step_name} - Called hydrate_positions_from_balances()"
                        )
            ...
            positions = getattr(self.shared_state, "positions", {}) or {}
            open_positions = {
                k: v for k, v in positions.items() if float(v.get("quantity", 0.0) or 0.0) > 0
            }
            ...
            self.logger.error(
                f"[StartupOrchestrator] {step_name} - Unexpected error: {e}", exc_info=True
            )
            return False
    ...
    async def _step_auditor_restart_recovery(self) -> bool:
        ...
        try:
            ...
            recover_method = getattr(
                self.exchange_truth_auditor, "restart_recovery", None
            ...
            if recover_method is not None:
                try:
                    ...
                    if asyncio.iscoroutine(result):
                        await result
                    self.logger.debug(
                        f"[StartupOrchestrator] {step_name} - Called {recover_method.__name__}()"
                    )
                except Exception as e:
                    self.logger.debug(
            ...
            self.logger.warning(f"[StartupOrchestrator] {step_name} - Non-fatal error: {e}")
            return True  # Non-fatal
    ...
    async def _step_build_capital_ledger(self) -> bool:
        ...
        try:
            ...
            try:
                if hasattr(self.shared_state, "rebuild_nav_from_state"):
                    ...
                    invested_capital = float(
                        rebuilt.get("invested_capital", invested_capital) or 0.0
                    )
            ...
            for detail in position_details:
                self.logger.debug(f"  {detail['symbol']}: ${detail['value']:.2f}")
    ...
    async def _step_verify_capital_integrity(self) -> bool:
        ...
        try:
            ...
            shadow_mode_config = (
                getattr(self.config, "SHADOW_MODE", False) if self.config else False
            )
                ...
                if nav > 0:
                    ...
                    if balance_error > 0.05:  # 5% threshold
                        ...
                        try:
                            ...
                            await self.shared_state.ensure_latest_prices_coverage(
                                self._price_fetcher
                            )
                            ...
                            fresh_quote = (
                                await self.shared_state.get_balance("USDT")
                                if self.shared_state
                                else {}
                            )
                            ...
                            fresh_error = (
                                abs((fresh_nav - (fresh_invested + fresh_free)) / fresh_nav)
                                if fresh_nav > 0
                                else 0.0
                            )
    ...
    async def _emit_state_rebuilt_event(self) -> None:
        ...
        try:
            if hasattr(self.shared_state, "emit_event"):
                await self.shared_state.emit_event(
                    "StartupStateRebuilt",
                    {
                        "timestamp": time.time(),
