...
class LifecycleManager:
    ...
    def set_state(
        ...
        if not force and not self._is_valid_transition(from_state, new_state):
            self.logger.warning(f"Invalid transition for {symbol}: " f"{from_state} -> {new_state}")
            return False
        ...
        self.logger.info(
            f"Lifecycle transition: {symbol} " f"{from_state} -> {new_state} ({reason})"
        )
    ...
    def set_cooldown(
        ...
        self.logger.debug(f"Cooldown set for {symbol}: {cooldown_seconds}s")
    ...
    def get_status(self) -> dict[str, Any]:
        ...
        return {
            ...
            "symbols_in_cooldown": len(
                [
                    s
                    for s, m in self._symbol_states.items()
