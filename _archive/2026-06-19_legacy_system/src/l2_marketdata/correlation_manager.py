...
class CorrelationCalculator:
    ...
    def calculate_correlation(returns_a: list[float], returns_b: list[float]) -> Optional[float]:
        ...
        try:
            ...
            numerator = sum(
                (returns_a[i] - mean_a) * (returns_b[i] - mean_b) for i in range(len(returns_a))
            )
...
class PortfolioConcentrationManager:
        ...
        for i, sym_a in enumerate(symbols):
            for sym_b in symbols[i + 1 :]:
                ...
                corr = CorrelationCalculator.calculate_correlation(
                    pos_a.historical_returns, pos_b.historical_returns
                )
    ...
    def validate_new_position(
        self, symbol: str, allocation: float, sector: str, exchange: str, returns: list[float]
        ...
        if allocation > self.max_position_allocation:
            ...
            logger.warning(
                f"CONCENTRATION: {symbol} {allocation:.1f}% exceeds max {self.max_position_allocation}%"
            )
            return result
        ...
        if new_sector_exposure > self.max_sector_allocation:
            ...
            self._log_result(result)
            logger.warning(
                f"SECTOR_CONCENTRATION: {sector} {new_sector_exposure:.1f}% exceeds max {self.max_sector_allocation}%"
            )
            return result
        ...
        if new_exchange_exposure > self.max_exchange_allocation:
            ...
            self._log_result(result)
            logger.warning(
                f"EXCHANGE_CONCENTRATION: {exchange} {new_exchange_exposure:.1f}% exceeds max {self.max_exchange_allocation}%"
            )
            return result
        ...
        with self.lock:
            if self.correlation_matrix:
                for existing_symbol in self.correlation_matrix.symbols:
                    ...
                    corr = CorrelationCalculator.calculate_correlation(
                        existing_pos.historical_returns, returns
                    )
                    ...
                    if corr is not None:
                        ...
                        if corr > self.max_position_correlation:
                            ...
                            self._log_result(result)
                            logger.warning(
                                f"HIGH_CORRELATION: {symbol} vs {existing_symbol} ρ={corr:.2f}"
                            )
                            return result
                        ...
                        if corr > self.warning_correlation_threshold:
                            logger.info(
                                f"ELEVATED_CORRELATION: {symbol} vs {existing_symbol} ρ={corr:.2f} - consider risk"
                            )
        ...
        self._log_result(result)
        logger.info(
            f"CORRELATION_ACCEPTED: {symbol} allocation={allocation:.1f}%, sector_exp={new_sector_exposure:.1f}%, exchange_exp={new_exchange_exposure:.1f}%"
        )
        return result
    ...
    def _calculate_sector_exposure(self, sector: str) -> float:
        ...
        with self.lock:
            return sum(
                pos.allocation for pos in self.active_positions.values() if pos.sector == sector
            )
    ...
    def _calculate_exchange_exposure(self, exchange: str) -> float:
        ...
        with self.lock:
            return sum(
                pos.allocation for pos in self.active_positions.values() if pos.exchange == exchange
            )
    ...
    def get_portfolio_risk_profile(self) -> dict:
        ...
        for pos in positions.values():
            ...
            exchange_exposure[pos.exchange] = (
                exchange_exposure.get(pos.exchange, 0) + pos.allocation
            )
        ...
        if correlation_matrix:
            for (sym_a, sym_b), corr in correlation_matrix.correlations.items():
                if corr > self.warning_correlation_threshold:
                    high_correlations.append({"pair": f"{sym_a}-{sym_b}", "correlation": corr})
            ...
            results.append(
                {
                    ...
                    "reason": log.reason,
                    "correlation_pairs": [
                        {"symbols": f"{p.symbol_a}-{p.symbol_b}", "correlation": p.correlation}
