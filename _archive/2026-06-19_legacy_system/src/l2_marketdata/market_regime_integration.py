...
class RegimeAwareMediator:
    ...
    async def apply_regime_adaptations(
        ...
        try:
            ...
            if self.meta_controller:
                await self._apply_to_meta_controller(symbol, regime, agent_weights, confidence)
            ...
            self.logger.error(f"Error applying regime adaptations for {symbol}: {e}", exc_info=True)
            return None
    ...
    async def _apply_to_meta_controller(
        ...
        try:
            ...
            self.logger.warning(f"Failed to apply regime to MetaController: {e}")
    ...
    async def _apply_to_agent_manager(
        ...
        try:
            ...
            self.logger.warning(f"Failed to apply regime to AgentManager: {e}")
    ...
    async def _apply_to_capital_allocator(
        ...
        try:
            ...
            self.logger.warning(f"Failed to apply regime to CapitalAllocator: {e}")
    ...
    async def _apply_to_execution(
        ...
        try:
            ...
            self.logger.warning(f"Failed to apply regime to ExecutionManager: {e}")
