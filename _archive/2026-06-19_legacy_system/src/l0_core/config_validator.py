...
class TradingConfig(BaseModel):
    ...
    max_universe_symbols: int = Field(
        default=30, ge=5, le=500, description="Max symbols in universe"
    )
    max_active_symbols: int = Field(
        default=5, ge=1, le=20, description="Max active trading symbols"
    )
    min_active_symbols: int = Field(
        default=3, ge=1, le=10, description="Min active trading symbols"
    )

    capital_profile: CapitalProfile = Field(
        default=CapitalProfile.BOOTSTRAP_GROWTH, description="Capital profile"
    )
    capital_profile_nav_threshold: float = Field(
        default=500.0, ge=0, description="NAV threshold for profile switching"
    )

    min_bars_for_market_ready: int = Field(
        default=150, ge=50, le=500, description="Min bars for market ready"
    )
    required_symbol_coverage: float = Field(
        default=0.70, ge=0.0, le=1.0, description="Required symbol coverage"
    )

    volatility_regime: VolatilityRegime = Field(
        default=VolatilityRegime.NORMAL, description="Volatility regime"
    )
    volatility_regime_low_pct: float = Field(
        default=0.0025, ge=0, description="Low volatility threshold %"
    )
    volatility_regime_high_pct: float = Field(
        default=0.006, ge=0, description="High volatility threshold %"
    )
    ...
    def _load_env_vars(self) -> dict[str, Any]:
        ...
        return {
            ...
            "trading": {
                ...
                "capital_profile_nav_threshold": float(
                    os.getenv("CAPITAL_PROFILE_NAV_THRESHOLD", "500.0")
                ),
                ...
                "volatility_regime_low_pct": float(
                    os.getenv("VOLATILITY_REGIME_LOW_PCT", "0.0025")
                ),
                "volatility_regime_high_pct": float(
                    os.getenv("VOLATILITY_REGIME_HIGH_PCT", "0.006")
                ),
            ...
            "observability": {
                ...
                "correlation_ids_enabled": os.getenv("CORRELATION_IDS_ENABLED", "true").lower()
                == "true",
            },
        }
