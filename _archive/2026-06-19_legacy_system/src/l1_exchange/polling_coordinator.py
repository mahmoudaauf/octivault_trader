        ...
        try:
            while self._running:
                try:
                    ...
                    self.logger.error(
                        "[PollingCoordinator] Open orders poll error: %s", e, exc_info=True
                    )
        ...
        try:
            while self._running:
                try:
                    ...
                    self.logger.error(
                        "[PollingCoordinator] Balance poll error: %s", e, exc_info=True
                    )
        ...
        try:
            while self._running:
                try:
                    ...
                    self.logger.error(
                        "[PollingCoordinator] Position poll error: %s", e, exc_info=True
                    )
    ...
    async def _emit_health_loop(self) -> None:
        ...
        try:
            while self._running:
                ...
                try:
                    await asyncio.wait_for(
                        self._stop_event.wait(), timeout=self.config.health_cadence_sec
                    )
                    break
                except asyncio.TimeoutError:
                    pass
