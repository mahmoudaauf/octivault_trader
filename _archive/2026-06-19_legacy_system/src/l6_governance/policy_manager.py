...
class PolicyManager:
    def __init__(self, logger, config):
        ...
        self._economic_guard = {
            "enabled": getattr(
                config, "ECONOMIC_GUARD_ENABLED", True
            ),  # Enabled by default (config can disable)
            "min_edge_bps": float(getattr(config, "MIN_EXPECTED_EDGE_BPS", 10.0)),
            "fallback_edge_bps": float(
                getattr(config, "FALLBACK_EDGE_BPS", 80.0)
            ),  # INCREASED: 80 bps fallback (2.7x fees)
            ...
            "slippage_bps": float(
                os.getenv("CR_PRICE_SLIPPAGE_BPS", "15")
            ),  # 0.15% slippage (system default)
            "buffer_bps": float(getattr(config, "SAFETY_BUFFER_BPS", 5.0)),  # 0.05% safety buffer
            "min_positive_edge_bps": float(
                getattr(config, "MIN_POSITIVE_EDGE_BPS", 35.0)
            ),  # INCREASED: 35 bps minimum (covers 30 bps costs + 5 bps profit)
            "fee_expectancy_multiplier": float(
                os.getenv("FEE_EXPECTANCY_MULTIPLIER", "2.5")
            ),  # INCREASED: Profit >= 2.5x fees (was 1.6x)
        ...
        self._phase2_guard = {
            "dust_ratio_trigger": float(
                getattr(config, "PHASE2_DUST_RATIO_TRIGGER", 0.8)
            ),  # 80% dust ratio trigger
            "active_since": None,  # Timestamp when phase2 was activated
            "activation_age_sec": float(
                getattr(config, "PHASE2_ACTIVATION_AGE_SEC", 300.0)
            ),  # 5 minutes grace period
            "position_grace_sec": float(
                getattr(config, "PHASE2_POSITION_GRACE_SEC", 600.0)
            ),  # 10 minutes position grace
        }
    ...
    def calculate_policy_nudges(
        self, metrics: dict[str, Any], mode_envelope: dict[str, Any], mode: str = "NORMAL"
    ) -> dict[str, float]:
        """
        ...
        def merge(target_dict, source_dict, weight):
            if weight <= 0.01:
                return  # Optimization for disabled policies
    ...
    def check_entry_profitability(
        self, planned_quote: float, expected_alpha: float, taker_bps: float
    ) -> tuple[bool, str]:
        """
        Rule: Expected gross profit >= 3 * total fees.
        """
        if planned_quote <= 0:
            return False, "Zero quote"
        ...
        if expected_profit < (total_fees * multiplier):
            return (
                False,
                f"Low expectancy: profit {expected_profit:.2f} < {multiplier}x fees {total_fees:.2f}",
            )
    ...
    async def _apply_dynamic_tradeability_policy(
        ...
        alpha = self._clamp(
            self._safe_float(getattr(self.config, "POLICY_CONF_BIAS_EMA_ALPHA", 0.25), 0.25),
            0.05,
            1.0,
        )
        ...
        prev_global = self._safe_float(
            dyn_cfg.get("ML_DYNAMIC_REQUIRED_CONF_POLICY_BIAS", 0.0), 0.0
        )
        ...
        if callable(update_dynamic):
            ...
            if (
                not hasattr(shared_state, "dynamic_config")
                or getattr(shared_state, "dynamic_config", None) is None
            ):
    ...
    def _apply_capital_utilization_policy(
        self, metrics: dict[str, Any], envelope: dict[str, Any], nudges: dict[str, Any]
    ):
        """
    ...
    async def _execute_decision(
        self, meta_controller, symbol: str, side: str, signal: dict, accepted_symbols_set: set
    ):
        # Example execution logic: call execution_logic for actual trade
        try:
            result = await meta_controller.execution_logic._execute_decision(
                symbol, side, signal, accepted_symbols_set
            )
            meta_controller.logger.info(f"[PolicyManager] Executed {side} for {symbol}: {result}")
            return result
        except Exception as e:
            meta_controller.logger.error(
                f"[PolicyManager] Error executing {side} for {symbol}: {e}", exc_info=True
            )
    ...
    async def evaluate_policies(self, meta_controller, loop_id):
        ...
        try:
            ...
            win_rate = 0.5
            try:
                from src.l0_core.metrics import get_metrics
            ...
            await self._apply_dynamic_tradeability_policy(
                meta_controller, metrics, mode=mode, nudges=nudges
            )

            # Log significant nudges
            if any(
                abs(v) > 0.001 if isinstance(v, (int, float)) else False for v in nudges.values()
            ):
    ...
    def _check_economic_profitability(
    ...
    async def dust_accumulation_guard(
        self,
        symbol: str,
        position_qty: float,
        ...
        try:
            ...
            if min_notional is None:
                try:
                    ...
                    self.logger.debug(
                        f"[INVARIANT:DustGuard] Failed to get minNotional for {symbol}: {e}"
                    )
            ...
            if notional < min_notional:
                ...
                if shared_state and hasattr(shared_state, "mark_accumulating"):
                    try:
                        ...
                        self.logger.debug(
                            f"[INVARIANT:DustGuard] Failed to record accumulating: {e}"
                        )
                ...
                try:
                    ...
                    if (
                        ...
                        if hasattr(shared_state, "dust_unhealable"):
                            shared_state.dust_unhealable = getattr(
                                shared_state, "dust_unhealable", {}
                            )
                            shared_state.dust_unhealable[
                                str(symbol).upper()
                            ] = "UNHEALABLE_LT_MIN_NOTIONAL"
                    ...
                    self.logger.debug(
                        "[INVARIANT:DustGuard] Auto-retire path failed for %s",
                        symbol,
                        exc_info=True,
                    )
