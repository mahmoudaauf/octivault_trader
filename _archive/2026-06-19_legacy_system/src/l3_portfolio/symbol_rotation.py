...
class SymbolRotationManager:
    ...
    def is_locked(self) -> bool:
        ...
        if is_locked:
            ...
            logger.debug("[SymbolRotation:SoftLock] Locked for %.1f more seconds", remaining)
    ...
    def can_rotate_to_score(self, current_score: float, candidate_score: float) -> bool:
        ...
        if can_rotate:
            improvement = (
                ((candidate_score - current_score) / current_score * 100)
                if current_score > 0
                else 0
            )
            logger.info(
    ...
    def enforce_universe_size(
        self, current_active: list[str], all_candidates: list[str]
    ) -> dict[str, Any]:
        """
        ...
        return {"action": "none", "affected_symbols": [], "reason": "Universe correctly sized"}
