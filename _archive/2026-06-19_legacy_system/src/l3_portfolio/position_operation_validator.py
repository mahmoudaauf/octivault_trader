...
class PositionOperationValidator:
    ...
    def __init__(self, shared_state: Any, config: Any):
        ...
        self.protect_external_positions = bool(getattr(config, "PROTECT_EXTERNAL_POSITIONS", True))
        self.allow_stable_trading = bool(getattr(config, "ALLOW_STABLE_TRADING", False))
        self.log_all_validations = bool(getattr(config, "LOG_ALL_VALIDATIONS", True))
        ...
        try:
            ...
            if classification == "BOT_POSITION":
                ...
                return OperationResult(
                    allowed=True, reason="Stack on existing bot position allowed"
                )
            ...
            # Default allow
            return OperationResult(
                allowed=True, reason=f"Entry allowed (classification={classification})"
            )

        except Exception as e:
            self.logger.error(f"[VALIDATE] TRADE_ENTRY {symbol}: ERROR - {e}", exc_info=True)
            return OperationResult(
                allowed=False, reason=f"Validation error: {e}", severity="CRITICAL"
            )
    ...
    async def can_trade_exit(
        ...
        try:
            ...
            if current_qty is None or current_qty <= 0:
                ...
                return OperationResult(
                    allowed=False, reason="No position to exit", severity="WARNING"
                )
            ...
            if classification == "EXTERNAL_POSITION":
                ...
                if self.protect_external_positions and not self._is_approved_external_operation(
                    reason
                ):
                    self.logger.warning(
            ...
            return OperationResult(
                allowed=True, reason=f"Exit allowed (classification={classification})"
            )
    ...
    async def can_liquidate(
        ...
        try:
            ...
            if current_qty is None or current_qty <= 0:
                return OperationResult(
                    allowed=False, reason="No position to liquidate", severity="WARNING"
                )
            ...
            if classification == "BOT_POSITION":
                ...
                return OperationResult(
                    allowed=True, reason="Liquidation of bot position allowed", severity="WARNING"
                )
    ...
    async def validate_operation(
        self, operation_type: OperationType, symbol: str, quantity: float, reason: str = ""
    ) -> OperationResult:
        """
        ...
        if operation_type == OperationType.TRADE_ENTRY:
            ...
            or operation_type == OperationType.DUST_CLEANUP
        ):
