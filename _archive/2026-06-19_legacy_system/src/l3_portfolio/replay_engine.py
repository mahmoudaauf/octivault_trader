...
from src.l3_portfolio.event_store import Event, EventStore, EventType, get_event_store
...
class DeterministicReplayEngine:
    ...
    async def replay_time_range(self, start_time: float, end_time: float) -> PortfolioState:
    ...
    async def replay_what_if(
        self, after_sequence: int, modifications: dict[str, Callable]
    ) -> PortfolioState:
        """
    ...
    async def find_loss_cause(self, symbol: str, loss_threshold: float = 100.0) -> dict[str, Any]:
        """
        ...
        decision_chain = [
            e for e in events[: loss["index"]] if e.event_type == EventType.TRADE_EXECUTED
        ][-5:]
    ...
    async def _replay(
        self, events: list[Event], initial_state: Optional[PortfolioState] = None
    ) -> PortfolioState:
        """
    ...
    def _apply_event_with_mods(
        self, state: PortfolioState, event: Event, modifications: dict[str, Callable]
    ) -> PortfolioState:
    ...
    def _assert_state_valid(self, state: PortfolioState):
        ...
        for pos_data in state.closed_positions[-10:]:
            pnl_ratio = abs(
                pos_data.get("pnl", 0)
                / (pos_data.get("quantity", 1) * pos_data.get("entry_price", 1) + 0.01)
            )
    ...
    def compare_states(self, state1: PortfolioState, state2: PortfolioState) -> dict[str, Any]:
        """Compare two states."""
        return {
