...
class PortfolioSegmentationManager:
    ...
    async def segment_portfolio(self) -> PortfolioSegmentation:
        ...
        productive_value = sum(
            h.current_value for h in holdings if h.bucket == BucketType.PRODUCTIVE_INVENTORY
        )
        dead_capital_value = sum(
            h.current_value for h in holdings if h.bucket == BucketType.DEAD_CAPITAL
        )
        ...
        health_score = self._calculate_health_score(cash_ratio, dead_ratio, len(holdings))
    ...
    def _assign_bucket(
        self, symbol: str, classification: HoldingClassification, position: dict
    ) -> BucketType:
    ...
    def _calculate_health_score(
        self, cash_ratio: float, dead_ratio: float, position_count: int
    ) -> float:
    ...
    def _get_immediate_actions(
        ...
        if cash_ratio < 10:
            actions.append(
                "🚨 CRITICAL: Operating cash below 10% - liquidate lowest-utility holdings"
            )
        ...
        dust_holdings = [
            h
            for h in holdings
            if h.classification in [HoldingClassification.DUST, HoldingClassification.NEAR_DUST]
        ]
        if dust_holdings:
            actions.append(
                f"⚠️  Liquidate {len(dust_holdings)} dust/near-dust positions ({', '.join([h.symbol for h in dust_holdings])})"
            )

        # Too fragmented?
        if position_count > self.MAX_POSITION_COUNT:
            actions.append(
                f"⚠️  Portfolio too fragmented ({position_count} positions) - consolidate"
            )
        ...
        if dead_holdings:
            actions.append(
                f"⚠️  Exit {len(dead_holdings)} dead capital positions ({', '.join([h.symbol for h in dead_holdings])})"
            )

        return actions

    def _get_strategic_recommendations(
        self, cash_ratio: float, dead_ratio: float, health_score: float, fragmentation: float
    ) -> list[str]:
...
"""
        ...
        if tier1:
            ...
            for h in tier1:
                self.logger(
                    f"    {h.symbol}: ${h.current_value:.2f} | Score: {h.utility_score:.0f} | PnL: {h.unrealized_pnl_pct:+.2%}"
                )
        ...
        # Tier 4: Exit
        tier4 = [
            h for h in seg.holdings if h.recommended_action in ["EXIT", "EXIT_SOON", "EXIT_ASAP"]
        ]
        if tier4:
            self.logger("  TIER 4 - EXIT (Low utility, exit priority):")
            for h in sorted(tier4, key=lambda x: x.priority):
                self.logger(
                    f"    {h.symbol}: ${h.current_value:.2f} | Score: {h.utility_score:.0f} | Action: {h.recommended_action}"
                )
