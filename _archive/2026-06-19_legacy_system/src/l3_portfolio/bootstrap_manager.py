    ...
    def get_status(self, symbol: str) -> dict[str, Any]:
        ...
        return {
            ...
            "budget_remaining": max(
                0, self._bootstrap_dust_bypass_budget - len(self._bootstrap_dust_bypass_symbols)
            ),
        }
...
class BootstrapOrchestrator:
    ...
    def enter_bootstrap_mode(self):
        ...
        if not self._bootstrap_mode_active:
            ...
            self.logger.info(
                f"🟢 Entered bootstrap mode " f"(NAV: {self._bootstrap_nav_at_start:.2f})"
            )
    ...
    async def apply_bootstrap_logic(self, signal: dict[str, Any]) -> dict[str, Any]:
        ...
        if self.is_active():
            ...
            if can_bypass_dust:
                self.logger.debug(f"Bootstrap bypass enabled for {symbol}")
    ...
    def get_status(self) -> dict[str, Any]:
        ...
        return {
            ...
            "duration_seconds": (
                time.time() - self._bootstrap_start_time if self._bootstrap_mode_active else None
            ),
            "initial_nav": self._bootstrap_nav_at_start,
            "current_nav": self.shared_state.nav,
            "bypass_manager_status": {
                "symbols_with_bypass": list(self.bypass_manager._bootstrap_dust_bypass_symbols),
                "budget_remaining": max(
                    0,
                    self.bypass_manager._bootstrap_dust_bypass_budget
                    - len(self.bypass_manager._bootstrap_dust_bypass_symbols),
                ),
            },
        }
