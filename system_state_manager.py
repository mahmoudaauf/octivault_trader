...
class SystemStateManager:
    ...
    def __init__(self, workspace_root: str = None):
        self.workspace_root = Path(
            workspace_root or "/Users/mauf/Desktop/Octi AI Trading Bot/octivault_trader"
        )
    ...
    def _load_operational_state(self) -> dict[str, Any]:
        ...
        if self.operational_state_file.exists():
            try:
                with open(self.operational_state_file) as f:
                    return json.load(f)
    ...
    def handle_restart_recovery(self) -> dict[str, Any]:
        ...
        self.operational_state["previous_restarts"].append(
            {"timestamp": datetime.now().isoformat(), "recovery_initiated": True}
        )
    ...
    def record_error(self, error: str, context: str = ""):
        ...
        self.session_memory["errors_encountered"].append(
            {"error": error, "context": context, "timestamp": datetime.now().isoformat()}
        )
        self.save_session_memory()
...
class ContextRecoveryEngine:
    ...
    def _build_recovery_info(self) -> dict[str, Any]:
        ...
        return {
            ...
            "last_known_phase": checkpoint["phase"]
            if checkpoint
            else self.state_manager.operational_state["current_phase"],
            "last_known_task": checkpoint["task"]
            if checkpoint
            else self.state_manager.operational_state["current_task"],
