...
def main():
    parser = argparse.ArgumentParser(
        description="Auto rule proposer — non-invasive proposals from diagnostics"
    )
    ...
    for p in proposals:
        print(
            f"{p['symbol']}: current={p['current_required_conf']} -> suggested={p['suggested_required_conf']}  (score={p['score']:.3f})"
        )
