...
# regex
RE_LOOP = re.compile(
    r"LOOP_SUMMARY\].*?loop_id=(\d+).*?capital_free=([0-9]+\.?[0-9]*).*?exec_attempted=(True|False).*?exec_result=([A-Z_]+).*?rej_count=(\d+)",
    re.I | re.S,
)
RE_GATED = re.compile(
    r"GateDebug:TRADEABILITY\].*?([A-Z0-9]+)\s+(BUY|SELL).*?signal_conf=([0-9]*\.?[0-9]+).*?required_conf=([0-9]*\.?[0-9]+).*?result=([A-Z]+)",
    re.I | re.S,
)
RE_EXEC_REJECT = re.compile(r"EXEC_REJECT\].*symbol=([A-Z0-9]+).*count=(\d+)", re.I)
RE_EXEC_REJECT_ALT = re.compile(
    r"SharedState - \[\s*EXEC_REJECT\s*\].*symbol=([A-Z0-9]+).*count=(\d+)", re.I
)
...
def pretty_print(out):
    for loop in out:
        print("\n" + "=" * 80)
        print(
            f"Loop {loop['loop_id']}: capital_free={loop['capital_free']:.2f} exec_attempted={loop['exec_attempted']} exec_result={loop['exec_result']} rej_count={loop['rej_count']}"
        )
        if not loop["symbols"]:
            print("  (no GateDebug or exec_reject events captured in this loop)")
            continue
        print(
            f"  {'SYMBOL':<10} {'sig_conf':>8} {'req_conf':>8} {'result':>8} {'exec_rej':>8} {'skip_rej':>8}"
        )
        for sym, info in sorted(
            loop["symbols"].items(), key=lambda x: x[1].get("exec_rejects", 0), reverse=True
        ):
