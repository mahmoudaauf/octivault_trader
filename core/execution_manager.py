...
# 
    ...
    self.max_spend_per_trade = float(
    getattr(config, "MAX_SPEND_PER_TRADE_USDT", 0))
    ...
    self.order_monitor_interval = float(
    getattr(config, "ORDER_MONITOR_INTERVAL", 15))
    self.stale_order_timeout_s = int(
    getattr(config, "STALE_ORDER_TIMEOUT_SECONDS", 120))
    self.max_concurrent_orders = int(getattr(config, "MAX_CONCURRENT_ORDERS", 5))
    self.enable_exec_cot = bool(
    ...
    _cfg('execution.allow_taker_if_within_bps', 0.0))
    self.min_free_reserve_usdt = float(
    _cfg('execution.min_free_reserve_usdt', 0.0))
    self.no_remainder_below_quote = float(
    _cfg('execution.no_remainder_below_quote', 0.0))
    # When NAV is tiny, serialize placement globally
    self.small_nav_threshold = float(
    _cfg('capital.small_nav_threshold_usdt', 50.0))

    # Liquidity healing
    self.max_liquidity_retries = int(getattr(config, "MAX_LIQUIDITY_RETRIES", 1))
    self.liquidity_retry_delay = float(
    getattr(config, "LIQUIDITY_RETRY_DELAY_SECONDS", 3.0))

    # Execution-block cooldowns (finite no-trade states)
    self.exec_block_max_retries = int(getattr(config, "EXEC_BLOCK_MAX_RETRIES", 3))
    self.exec_block_cooldown_sec = int(
    getattr(config, "EXEC_BLOCK_COOLDOWN_SEC", 600))
    ...
    *,
    ...
    bootstrap_override = bool(policy_ctx.get(
    "bootstrap_sell_override")) or rotation_override or ("bootstrap_exit" in tag_lower)
    bootstrap_override = bootstrap_override and bool(
    getattr(self.config, "BOOTSTRAP_ALLOW_SELL_BELOW_FEE", True))
    bootstrap_min_net = float(
    policy_ctx.get("bootstrap_sell_min_net", getattr(
        self.config, "BOOTSTRAP_MAX_NEGATIVE_PNL", 0.0)) or 0.0
    )
    ...
    buffer_pct = float(
    getattr(self.config, "TP_MIN_BUFFER_BPS", 0.0) or 0.0) / 10000.0
    entry_fee_mult = float(
    getattr(self.config, "MIN_PLANNED_QUOTE_FEE_MULT", 2.5) or 2.5)
    exit_fee_mult = float(
    getattr(self.config, "MIN_PROFIT_EXIT_FEE_MULT", 2.0) or 2.0)
    ...
    min_abs = float(
    getattr(self.config, "MIN_PORTFOLIO_IMPROVEMENT_USD", 0.05) or 0.0)
    min_pct = float(
    getattr(self.config, "MIN_PORTFOLIO_IMPROVEMENT_PCT", 0.0015) or 0.0)
    ...
    retirement_threshold = getattr(
    self.shared_state, "dust_retirement_rejection_threshold", 3)
    ...
    self.logger.warning(
    f"[DUST_RETIRED] Fallback permanent_dust tracking for {sym}")
    ...
    price = float(pos.get("mark_price", 0.0)) or float(
    pos.get("avg_price", 0.0)) or float(pos.get("entry_price", 0.0))
    if not price or price <= 0:
    ...
    event = {"ts": time.time(), "component": "ExecutionManager",
         "event": event_type, "symbol": symbol, **details}
    ...
    **feas_detail,
    ...
    if atr_pct <= 0:
    atr_pct = float(
    getattr(self.config, "MICRO_TRADE_KILL_FALLBACK_ATR_PCT", 0.0) or 0.0)
    round_trip_fee_rate = float(self.trade_fee_pct) * 2.0
    fee_mult = float(
    getattr(self.config, "MICRO_TRADE_KILL_ATR_FEE_MULT", 1.0) or 1.0)
    ...
    fee_mult = Decimal(
    str(getattr(self.config, "MIN_PLANNED_QUOTE_FEE_MULT", 2.5) or 2.5))
    ...
    acc_val = Decimal(str(intent.accumulated_quote)
                  ) if intent and intent.state == "ACCUMULATING" else Decimal("0")
    ...
    self.logger.warning(
    "[EM] ExecutionProbe = FAIL (Reason: %s, Gap: %.2f). Readiness = FALSE.", v_reason, gap)
    ...
    *,
    ...
    self.logger.debug(
    "[EM] force_finalize_position failed for %s", symbol, exc_info=True)
    return res
    ...
    self.logger.debug(
    "[EM] Failed to update position as CLOSED for %s", sym, exc_info=True)
    try:
    ...
    *,
    ...
    is_liq_full = is_liquidation or any(
    x in (tag or "").lower() for x in ("tp_sl", "balancer"))
    ...
    is_real_mode = bool(getattr(self.config, "LIVE_MODE", False)) and not bool(getattr(self.config, "SIMULATION_MODE", False)) and not bool(
    getattr(self.config, "PAPER_MODE", False)) and not bool(getattr(self.config, "TESTNET_MODE", False))
    ...
    is_liq_full = is_liquidation or any(
    x in tag_lower for x in ("tp_sl", "balancer"))
    liq_reason = str(
    ...
    return net_gate
    policy_authority = str(policy_ctx.get("authority")
                       or policy_ctx.get("policy_authority") or "").lower()
    ...
    fee_quote=fee_quote,
    reason=str(policy_ctx.get("exit_reason")
               or policy_ctx.get("reason") or "SELL_FILLED"),
    )
    ...
    self.logger.warning(
    f"[EXEC:LIQ] ⚠️ Liquidation SELL failed: status={status}, qty={exec_qty}")
    ...
    adjusted_quote = float(
    adjusted["quote_amount"]) if adjusted["quote_amount"] is not None else None
    ...
    min_target = float(
    getattr(self.config, "PROFIT_TARGET_BASE_USD_PER_HOUR", 20.0))
    ...
    self.logger.warning(
    f"[EXEC] 🛑 Circuit Breaker OPEN. Blocking trade for {sym}.")
    ...
    bootstrap_bypass = policy_ctx.get(
    "bootstrap_bypass", False) if policy_ctx else False
    ...
    self.logger.info(
    f"[EM:BOOTSTRAP] Bypassing agent reservation check for bootstrap execution")
    ...
    execute_quote = float(
    gap) if reason == "OK_DOWNSCALED" else float(planned_quote)
    ...
    is_liquidation = any(x in (tag or "")
                     for x in ("liquidation", "tp_sl", "balancer"))
    ...
    await self.shared_state.clear_rejections(sym, side.upper())
    self.logger.info(
    f"[MemoryOfFailure] ✅ Cleared rejections for {sym} {side} (successful execution)")
    if side == "buy":
    ...
    self.logger.error(
    f"[EM:SELL_EXCEPTION] symbol={sym} side={side} exception_type={type(e).__name__} error_type={error_type} message={error_msg}", exc_info=True)
    ...
    need_quote = float(qty) * float(current_price) * \
    float(1.0 + self.trade_fee_pct) * float(self.safety_headroom)
    heal_context = {"reason": "affordability_gate", "symbol": symbol,
                "needed_quote": need_quote, "current_price": current_price}
    ...
    if not ok_aff:
    if "INSUFFICIENT_QUOTE" in why or "QUOTE_LT_MIN_NOTIONAL" in why:
    need_quote = float(quote) * float(1.0 + self.trade_fee_pct) * \
    ...
    filters = await self.exchange_client.ensure_symbol_filters_ready(symbol)
    step_size, min_qty, max_qty, tick_size, min_notional = self._extract_filter_vals(
    filters)
    ...
    self.logger.debug(
    "[EM] Active order exists for %s %s; skipping.", symbol, side.upper())
    ...
    no_downscale = bool(self._current_policy_context.get(
    "_no_downscale_planned_quote", False))
    ...
    if hasattr(self, '_current_policy_context') and self._current_policy_context:
    is_bootstrap = bool(self._current_policy_context.get(
    "bootstrap_bypass", False))
    ...
    filled = bool(status and str(status.get('status', '')).upper()
    ...
    q_asset = self._split_symbol_quote(symbol)
    gross_needed = float(final_qty * current_price) * (1.0 +
                                                   float(self.trade_fee_pct)) * float(self.safety_headroom)
    _free, _ok_rem, _why_rem = await self._get_free_quote_and_remainder_ok(q_asset, gross_needed)
    if not _ok_rem:
    await self._log_execution_event("order_skip", symbol, {
    ...
    self.logger.info(
    "ExecutionManager.start: symbol filters warmed (if supported)")
    except Exception:
    self.logger.debug(
    "ExecutionManager.start warmup failed (non-fatal)", exc_info=True)
