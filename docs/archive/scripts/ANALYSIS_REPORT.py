...
def analyze_logs():
                    ...
                    last_loops.append(
                        {
                            ...
                            "exec_attempted": exec_attempted.group(1)
                            if exec_attempted
                            else "False",
    ...
    print("\n📋 LAST 5 LOOP CYCLES:")
    print(
        f"  {'ID':>3} | {'Decision':>7} | {'Exec Att':>8} | {'Result':>16} | {'Trade':>6} | {'PnL':>8}"
    )
    print(f"  {'-'*3}+{'-'*7}+{'-'*8}+{'-'*16}+{'-'*6}+{'-'*8}")
    for loop in last_loops:
        print(
            f"  {loop['id']:>3} | {loop['decision']:>7} | {loop['exec_attempted']:>8} | {loop['exec_result']:>16} | {loop['trade_opened']:>6} | {loop['pnl']:>8.2f}"
        )
    ...
    if exec_results.get("REJECTED", 0) > 0:
        ...
        print(
            f"   Most common reason: {max(rejection_reasons.items(), key=lambda x: x[1])[0] if rejection_reasons else 'N/A'}"
        )
