...
def parse_indicators(lines):
    ...
    for line in lines.split("\n"):
        ...
        try:
            ...
            if (
                "EXEC_REJECT" in line
                or "MICRO_BACKTEST_WIN_RATE_BELOW_THRESHOLD" in line
                or "EXEC_REJECT]" in line
            ):
                indicators["exec_rejects"].append(line.strip())

            # Detection: Gate drops / deadlock diagnostics
            if (
                "NO SIGNALS PASSED FILTERS" in line
                or "NO SIGNALS PASSED" in line
                or "NO SIGNALS PASSED FILTERS" in line.upper()
            ):
                    ...
                    for i, tok in enumerate(parts):
                        ...
                        if tok == "rejected" and i + 1 < len(parts):
                            try:
                                ...
                                count = None
                    if symbol:
                        indicators["rejection_counts"][symbol] = indicators["rejection_counts"].get(
                            symbol, 0
                        ) + (count or 1)
                except Exception:
                    pass
            ...
            if "Published TradeIntent:" in line or "Received signal for" in line:
                ...
                try:
                    if "from" in line:
                        ...
                        indicators["strategy_counts"][src_token] = (
                            indicators["strategy_counts"].get(src_token, 0) + 1
                        )
                except Exception:
                    pass
...
"""
    ...
    print("  Expected: 1-2 per hour")
    print(
        f"  Status: {Colors.GREEN if metrics['recovery_bypasses'] > 0 else Colors.YELLOW}{'✅ DETECTED' if metrics['recovery_bypasses'] > 0 else '⏳ Waiting...'}{Colors.ENDC}\n"
    )
    ...
    print("  Expected: 0-1 per hour")
    print(
        f"  Status: {Colors.GREEN if metrics['forced_rotations'] > 0 else Colors.YELLOW}{'✅ DETECTED' if metrics['forced_rotations'] > 0 else '⏳ Not yet...'}{Colors.ENDC}\n"
    )
    ...
    print("  Expected: 5-10 per hour")
    print(
        f"  Status: {Colors.GREEN if metrics['entry_sizes'] > 0 else Colors.YELLOW}{'✅ ALIGNED' if metrics['entry_sizes'] > 0 else '⏳ Waiting...'}{Colors.ENDC}\n"
    )

    print(f"{Colors.CYAN}Errors/Warnings:{Colors.ENDC}")
    print(
        f"  Count: {Colors.RED if metrics['errors'] > 0 else Colors.GREEN}{metrics['errors']}{Colors.ENDC}"
    )
    print(
        f"  Status: {Colors.GREEN if metrics['errors'] == 0 else Colors.RED}{'✅ No issues' if metrics['errors'] == 0 else '⚠️  CHECK LOGS'}{Colors.ENDC}\n"
    )
...
def monitor_live(log_file, refresh_interval=5):
    ...
    print(
        f"{Colors.YELLOW}🔍 Starting monitor... refreshing every {refresh_interval}s{Colors.ENDC}\n"
    )
    time.sleep(2)
    ...
    try:
        while True:
            ...
            if current_time - last_display >= refresh_interval:
                ...
                print("⏱️  Monitor running... (Press Ctrl+C to stop)")
