...
def get_recent_logs(filename, num_lines=100):
    ...
    try:
        result = subprocess.run(
            ["tail", "-n", str(num_lines), filename], capture_output=True, text=True, timeout=5
        )
...
def main():
    ...
    if loops:
        print("\n📋 LAST 10 LOOPS:")
        print(
            f"  {'ID':>3} | {'Decision':>6} | {'Attempted':>9} | {'Result':>15} | {'Trades':>6} | PnL {'':>7}"
        )
        print(f"  {'-'*3}+{'-'*6}+{'-'*9}+{'-'*15}+{'-'*6}+{'-'*14}")

        for loop in loops[-10:]:
            print(
                f"  {loop['id']:>3} | {loop['decision']:>6} | {loop['exec_attempted']:>9} | {loop['exec_result']:>15} | {loop['trade_opened']!s:>6} | {loop['pnl']:>7.2f}"
            )
