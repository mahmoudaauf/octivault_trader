...
class TradingSessionMonitor:
    ...
    async def _print_quick_status(self, elapsed_hours: float, remaining_hours: float):
        ...
        try:
            ...
            profit_loss_pct = (
                (profit_loss / self.initial_portfolio_value * 100)
    ...
    async def _print_hourly_report(self, hour: int):
        ...
        logger.info("=" * 80)
        logger.info(
            f"Elapsed Time: {int(elapsed // 3600)}h {int((elapsed % 3600) // 60)}m {int(elapsed % 60)}s"
        )
        ...
        logger.info(
            f"├─ Change: ${nav - self.initial_portfolio_value:,.2f} "
        ...
        logger.info(
            f"├─ Won: {self.trades_won} ({(self.trades_won / self.trades_executed * 100) if self.trades_executed > 0 else 0:.1f}%)"
        )
        logger.info(
            f"├─ Lost: {self.trades_lost} ({(self.trades_lost / self.trades_executed * 100) if self.trades_executed > 0 else 0:.1f}%)"
        )
        ...
        logger.info(
            f"├─ Profit Reinvestment Rate: {(self.reinvested_amount / self.total_profit * 100) if self.total_profit > 0 else 0:.1f}%"
        )
        logger.info(
            f"└─ Compounding Effect: {((self.current_portfolio_value - self.initial_portfolio_value) / self.initial_portfolio_value * 100):+.2f}%"
        )
    ...
    async def _print_final_report(self):
        ...
        logger.info(
            f"├─ Percentage Gain: {((nav - self.initial_portfolio_value) / self.initial_portfolio_value * 100):+.2f}%"
        )
        logger.info(f"├─ Highest Value: ${self.highest_portfolio_value:,.2f}")
        logger.info(f"├─ Lowest Value: ${self.lowest_portfolio_value:,.2f}")
        logger.info(
            f"└─ Max Drawdown: ${self.initial_portfolio_value - self.lowest_portfolio_value:,.2f} "
        ...
        logger.info(
            f"└─ Average Trade Duration: {(actual_duration * 3600 / self.trades_executed):.0f}s per trade"
        ...
        logger.info(
            f"├─ Profit Factor: {self.total_profit / self.total_loss if self.total_loss > 0 else float('inf'):.2f}"
        )
        logger.info(
            f"├─ Average Win: ${self.total_profit / self.trades_won if self.trades_won > 0 else 0:.2f}"
        )
        logger.info(
            f"├─ Average Loss: ${self.total_loss / self.trades_lost if self.trades_lost > 0 else 0:.2f}"
        )
        logger.info(
            f"└─ Expectancy: ${profit_loss / self.trades_executed if self.trades_executed > 0 else 0:.2f} per trade"
        )

        logger.info("\n🔄 REINVESTMENT & COMPOUNDING")
        logger.info(f"├─ Amount Reinvested: ${self.reinvested_amount:,.2f}")
        logger.info(
            f"├─ Reinvestment Rate: {(self.reinvested_amount / self.total_profit * 100) if self.total_profit > 0 else 0:.1f}%"
        )
        ...
        logger.info("└─ Sharpe Ratio: N/A (requires volatility data)")
...
async def run_monitored_trading_session(duration_hours: int = 3, paper_trading: bool = False):
    ...
    try:
        ...
        spec = importlib.util.spec_from_file_location(
            "master_orchestrator", Path(__file__).parent / "🎯_MASTER_SYSTEM_ORCHESTRATOR.py"
        )
        ...
        results = await asyncio.gather(monitor_task, orchestrator_task, return_exceptions=True)
...
async def main():
    ...
    parser = argparse.ArgumentParser(description="Run 3-hour trading session with monitoring")
    parser.add_argument(
        "--duration", type=int, default=3, help="Session duration in hours (default: 3)"
    )
    ...
    success = await run_monitored_trading_session(
        duration_hours=args.duration, paper_trading=args.paper
    )
