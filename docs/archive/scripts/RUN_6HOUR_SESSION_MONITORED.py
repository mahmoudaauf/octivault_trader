...
class CheckpointData:
    ...
    def add_event(self, event_type: str, description: str):
        self.events.append(
            {"time": datetime.now().isoformat(), "type": event_type, "description": description}
        )

    def add_issue(self, severity: str, description: str):
        self.issues.append(
            {"time": datetime.now().isoformat(), "severity": severity, "description": description}
        )
...
class Phase2Monitor:
    ...
    def check_recovery_bypass_triggered(self, log_line: str) -> bool:
        ...
        if "[Meta:SafeMinHold] Bypassing min-hold check" in log_line:
            ...
            self.bypass_logs.append({"time": datetime.now().isoformat(), "log": log_line})
            return True
        return False
    ...
    def check_forced_rotation_override(self, log_line: str) -> bool:
        ...
        if "[REA:authorize_rotation] ⚠️ MICRO restriction OVERRIDDEN" in log_line:
            ...
            self.override_logs.append({"time": datetime.now().isoformat(), "log": log_line})
            return True
        return False

    def track_entry_size(self, size_usdt: float):
        """Track entry size for consistency check"""
        self.entry_sizes.append({"time": datetime.now().isoformat(), "size": size_usdt})
    ...
    def get_summary(self) -> dict:
        ...
        return {
            ...
            "entry_size_avg": sum(entry_sizes_list) / len(entry_sizes_list)
            if entry_sizes_list
            else 0,
            "entry_size_consistency": "aligned"
            if all(abs(size - 25) < 1 for size in entry_sizes_list)
            else "misaligned",
...
class SessionMonitor:
    ...
    def start_session(self):
        ...
        logger.info(
            f"⏱️  DURATION: {self.session_duration_hours} hours ({self.session_duration_minutes} minutes)"
        )
    ...
    def create_checkpoint(self, checkpoint_num: int, checkpoint_name: str) -> CheckpointData:
        ...
        logger.info(
            f"⏱️  ELAPSED: {checkpoint.elapsed_minutes:.1f} minutes / {self.session_duration_minutes} minutes"
        )
        logger.info(f"{'='*80}")
...
async def run_6hour_session_monitored():
    ...
    try:
        ...
        cp2.add_metric(
            "Recovery Exit Bypass", f"Triggers: {monitor.phase2_monitor.recovery_bypasses}"
        )
        cp2.add_metric(
            "Forced Rotation Override", f"Triggers: {monitor.phase2_monitor.forced_rotations}"
        )
