...
PAT_SIGNAL = re.compile(
    r"Received signal for\s+([A-Z0-9]+)\s+from\s+([A-Za-z0-9_]+)|Published TradeIntent:\s*([A-Z0-9]+)\s+(BUY|SELL)",
    re.I,
)
...
def run_watchdog():
    ...
    for line in g:
        ...
        if PAT_SKIP_REJECT.search(line):
            ...
            # prune old
            while (
                recent_rejections[sym] and ts - recent_rejections[sym][0][0] > RECENT_WINDOW_SECONDS
            ):
                recent_rejections[sym].popleft()
            recent_sum = sum(c for _, c in recent_rejections[sym])
            if (
                total_rejections[sym] >= REJECTION_ALERT_THRESHOLD
                or recent_sum >= REJECTION_ALERT_THRESHOLD
            ):
                alert(
                    f"High rejection count for {sym}: total={total_rejections[sym]} recent({RECENT_WINDOW_SECONDS}s)={recent_sum}"
                )
        ...
        if PAT_NO_SIGNALS.search(line):
            ...
            if len(recent_no_signals) >= NO_SIGNALS_RATE_THRESHOLD:
                alert(
                    f"NO SIGNALS events high: {len(recent_no_signals)} occurrences in last {RECENT_WINDOW_SECONDS}s"
                )
