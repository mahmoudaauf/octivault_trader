...
class Phase4Verifier:
    ...
    def _check_previous_phases(self):
        ...
        if core_controller.exists():
            with open(core_controller) as f:
                content = f.read()
                if "FIX 1" in content and "FIX 5" in content:
                    print("  ✅ Phase 1: Implementation (All 5 fixes found)")
                    self.checks_passed += 1
                else:
                    print("  ❌ Phase 1: Some fixes not found")
                    self.checks_failed += 1
        else:
            print("  ❌ Phase 1: meta_controller.py not found")
            self.checks_failed += 1
        ...
        if unit_tests.exists():
            ...
            print("  ❌ Phase 2: Unit tests not found")
            self.checks_failed += 1

        # Check Phase 3: Integration Tests
        integration_tests = (
            self.workspace_root / "tests" / "test_portfolio_fragmentation_integration.py"
        )
        if integration_tests.exists():
            with open(integration_tests) as f:
                content = f.read()
                test_count = content.count("def test_")
                print(f"  ✅ Phase 3: Integration Tests ({test_count} tests found)")
                self.checks_passed += 1
        else:
            print("  ❌ Phase 3: Integration tests not found")
            self.checks_failed += 1
    ...
    def _check_configuration(self):
        ...
        if sandbox_config.exists():
            print("  ✅ Sandbox configuration: config/sandbox.yaml")
            self.checks_passed += 1
        else:
            print("  ❌ Sandbox configuration not found")
            self.checks_failed += 1

        # Check monitoring script
        monitor_script = self.workspace_root / "monitoring" / "sandbox_monitor.py"
        if monitor_script.exists():
            print("  ✅ Monitoring system: monitoring/sandbox_monitor.py")
            self.checks_passed += 1
        else:
    ...
    def _check_infrastructure(self):
        ...
        if deployment_guide.exists():
            print("  ✅ Deployment guide: PHASE_4_DEPLOYMENT_GUIDE.md")
            self.checks_passed += 1
        else:
        ...
        if readiness_doc.exists():
            print("  ✅ Sandbox readiness: PHASE_4_SANDBOX_READINESS.md")
            self.checks_passed += 1
        else:
            print("  ❌ Sandbox readiness document not found")
            self.checks_failed += 1
