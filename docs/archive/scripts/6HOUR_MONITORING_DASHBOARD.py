...
class MonitoringDashboard:
    ...
    def find_orchestrator_pid(self):
        ...
        try:
            result = subprocess.run(
                ["pgrep", "-f", "MASTER_SYSTEM_ORCHESTRATOR"], capture_output=True, text=True
            )
    ...
    def extract_log_metrics(self):
        ...
        try:
            ...
            for line in reversed(lines):
                ...
                if "dust" in line.lower() and (
                    "heal" in line.lower() or "consolidated" in line.lower()
                ):
    ...
    def display_metrics(self, system_metrics, log_metrics, elapsed_time):
        ...
        print("\n" + "=" * 80)
        print("📊 6-HOUR ASSESSMENT MONITORING DASHBOARD")
        ...
        if system_metrics.get("status") == "running":
            ...
            print(
                f"   Memory: {system_metrics.get('memory_mb', 0):.0f} MB ({system_metrics.get('memory_percent', 0):.1f}%)"
            )
                ...
                json.dump(
                    {
                        ...
                        "checkpoints": {
                            str(h): t.isoformat() for h, t in self.checkpoint_times.items()
                        },
