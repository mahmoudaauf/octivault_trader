...
class SixHourSession:
    ...
    def start_system(self):
        ...
        try:
            ...
            cmd = ["python3", "🎯_MASTER_SYSTEM_ORCHESTRATOR.py"]

            self.log(f"📍 Command: {' '.join(cmd)}", "INFO")
            self.log("📍 Environment: APPROVE_LIVE_TRADING=YES", "INFO")
    ...
    def run_session(self):
        ...
        self.log("⏱️  Duration: 6 hours", "START")
        self.log("📊 Checkpoints: Every 30 minutes (12 total)", "START")
        self.log("=" * 60, "START")
        self.log("", "START")
    ...
    def generate_report(self):
        ...
        elapsed = self.get_elapsed()
        self.log("✅ Session completed successfully!", "REPORT")
        ...
        for cp in self.checkpoints:
            self.log(
                f"  CP {cp['checkpoint']:2d}: {cp['elapsed']} elapsed | "
                f"{cp['progress_percent']:5.1f}% | "
                f"CPU {cp['metrics']['cpu_percent']:5.1f}% | "
                f"Mem {cp['metrics']['memory_mb']:7.1f} MB",
                "REPORT",
            )
        self.log("", "REPORT")
