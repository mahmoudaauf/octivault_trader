...
class CheckpointMonitor:
    ...
    def start_trading_system(self, paper_trading: bool = False):
        ...
        try:
            ...
            cmd = [sys.executable, str(project_root / "🎯_MASTER_SYSTEM_ORCHESTRATOR.py")]
    ...
    def check_component_health(self) -> tuple[bool, dict]:
        ...
        metrics = {"components_running": 0, "components_healthy": 0, "unhealthy_components": []}
    ...
    async def run_checkpoint(self, checkpoint_num: int) -> dict:
        ...
        if adapt_metrics["market_adaptation"]:
            logger.info("✅ System adapting to market conditions")
        ...
        if profit_metrics["profitable"]:
            logger.info("✅ System is PROFITABLE")
            ...
            logger.warning("⚠️  Not yet profitable")
            logger.info(f"   ├─ Current P&L: ${profit_metrics['pnl']:.2f}")
            logger.info(f"   └─ Balance: ${profit_metrics['balance']:.2f}")
        ...
        checkpoint_data["checks"]["health"] = {"passed": health_ok, "metrics": health_metrics}
        ...
        # Summary
        all_passed = all(c["passed"] for c in checkpoint_data["checks"].values())
        ...
        if all_passed:
            ...
            failed_checks = [
                name for name, data in checkpoint_data["checks"].items() if not data["passed"]
            ]
    ...
    async def run_session(self, paper_trading: bool = False):
        ...
        try:
            while self.is_process_alive():
                ...
                logger.info(
                    f"⏳ Waiting {self.checkpoint_interval_minutes} minutes for next checkpoint..."
                )
    ...
    async def print_summary(self):
        ...
        if self.checkpoints:
            passed = sum(
                1 for cp in self.checkpoints if all(c["passed"] for c in cp["checks"].values())
            )
