...
class ContinuousMonitor:
    ...
    def display_metrics(self, health):
        ...
        if health["process_info"]:
            ...
            print(
                f"   PID: {info['pid']} | CPU: {info['cpu_percent']:.1f}% | Memory: {info['memory_mb']:.0f} MB"
            )

        print(f"\n{Colors.CYAN}Phase 2 Indicators (Cumulative):{Colors.ENDC}")
        print(
            f"  🔓 Recovery Bypasses:      {Colors.YELLOW}{self.metrics['recovery_bypasses']:>4}{Colors.ENDC}"
        )
        print(
            f"  🔄 Forced Rotations:       {Colors.YELLOW}{self.metrics['forced_rotations']:>4}{Colors.ENDC}"
        )
        print(
            f"  💰 Entry Sizes Aligned:    {Colors.YELLOW}{self.metrics['entry_sizes']:>4}{Colors.ENDC}"
        )
        print(
            f"  📊 Trades Executed:        {Colors.YELLOW}{self.metrics['trades_executed']:>4}{Colors.ENDC}"
        )

        print(f"\n{Colors.CYAN}System Issues:{Colors.ENDC}")
        print(
            f"  ❌ Errors:                 {Colors.RED if self.metrics['errors'] > 0 else Colors.GREEN}{self.metrics['errors']:>4}{Colors.ENDC}"
        )
        print(
            f"  ⚠️  Warnings:               {Colors.YELLOW}{self.metrics['warnings']:>4}{Colors.ENDC}"
        )
    ...
    def display_status_line(self):
        ...
        print("Press Ctrl+C to stop | Commands: 'ps' for process, 'log' for tail\n")
