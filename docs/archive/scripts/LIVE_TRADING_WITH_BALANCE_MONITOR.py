...
class BalanceMonitor:
    ...
    def get_performance_summary(self):
        ...
        return {
            ...
            "session_duration_seconds": len(self.balance_history)
            * 60,  # Assuming 1 update per minute
        }
...
def startup_production_with_monitoring():
    ...
    print("✅ State files: state/operational_state.json")
    print()
    ...
    print("   State recovery: ENABLED")
    print("   Auto-recovery: ENABLED")
    print("   Balance monitoring: ENABLED")
    print("   State persistence: ENABLED (every 60 seconds)")
    print()
...
async def main_trading_loop_with_monitoring(
    state_mgr: SystemStateManager, balance_monitor: BalanceMonitor
):
    ...
    try:
        while True:
            ...
            state_mgr.update_operational_state(
                phase="live_trading", task=f"continuous_trading_cycle_{cycle_count}"
            )
            ...
            if controller:
                try:
                    ...
                    state_mgr.record_error(error=str(e), context=f"trading_cycle_{cycle_count}")
        ...
        state_mgr.record_task_completion(task="live_trading_session", status="completed_by_user")
        ...
        state_mgr.update_operational_state(final_balance_performance=final_perf)
        print("\n✅ Final state saved")

    except Exception as e:
        print(f"\n❌ Error in trading loop: {e}")
        state_mgr.record_error(error=str(e), context="main_trading_loop_with_monitoring")
        balance_monitor.display_summary()
