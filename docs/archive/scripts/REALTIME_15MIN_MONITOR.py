...
def get_log_stats(log_file: str, since_lines: int = 0):
    ...
    try:
        result = subprocess.run(
            f"tail -n 500 {log_file}", shell=True, capture_output=True, text=True, timeout=2
        )
        ...
        stats = {
            ...
            "models_trained": len(
                [l for l in lines if "Model saved to" in l or "model saved" in l]
            ),
...
def get_process_status():
    ...
    try:
        ...
        if result.stdout.strip():
            ...
            if len(parts) >= 11:
                return {"pid": parts[1], "cpu": parts[2], "mem": parts[3], "status": "RUNNING ✅"}
...
def print_status_check(
    check_num: int, elapsed_sec: int, stats: dict, proc_status: dict, lines: list
):
    """Print a formatted status check"""
    print("\n" + "=" * 90)
    print(
        f"📊 STATUS CHECK #{check_num} - {datetime.now().strftime('%H:%M:%S')} | "
    ...
    if "pid" in proc_status:
        print(
            f"   PID: {proc_status['pid']} | CPU: {proc_status.get('cpu', '?')}% | "
...
def main():
    ...
    print("⏱️  Duration: 15 minutes (900 seconds)")
    print("📊 Status updates: Every 2 minutes (120 seconds)")
