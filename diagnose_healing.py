...
async def diagnose():
    ...
    try:
        ...
        print(f"  • Total positions: {len([b for b, v in balances.items() if b != 'USDT' and float(v.get('free', 0)) > 0])}")
        ...
        print(f"  GATE 1: dust_value > min_dead_to_heal?")
        ...
        if positions:
            ...
            for sym, pos in sorted(positions.items(), key=lambda x: x[1]['value'], reverse=True)[:10]:
                status = "🔴 DUST" if pos['value'] < 25 else "🟢 PRODUCTIVE"
                print(f"  {status}  {sym:8s} qty={pos['qty']:.8f} price=${pos['price']:>10.2f} value=${pos['value']:>8.2f}")
            if len(positions) > 10:
