            ...
            if asyncio.iscoroutine(lock):
                ...
                class _AwaitableLockWrapper:
                    ...
                    async def __aenter__(self):
                        ...
                        if hasattr(self._lock, "__aenter__") and asyncio.iscoroutinefunction(
                    ...
                    async def __aexit__(self, exc_type, exc, tb):
                        ...
                        if hasattr(self._lock, "__aexit__") and asyncio.iscoroutinefunction(
                            self._lock.__aexit__
                        ):
...
async def inject_agent_signal(
    ...
    if action != raw_action:
        logger.warning(
            f"Signal action normalized to HOLD (was '{raw_action}') for {agent_name}:{sym}"
        )
    ...
    logger.debug(f"[{agent_name}] Injected signal for {sym}: {action} " f"(confidence={conf:.2f})")


async def get_latest_signal(
    shared_state: Any, agent_name: str, symbol: str
...
async def preload_ohlcv(
    ...
    logger.info(
        f"📦 Preloading OHLCV data for {len(symbols)} symbols (max_concurrency={max_conc})..."
    )
...
async def inject_signals_batch(shared_state: Any, signals: list[dict[str, Any]]) -> None:
    ...
    for sig in signals:
        ...
        if not agent_name or not symbol:
            logger.warning(
                f"Skipping malformed signal in batch due to missing agent_name or symbol: {sig}"
            )
            continue
