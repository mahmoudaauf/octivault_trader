...
class SandboxMonitor:
    ...
    def __init__(self, config_path: str = None):
        self.config_path = (
            config_path
            or "/Users/mauf/Desktop/Octi AI Trading Bot/octivault_trader/config/sandbox.yaml"
        )
            ...
            if metrics.errors_this_cycle:
                ...
                for error in metrics.errors_this_cycle:
                    self.error_log.append(
                        {"cycle": self.cycle_count, "timestamp": metrics.timestamp, "error": error}
                    )
    ...
    async def _get_portfolio_metrics(self) -> PortfolioMetrics:
        ...
        consolidation_triggered = (
            self.current_phase >= 3
            and health_status == "SEVERE"
            and (self.cycle_count % 120 == 0)  # Every 2 hours
        )
    ...
    def _record_health_transition(self, prev_status: str, curr_status: str):
        ...
        if self.metrics_history:
            ...
            prev_h = (
                self.metrics_history[-2].herfindahl_index
            ...
            prev_m = (
                self.metrics_history[-2].size_multiplier if len(self.metrics_history) > 1 else 1.0
            )
            ...
            self.logger.info(
                f"Health transition: {prev_status} → {curr_status} (H: {herfindahl_change:.3f}, M: {multiplier_change:.2f}x)"
            )
    ...
    def _print_cycle_summary(self):
        ...
        # Performance
        avg_cycle_time = (
            sum(m.cycle_duration_ms for m in self.metrics_history) / len(self.metrics_history)
            if self.metrics_history
            else 0
        )
        avg_check_time = (
            sum(m.health_check_duration_ms for m in self.metrics_history)
        ...
        self.logger.info(
            f"  Status Distribution: HEALTHY={healthy} FRAGMENTED={fragmented} SEVERE={severe}"
        )
    ...
    def _generate_report(self) -> dict[str, Any]:
        ...
        report = {
            ...
            "health_distribution": {
                ...
                "severe": severe,
                "healthy_percentage": (healthy / self.cycle_count * 100)
                if self.cycle_count > 0
                else 0,
                "fragmented_percentage": (fragmented / self.cycle_count * 100)
                if self.cycle_count > 0
                else 0,
                "severe_percentage": (severe / self.cycle_count * 100)
                if self.cycle_count > 0
                else 0,
            },
            "consolidation": {
                "total_events": self.consolidation_events,
                "frequency_per_hour": self.consolidation_events / total_duration
                if total_duration > 0
                else 0,
            ...
            "validation": {
                ...
                "ready_for_phase_5": (
                    error_rate == 0 and avg_cycle < 50 and self.consolidation_events > 0
                ),
            },
        }
    ...
    def _print_final_summary(self, report: dict[str, Any]):
        ...
        print("\nHealth Distribution:")
        print(
            f"  HEALTHY:    {report['health_distribution']['healthy']:3d} ({report['health_distribution']['healthy_percentage']:5.1f}%)"
        )
        print(
            f"  FRAGMENTED: {report['health_distribution']['fragmented']:3d} ({report['health_distribution']['fragmented_percentage']:5.1f}%)"
        )
        print(
            f"  SEVERE:     {report['health_distribution']['severe']:3d} ({report['health_distribution']['severe_percentage']:5.1f}%)"
        )
        ...
        print("\nValidation Results:")
        print(
            f"  Zero Regressions:   {'✅ YES' if report['validation']['zero_regressions'] else '❌ NO'}"
        )
        print(
            f"  All Checks Success: {'✅ YES' if report['validation']['all_health_checks_successful'] else '❌ NO'}"
        )
        print(
            f"  Performance OK:     {'✅ YES' if report['performance']['performance_acceptable'] else '❌ NO'}"
        )
        print(
            f"  Ready for Phase 5:  {'✅ YES' if report['validation']['ready_for_phase_5'] else '❌ NO'}"
        )
        print("\n" + "=" * 80)
