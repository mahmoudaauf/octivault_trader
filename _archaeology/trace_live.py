...
SKIP_DIRS = {".git", "__pycache__", ".venv", "venv", "env", "node_modules",
             "_archive", "_archaeology", ".claude", "artifacts", ".mypy_cache",
...
def main():
    ...
    for path in unreached_sorted:
        if path.startswith("src/"):     buckets["src/"].append(path)
        elif path.startswith("agents/"):buckets["agents/"].append(path)
        elif path.startswith("automation/"): buckets["automation/"].append(path)
        elif "/" not in path:           buckets["root/"].append(path)
        else:                           buckets["other/"].append(path)
    ...
    for k, v in buckets.items():
        print(f"  {k:<14} {len(v)}")
    print(f"\nDetails written to:")
    print(f"  _archaeology/live_dependency_closure.txt")
    print(f"  _archaeology/unreached_from_entry.txt")
