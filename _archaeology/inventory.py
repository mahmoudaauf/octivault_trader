...
PATCH_RE = re.compile(
    r"(?i)(_fix|fix_|_old|old_|_backup|backup_|_deprecated|"
...
def main():
        ...
        rows.append(
            {
                ...
                "is_patch_named": bool(PATCH_RE.search(p.name))
                or p.name.isupper().__bool__()
                and "FIX" in p.name.upper(),
                "screams_fix": "FIX" in p.name.upper()
                or "BACKUP" in p.name.upper()
                or "OLD" in p.name.upper(),
    ...
    out.append(
        f"| Other folders               | {sum(1 for r in rows if not r['in_src'] and not r['at_root'])} |"
    )
    out.append("")
    ...
    if not duplicates:
        ...
        for key, group in sorted(duplicates.items(), key=lambda kv: -len(kv[1]))[:30]:
            ...
            for r in sorted(group, key=lambda x: x["path"]):
                out.append(
                    f"- `{r['path']}` — {r['loc']} LOC, {r['status']}, imported_by={r['imported_by']}"
                )
            out.append("")

    out.append("## 7. Top 20 LIBRARIES by fan-in (most-imported = most critical)")
    out.append("")
    libs = sorted([r for r in rows if r["status"] == "🟢 LIBRARY"], key=lambda r: -r["imported_by"])
    ...
    print("✅ Wrote ARCHITECTURE_REALITY.md")
    print(f"   Files scanned: {len(rows)}")
    print(
        f"   Entry points: {len(eps)} | Libraries: {len(libs)} | "
