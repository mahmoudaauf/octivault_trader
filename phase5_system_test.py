...
logging.basicConfig(
    ...
    handlers=[logging.StreamHandler(), logging.FileHandler("paper_trade_phase5.log")],
)
...
class Phase5SystemTest:
    ...
    async def run_trading_cycle(self, cycle_num: int):
        ...
        try:
                ...
                if isinstance(signal, dict):
                    ...
                    if decision:
                        ...
                        if order_result:
                            ...
                            logger.info(
                                f"  [Cycle {cycle_num}] ✅ BUY {symbol} @ {prices.get(symbol, 40000)}"
                            )
                            ...
                            if random.random() < 0.1:  # 10% chance to SELL
                                ...
                                if sell_result:
                                    ...
                                    logger.info(
                                        f"  [Cycle {cycle_num}] ✅ SELL {symbol} (FIX #2 active)"
                                    )
            ...
            if cycle_num % 10 == 0:
                ...
                logger.info(
                    f"Cycle {cycle_num}/{self.expected_cycles} - Latency: {cycle_time:.3f}s (Avg: {avg_latency:.3f}s)"
                )
    ...
    async def run(self):
        ...
        for cycle_num in range(1, min(self.expected_cycles + 1, 100)):  # Run 100 cycles for demo
            ...
            await asyncio.sleep(
                max(0, self.cycle_interval - (time.time() - self.start_time.timestamp()))
            )
    ...
    async def generate_report(self):
        ...
        logger.info(
            f"Orders Placed: {self.orders_placed['BUY']} BUY + {self.orders_placed['SELL']} SELL"
        )
        ...
        results = {
            ...
            "status": "✅ SUCCESS"
            if self.cycles_executed > self.expected_cycles * 0.9
            else "⚠️ PARTIAL",
        }
        ...
        logger.info("Results saved to phase5_results.json")
...
class MockExchangeClient:
    ...
    async def get_prices(self, symbols: list) -> dict[str, float]:
        return {
            sym: 40000.0 + random.uniform(-500, 500) if sym == "BTC" else 2500.0 for sym in symbols
        }
