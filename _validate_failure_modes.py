...
for e in sl_events[-8:]:
    print(f"   {e.get('ts','')[:19]} {e.get('event','')} | {e.get('symbol','')} | tag={e.get('tag','')}")
...
print(f"\n🟢 LAST 5 BUYS:")
for e in buys[-5:]:
    print(f"   {e.get('ts','')[:19]} {e.get('symbol',''):<10} qty={e.get('executed_qty')} @ ${e.get('avg_price')} tag={e.get('tag')}")
...
for sym in ["ETHUSDT", "SOLUSDT", "XRPUSDT"]:
    ...
    for b in sym_buys[-2:]:
        print(f"      BUY  {b.get('ts','')[:19]} qty={b.get('executed_qty')} @ ${b.get('avg_price')} tag={b.get('tag')}")
    for s in sym_sells[-2:]:
        print(f"      SELL {s.get('ts','')[:19]} qty={s.get('executed_qty')} @ ${s.get('avg_price')} tag={s.get('tag')}")
