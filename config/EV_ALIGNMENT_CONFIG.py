...
class CanonicalEVConfig:
    ...
    def validate_alignment(cls, uure_round_trip: float, em_round_trip: float) -> tuple[bool, str]:
        ...
        if difference <= threshold:
            ...
            return (
                False,
                f"✗ Misaligned (UURE={uure_round_trip:.6f}% EM={em_round_trip:.6f}% diff={difference:.6f}%)",
            )

    @classmethod
    def validate_multiplier_consistency(
        cls, regime: str, uure_mult: float, em_mult: float
    ) -> tuple[bool, str]:
        """
        ...
        if difference <= threshold:
            return (
                True,
                f"✓ {regime}: Consistent (UURE={uure_mult:.2f} EM={em_mult:.2f} diff={difference:.4f})",
            )
        else:
            return (
                False,
                f"✗ {regime}: Inconsistent (UURE={uure_mult:.2f} EM={em_mult:.2f} diff={difference:.4f})",
            )
...
if __name__ == "__main__":
    ...
    print(
        f"EV Multipliers: normal={CANONICAL_EV_CONFIG.EV_MULT_NORMAL} bull={CANONICAL_EV_CONFIG.EV_MULT_BULL} other={CANONICAL_EV_CONFIG.EV_MULT_OTHER}"
    )
